 ...
 */
@Service
public class BdcXtLscsServiceImpl implements BdcXtLscsService {
    private static final Logger LOGGER = LoggerFactory.getLogger(BdcXtLscsServiceImpl.class);
    ...
    @Override
    public int addValues(List<BdcXtLscsDO> lscsDOList) {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public int deleteRecords(List<BdcXtLscsDO> lscsDOList) {
        if(CollectionUtils.isEmpty(lscsDOList)) {
            throw new MissingArgumentException("删除临时参数操作传参为空");
        }

        for(BdcXtLscsDO xtLscsDO : lscsDOList) {
