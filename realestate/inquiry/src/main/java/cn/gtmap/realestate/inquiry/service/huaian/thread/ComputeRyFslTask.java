 ...
 */
public class ComputeRyFslTask implements Runnable{
    private static final Logger LOGGER = LoggerFactory.getLogger(ComputeRyFslTask.class.getName());
    ...
    public ComputeRyFslTask(GzltjQO gzltjQO, List<BdcRyGzltjXmxxVO> bdcRyGzltjXmxxVOList, BdcGzltjXmxxMapper bdcGzltjXmxxMapper){
    ...
    public void run() {
        ...
        if(Constants.GZLTJ_TJLX_XM.equals(gzltjQO.getTjlx())){
            bdcGzltjXmxxDTOList = this.bdcGzltjXmxxMapper.countRyFslByXmid(gzltjQO);
        }else if(Constants.GZLTJ_TJLX_SLBH.equals(gzltjQO.getTjlx())){
            bdcGzltjXmxxDTOList = this.bdcGzltjXmxxMapper.countRyFslByGzlslid(gzltjQO);
        }

        // 2、将人员复审量信息转换成 Map 对象 ( Key:人员ID, Value: 复审量 )
        if(CollectionUtils.isNotEmpty(bdcGzltjXmxxDTOList)){
            ...
            while (bdcGzltjXmxxVOIterator.hasNext()) {
                ...
                if(StringUtils.isNotBlank(bdcRyGzltjXmxxVO.getRyid()) && resultMap.containsKey(bdcRyGzltjXmxxVO.getRyid())){
                    bdcRyGzltjXmxxVO.setFsl(resultMap.get(bdcRyGzltjXmxxVO.getRyid()));
                }
            }
        }
        LOGGER.info("计算人员复审量结束，耗时：{}， 查询参数：{}" , System.currentTimeMillis() - startTime, JSON.toJSONString(gzltjQO));
    }
}
