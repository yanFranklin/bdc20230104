$(document).ready(function () {
    ...
    getZd(function (data) {
        ...
        // 获取是否终生
        $.each(data.sf, function (index, item) {
            $('#zsjz').append('<option value="' + item.DM + '">' + item.MC + '</option>');
        });

        form.render('select');
    });

    form.on('select(zsjz)',function (data) {
    ...
    if (readOnly || !isNullOrEmpty(formStateId)) {
        getStateByIdQlxx(readOnly, formStateId, "jzq");
    }
    var showXgLog = $.getUrlParam("showXgLog");
    if(showXgLog){
        queryXgLog(xmid);
        renderChange("",form,"jzq");
        renderDate(form,"jzq");
    }
});