 ...
 * @description 土地承包经营权、农用地的其他使用权登记信息（非林地）表单JS
 */
$(document).ready(function () {
    ...
    getZd(function (data) {
        ...
        $.each(data.tdsyqxz, function (index, item) {
            $('#tdsyqxz').append('<option value="' + item.DM + '">' + item.MC + '</option>');
        });
        $.each(data.syttlx, function (index, item) {
            $('#syttlx').append('<option value="' + item.DM + '">' + item.MC + '</option>');
        });
        $.each(data.yzyfs, function (index, item) {
            $('#yzyfs').append('<option value="' + item.DM + '">' + item.MC + '</option>');
        });
        $.each(data.ydyhfl, function (index, item) {
            $('#ydyhfl').append('<option value="' + item.DM + '">' + item.MC + '</option>');
        });
     ...
     * 保存更新表单数据
     */
    saveForm('/qlxx/tdcbnydsyq');

    //设置权限控制样式
    if (readOnly || !isNullOrEmpty(formStateId)) {
        getStateByIdQlxx(readOnly, formStateId, "tdcbjyqNyddqtsyq");
    }
    var showXgLog = $.getUrlParam("showXgLog");
    if(showXgLog){
        queryXgLog(xmid);
        renderChange("",form,"tdcbjyqNyddqtsyq");
        renderDate(form,"tdcbjyqNyddqtsyq");
    }
});