...
$(document).ready(function () {
    ...
    getZd(function (data) {
        ...
        $.each(data.gjzwlx, function (index, item) {
            $('#gjzwlx').append('<option value="' + item.DM + '">' + item.MC + '</option>');
        });
     ...
     * 保存更新表单数据
     */
    saveForm('/qlxx/gjzwsyq');

    //设置权限控制样式
    if (readOnly || !isNullOrEmpty(formStateId)) {
        getStateByIdQlxx(readOnly, formStateId, "gzwsyq");
    }

    var showXgLog = $.getUrlParam("showXgLog");
    if(showXgLog){
        queryXgLog(xmid);
        renderChange("",form,"gzwsyq");
        renderDate(form,"gzwsyq");
    }
});
