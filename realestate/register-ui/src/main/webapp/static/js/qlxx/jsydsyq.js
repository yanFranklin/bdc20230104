 ...
 * @description 建设用地使用权、宅基地使用权登记信息表单JS
 */
...
$(document).ready(function () {
    ...
    getZd(function (data) {
        ...
        $.each(data.qlsdfs, function (index, item) {
     ...
     * 保存更新表单数据
     */
    saveForm('/qlxx/jsydsyq');

    /**
     * 设置权限控制样式
     */
    if (readOnly || !isNullOrEmpty(formStateId)) {
        getStateByIdQlxx(readOnly, formStateId, "jsydsyq");
    }
    var showXgLog = $.getUrlParam("showXgLog");
    if(showXgLog){
        queryXgLog(xmid);
        renderChange("",form,"jsydsyq");
        renderDate(form,"jsydsyq");
    }
});