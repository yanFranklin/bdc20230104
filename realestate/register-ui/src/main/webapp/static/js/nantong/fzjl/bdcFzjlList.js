    ...
    layui.use(['form', 'table', 'jquery', 'formSelects'], function () {
                ...
                case 'endProcess':
                    if (sfyjf) {
                        endProcessAjax(processInsId).done(function () {
                            if (endProcessSucceed && parent.queryObject && parent.fzjlIndex) {
                                layer.alert("办结成功，即将关闭页面！");
                                parent.tableReload('pageTable', {data: JSON.stringify(parent.queryObject)}, '/realestate-inquiry-ui/dtcx/list/result');
                                setTimeout(function () {
                                    parent.layer.close(parent.fzjlIndex);
                                }, 1500);
                            }
                        });
                    } else {
        ...
        table.on('tool(dataTable)', function (obj) {
            ...
            layer.open({
                ...
                area: ['100%', '100%'],
                resize: true,
                content: '/realestate-register-ui/view/fzjl/fzjl.html?zsid=' + data.zsid + '&taskId=' + taskId + '&processInsId=' + processInsId + "&formStateId=" + formStateId + "&readOnly=" + readOnly
            });
