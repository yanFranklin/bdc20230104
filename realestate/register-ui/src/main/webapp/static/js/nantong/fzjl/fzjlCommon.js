 ...
 * @returns {*}
 */
function checkJfzt(processInsId) {
    var deferred = $.Deferred();
    $.ajax({
        url: BASE_URL + '/bdcdy/jfxx/' + processInsId,
        type: "GET",
        dataType: "json",
        async: false,
        success: function (data) {
 ...
 * @returns {boolean}
 */
function endProcessAjax(gzlslid) {
    addModel();
    var deferred = $.Deferred();
    $.ajax({
        type: "POST",
        url: BASE_URL + '/workflow/end',
        data: {gzlslid: gzlslid},
        async: false,
        success: function () {
            endProcessSucceed = true;
            successMsg("办结成功！");
        }, error: function (e) {
            endProcessSucceed = false;
            warnMsg("请求失败!");
        }, complete: function () {
            deferred.resolve();
            removeModel();
...
function sf(slbh, gxbz) {
    $.ajax({
        url: "/realestate-register-ui/rest/v1.0/fzjl/sf?slbh=" + slbh + "&gxbz=" + gxbz,
        type: "GET",
        async: false,
        success: function () {
