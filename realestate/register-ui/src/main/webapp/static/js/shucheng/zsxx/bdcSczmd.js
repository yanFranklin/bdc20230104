...
layui.use(['element', 'form', 'jquery', 'laydate', 'laytpl', 'layer','response'], function () {
    ...
    function renderZmdData() {
        ...
        url = "/realestate-register-ui/rest/v1.0/zsxx/zmd?gzlslid=" + processInsId + "&bdcdyh=" + bdcdyh + "&zsyl=" + zsyl;
        //获取数据
        $.ajax({
            url: url,
            ...
            success: function (data) {
                if (data) {
                    ...
                    //证明内容可配置
                    if(!isNullOrEmpty(data.zmnr)) {
                        $(".bdc-zzsm").html("");
                        var zmnrArr =data.zmnr.split("$");
                        for(var i = 0; i< zmnrArr.length; i++){
                            if(i ===0){
                                $(".bdc-zzsm").append("<span>" + zmnrArr[i] + "</span>");
                            }else {
                                $(".bdc-zzsm").append("<p>" + zmnrArr[i] + "</p>");
                            }
                        }
                    }
