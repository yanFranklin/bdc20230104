...
layui.use(['element', 'form', 'jquery', 'laydate', 'laytpl', 'layer'], function () {
    ...
    form.on('submit(printBtn)', function (data) {
        ...
        // 打印的xmid，优先取有证书的xmid
        var printXmid;
        if (xmidList && xmidList.length > 0) {
            printXmid = xmidList[0];
        } else {
            printXmid = xmid;
        }
        var dataUrl = getIP() + "/realestate-register-ui/rest/v1.0/print/fzjl/xml?dylx=" + dylx + "&xmid=" + printXmid + "&gzlslid=" + processInsId;
