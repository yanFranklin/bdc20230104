...
layui.use(['jquery', 'form', 'layer'], function () {
    ...
    $(function () {
        ...
        if(!isNullOrEmpty(hidenPrevButton)){
            $("#prev").addClass("bdc-hide");
        }
        if(isNotBlank(lclx) && lclx =="pllc"){
            $("#copycq").addClass("bdc-hide");
        }
        //判断非批量流程即为单个流程时隐藏证书证明的关闭按钮
        if(lclx =="jdlc"){
            $("#close").hide()
        }
        // 根据配置内容
        ...
        form.on('submit(end)', function (data) {
            ...
            addModel();
            // 更新权利其他状况、附记和产权证号
            var result = updateZsQlqtzkFjAndCqzh(zsid, xmid, qlqtzk, fj, bdcqzh);
        ...
        form.on("submit(prev)", function (data) {
            var backUrl = getContextPath() + "/view/xxbl/lc_qlxx.html?qllx=" + qllx + "&bdcdyfwlx=" + bdcdyfwlx + "&xmid=" + xmid + "&processInsId=" + processInsId + "&type=" + type + "&taskId=" + taskId + "&ygzlslid=" + ygzlslid + "&readOnly=" + readOnly + "&lclx=" + lclx;;
         ...
         * 监听复制
         */
        form.on('submit(copybdcdy)', function (data) {
            plbl(processInsId,qllx,xmid,gzldyid);
        });
        form.on('submit(copycq)', function (data) {
            plblCq(processInsId,qllx,xmid,gzldyid);
        });
 ...
 * @param data
 */
function plbl(processInsId,qllx,xmid,gzldyid) {
    var yxmid = xmid;
    var ybdcdyh = deleteWhitespace($("#bdcdyh").val(),"all");
    var qllx = qllx;
    if (isNotBlank(yxmid)) {
        $.ajax({
            url: getContextPath() + '/rest/v1.0/blxx/getGzldyid?xmid=' + xmid,
            type: 'GET',
            success: function (gzldyid) {
                ...
                var index = layer.open({
                    ...
                    maxmin: true,
                    area: ['1150px', '600px'],
                    content: '../xxbl/plxxbl_selectbdcdyh.html?yxmid=' + yxmid + '&gzldyid=' + gzldyid + '&ybdcdyh=' + ybdcdyh + '&qllx=' + qllx
                });
                layer.full(index);
            }, error: function (xhr, status, error) {
                removeModel();
...
function plblCq(processInsId,qllx,xmid,gzldyid) {
    var yxmid = xmid;
    var ybdcdyh = deleteWhitespace($("#bdcdyh").val(),"all");
    var qllx = qllx;
    if (isNotBlank(yxmid)) {
        $.ajax({
            ...
            success: function (gzldyid) {
                ...
                var index = layer.open({
                    ...
                    maxmin: true,
                    area: ['1150px', '600px'],
                    content: '../xxbl/plxxbl_selectbdcq.html?gzldyid=' + gzldyid+'&yxmid=' + yxmid
                });
                layer.full(index);
            }, error: function (xhr, status, error) {
                removeModel();
