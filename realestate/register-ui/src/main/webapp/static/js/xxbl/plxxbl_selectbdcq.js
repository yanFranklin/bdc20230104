    ...
    $(function () {
            ...
            {type: 'checkbox', width: 50, fixed: 'left'},
            {
                field: 'bdcqzh', title: '不动产权证号', width: "25%", templet: function (obj) {
                    if (!isNullOrEmpty(obj.bdcqzh)) {
                        ...
                        return '<span style="color:#1d87d1; cursor:pointer" title="' + obj.bdcqzh + '" onclick="">' + obj.bdcqzh + '</span>'
                    } else {
                        return '<span></span>';
                    }
                }
        ...
        table.on('toolbar(blTable)', function (obj) {
            ...
            for (var i = 0; i < selectData.length; i++) {
                ...
                paramJson.push(selectData[i].xmid);
            }
            console.log(paramJson);
            if (layEvent === "copy") {
                addModel();
                $.ajax({
                    url: getContextPath() + '/rest/v1.0/blxx/plbl?yxmid=' + yxmid + '&xmidList=' + paramJson,
                    type: 'GET',
