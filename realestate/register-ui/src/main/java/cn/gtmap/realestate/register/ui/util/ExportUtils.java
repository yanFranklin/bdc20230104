...
public class ExportUtils {
    ...
    private static final String[] CONVERT_LIST = new String[]{"dzwyt", "qlxz"};
     ...
     * @description 导出文件
     */
    public static void exportExcel(String fileName, LinkedHashMap<String, String> exportCol, List<Map> exportData, HttpServletResponse response, Map<String, List<Map>> zdListMap) throws WriteException, IOException {
        // 取得输出流
        ...
        for (Map<String, Object> dataMap : exportData) {
            ...
            while (iter.hasNext()) {
                ...
                if (dataMap.get(value) != null && ArrayUtils.contains(CONVERT_LIST, value)) {
                    sheet.addCell(new Label(colnum, rownum, getMcByDm(zdListMap, value, String.valueOf(dataMap.get(value))), wcfLeft));
                } else if (dataMap.get(value) != null && StringUtils.equals("zdzhyt", value.toString())) {
                    sheet.addCell(new Label(colnum, rownum, getMcByDm(zdListMap, "tdyt", String.valueOf(dataMap.get(value))), wcfLeft));
                } else if (dataMap.get(value) != null && dataMap.get(value) instanceof Date) {
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                    sheet.addCell(new Label(colnum, rownum, sdf.format(dataMap.get(value)), wcfLeft));
                } else if (dataMap.get(value) != null) {
                    sheet.addCell(new Label(colnum, rownum, String.valueOf(dataMap.get(value)), wcfLeft));
                } else {
