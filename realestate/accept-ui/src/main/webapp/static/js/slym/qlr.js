                    ...
                    success: function (data) {
                        if (data !== null) {
                            ...
                            if(data.jgid==null){
                                ...
                                if(!isNullOrEmpty(data.qlrlx)){
                                    $("#qlrlx").val(data.qlrlx);
                                    $("#qlrlx").next().find('dd[lay-value='+data.qlrlx+']').click();
                                }
                            }
...
function windowParentLoad() {
    ...
    //如果是抵押权或者组合流程刷新第三权利人
    if ((lclx === "zhlc" || lclx === "plzh" || dydj === "true") && (typeof (eval(window.parent.reloadDsqlr)) == "function")) {
        window.parent.reloadDsqlr();
    }
    if (lclx === "pllc" && typeof (eval(window.parent.loadLzrxx)) === "function") {
        window.parent.loadLzrxx(xmfb);
    }
    if (lclx === "zhlc" && typeof (eval(window.parent.loadLzrxx)) === "function") {
        var liIndex = window.parent.$(".qlxx.layui-this").find('input[name="liIndex"]').val()
        var xmid = $(window.parent.$(".layui-this").find("input[name='xmid']")[0]).val();
        window.parent.loadLzrxx("lzrxx" + liIndex, liIndex, xmid);
    }
    if (lclx === "plzh" && typeof (eval(window.parent.loadLzr)) === "function") {
