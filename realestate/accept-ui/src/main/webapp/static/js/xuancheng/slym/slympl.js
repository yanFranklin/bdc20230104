...
function getQlCols() {
    ...
    if (qllx === 4 || qllx === 6 || qllx === 8) {
        ...
        cols = [
            ...
            {field: 'bdcdyh', title: '不动产单元号', minWidth: 270, templet: '#bdcdyhTpl'},
            {field: 'ysfwbm', title: '预售房屋编码', minWidth: 140,hide: islq},
            {field: 'zl', title: '坐落', minWidth: 240},
            {field: 'dzwmj', title: '定着物面积', minWidth: 240,hide: islq},
            {field: 'bdcdywybh', title: '不动产唯一编码', minWidth: 140},
            {field: 'qllx', title: '权利类型', minWidth: 100, templet: '#qllxTpl'},
            {
                field: 'qlxz', title: '权利性质', minWidth: 100,
            ...
            {field: 'zh', title: '幢号', minWidth: 100,hide: islq},
            {field: 'fj', title: '附记', minWidth: 250, event: 'editFj', style:'cursor: pointer;'},
            {field: 'cz', title: '操作', minWidth: 220, templet: '#bdcdyczTpl', fixed: 'right'}
        ];
        ...
        cols = [
            ...
            {field: 'zdzhyt', title: '宗地宗海用途', minWidth: 120, templet: '#zdzhytTpl'},
            {field: 'zh', title: '幢号', minWidth: 100,hide: true},
            {field: 'cz', title: '操作', minWidth: 220, templet: '#bdcdyczTpl', fixed: 'right'}
...
function loadButtonArea(ymlx) {
    ...
    var json = {
        lclx: printDTO[0].lclx, qlxx: printDTO, printBtn: slymPrintBtn, fkdy: zhlcfkdy,
...
* 加载领证人信息
*/
function generateLzrxx(lzrdata, xmfb) {
    var qllx = $($("input[name='qllx']")[0]).val();
    // 传了副本才渲染领证方式
    if (xmfb) {
        lzfs[xmid] = xmfb.lzfs;
...
function loadYjxx(){
    ...
    console.log(djxl);
    var slbh = $('#sjbh').val();
    var url = getContextPath() + "/xuancheng/slym/yjxx.html?processInsId=" + processInsId + "&djxl=" + djxl + "&slbh=" + slbh + "&xmid=" +xmid +"&lclx=pl";
    layer.open({
        type: 2,
        area: ['960px', '300px'],
        fixed: false, //不固定
