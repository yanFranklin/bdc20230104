...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
    ...
    var isSubmit = true;
    var result = verifyformsfxx(form);
...
function loadSfxmPz() {
    var url = getContextPath() + "/sf/xm/pz/HA_0000000";
    if (sfymlx === "ycsl") {
        ...
        success: function (data) {
            if (data !== null) {
                ...
                if (data.bdcSlQlrSfxxDTO == null && data.bdcSlYwrSfxxDTO == null) {
                    ...
                    if (data.bdcSlYwrSfxxDTO != null && !isNotBlank(data.bdcSlYwrSfxxDTO.qlrlb)) {
...
function generateSfxx(bdcSlSfxxDTO, sqrlb,sfcxjs) {
    if (bdcSlSfxxDTO !== null) {
        //默认缴费状态为未缴费
        if (bdcSlSfxxDTO.sfzt === null) {
            bdcSlSfxxDTO.sfzt = 1;
        }
        //默认缴库状态为否
        if (bdcSlSfxxDTO.yhjkrkzt === null) {
                ...
                if (sfxmPz.bdcSlSfxmSfbzList !== null && sfxmPz.bdcSlSfxmSfbzList.length > 0) {
                    for (var j = 0; j < sfxmPz.bdcSlSfxmSfbzList.length; j++) {
                        ...
                        if (bdcSlSfxmSfbz.sfxmbz === sfxmbz) {
                            ...
                            if (isNotBlank(zdzhmj) && sfxmdm == tdjyfwf){
                                var ysjeEl = $(elem).parent().parent().parent().find("input[name='ysje']")[0];
                                $(ysjeEl).val(parseFloat(calculateFloat((bdcSlSfxmSfbz.sfxmdj) * zdzhmj)));
                                $(slEl).val(zdzhmj);
                                countMoney(elem);
                                continue;
                            }
...
function changeSl(elem) {
    ...
    var sfxmdmEl = $(elem).parent().parent().parent().find("select[name='sfxmdm']")[0];
    var sfxmdm = $(sfxmdmEl).val();
    if (sfxmdm == tdjyfwf && isNotBlank(sl)){
        ...
        $(ysjeEl).val(parseFloat(calculateFloat(sfxmdj * sl)));
        countMoney(elem);
        return;
    }else {
        ityzl_SHOW_INFO_LAYER("宗地宗海面积为空，无法生成土地交易服务费");
    }
    if (sl === null || sl === "") {
...
function saveSfxx(elem) {
    ...
    if (qlrsfxx.length > 0 || ywrsfxx.length > 0) {
        ...
        bdcsfxxForm.each(function () {
            ...
            // 如果合计大于0且收费状态为不收费时，修改收费状态为未缴费
            var sfzt = $(this).find("#sfzt").val();
            if (bdcsfxx.hj > 0 && (sfzt == '0' || sfzt == '1')) {
                bdcsfxx.sfzt = 1;
            }
 ...
 */
function fskpFun() {
    getReturnData("/sf/fskp", JSON.stringify({"gzlslid": processInsId}), "POST", function (data) {
...
// 生成二维码
function generateEwm(sfxxid, jfsewmurl, qlrlb) {
    if (isNotBlank(jfsewmurl)) {
        ityzl_SHOW_WARN_LAYER("已生成收费二维码");
        return false;
    }
    var jfrxm = $("#jfrxm" + qlrlb).val();
    var jfrdh = $("#jfrdh" + qlrlb).val();
    addModel();
    getReturnData("/sf/jfbl", JSON.stringify({"sfxxid": sfxxid, "jfrxm": jfrxm, "jfrdh": jfrdh}), "POST", function (data) {
        removeModal();
        querySfxx();
    }, function (xhr) {
        delAjaxErrorMsg(xhr);
...
// 查看缴款通知书
function queryJktzs(qlrlb, jfsewmurl) {
    if (!isNotBlank(jfsewmurl)) {
        ityzl_SHOW_WARN_LAYER("未生成收费二维码，请先生成收费二维码！");
        return false;
    }
    addModel();
    printSfd("jktzs", qlrlb);
    removeModal();
}

// 查询缴费情况
function queryJfqk(sfxxid) {
    addModel();
    getReturnData("/sf/cxjfqk", JSON.stringify({"sfxxid": sfxxid}), "POST", function (data) {
        removeModal();
        querySfxx();
    }, function (xhr) {
        delAjaxErrorMsg(xhr);
...
// 作废缴款通知书
function zfjktzs(sfxxid) {
    getReturnData("/sf/zfjktzs", JSON.stringify({"sfxxid": sfxxid}), "POST", function (data) {
        if (data && data.code != null) {
            if (data.code === "0") {
                ityzl_SHOW_SUCCESS_LAYER(data.msg);
                querySfxx();
...
function verifyformsfxx(form){
    ...
    form.verify({
        ...
        jfrsjh: function (value, item) {
            var isMob = /^0?1[3|4|5|6|7|8|9][0-9]\d{8}$/;//手机号码
            var id = item.parentElement.parentElement.parentElement.parentElement.parentElement.id;
            if (!(($("#qlrsfxx").css("display") === 'none' && id === 'qlrsfxx') || ($("#ywrsfxx").css("display") === 'none' && id === 'ywrsfxx'))) {
                if (!isNotBlank(value)) {
                    $(item).addClass('layui-form-danger');
                    isSubmit = false;
                    verifyMsg = "缴费人联系电话不能为空！";
                    result["isSubmit"] = isSubmit;
                    result["verifyMsg"] = verifyMsg;
                } else if (!isMob.test(value)) {
                    $(item).addClass('layui-form-danger');
                    isSubmit = false;
                    verifyMsg = "缴费人联系电话格式不正确，应为11位手机号码！";
                    result["isSubmit"] = isSubmit;
                    result["verifyMsg"] = verifyMsg;
                }
            }
