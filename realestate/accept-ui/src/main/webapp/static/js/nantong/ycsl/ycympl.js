...
function saveYcslXx() {
    ...
    if(tabid ==="swxx"){
        //交易税收tab
        saveBdcSlJyxxPl("jyssTab");
        saveBdcSlFwxxPl("jyssTab");
        saveZrzcfxx();//保存申请人
    }else{
        //受理信息tab
        saveJbxx();
        saveSjcl();
        saveBdcSlJyxxPl("slTab");
        saveBdcSlXm(".slxm");
        saveBdcSlFwxxPl("slTab");
        // saveBdcSlDyaq(".dyaq");
        saveSqr();//保存申请人
        saveBdcSlLzrxx(".lzxx");
    }
...
function generateYfwfzmPdf($qlrbasic){
    ...
    if(isNotBlank(bdcSlSqrListCache) && bdcSlSqrListCache.length > 0){
        $.each(bdcSlSqrListCache, function(index, value){
            if(value.bdcSlSqrDO.zjh == qlrzjh){
                $.each(value.bdcSlJtcyDOList, function(i, jtcyxx){
                    qlrxx.push({
                        qlrmc: jtcyxx.jtcymc,
                        qlrzjh: jtcyxx.zjh
                    });
...
//推送计税信息
function tsjsxx(xmid){
    addModel();
    $.ajax({
        url: getContextPath() + "/slym/sw/tsJsxx/"+processInsId+"/nt_tsswxx",
        type: "GET",
        data: {xmid: xmid},
        dataType: "json",
...
function refreshJyxxAndFwxx(){
    $.ajax({
        ...
        success: function (data) {
            ...
            generateFwxx(data);
            //加载发票信息模块
            generateFpxx(data[0].bdcZrfSwxxList, data[0].bdcSlJyxxDO);
...
function dealSpfJyxx(data, cxmid) {
    console.log("关联信息："+data);
    if(!isNotBlank(cxmid)){
        cxmid = xmid;
    }
    getReturnData("/ycsl/jyxx/dealFsssBaxx?xmid=" + cxmid, JSON.stringify(data), "POST", function () {
        removeModal();
    }, function (error) {
        console.info(error);
        removeModal();
...
//处理生成交易信息
function dealSlJyxx(data, cxmid) {
    if(!isNotBlank(cxmid)){
        cxmid = xmid;
    }
    getReturnData("/ycsl/jyxx/dealSpfClfJyxxPl?xmid=" + cxmid, JSON.stringify(data), "POST", function () {
        removeModal();
    }, function (error) {
        console.info(error);
        removeModal();
...
//验证比对信息
function checkBdxx(xmid, callback) {
    var spflc=false;
    if(fwfclx !=="clf"){
        spflc =true;
    }
...
// 删除交易房屋信息
function removeFwJyxx(xmid){
    addModel();
    $.ajax({
        url: getContextPath()+"/ycsl/jyxx/fwjyxx?xmid=" + xmid,
        type: 'DELETE',
        dataType: 'json',
        success: function (data) {
            removeModal();
...
// payTaxByXmid 交税
function payTaxByXmid(xmid){
    if(fwfclx ==="clf"){
        ityzl_SHOW_INFO_LAYER("财政接口暂未提供");
    }else {
        checkBdxx(xmid, function (data) {
            if(data){
                payTax(xmid);
            }else{
