...
function generateQlr(sqr) {
    layui.use(['form', 'jquery', 'laytpl', 'element', 'laydate', 'formSelects'], function () {
        ...
        if (isAdd && isNotBlank(sqr.sqrmc)) {
            yharr.push({"name": sqr.sqrmc, "value": sqr.sqrmc, "selected": "selected"});
...
function successCz(fun) {
    layer.msg('<img src="../../static/lib/bdcui/images/success-small.png" alt="">保存成功', {
        ...
        //cancel();
        var $thisTab = $(window.parent.document).find('.layui-show');
        parent.loadYcslxx(xmid,$thisTab);
        closeWin();
    });
    removeModal();
}
