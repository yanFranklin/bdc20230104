function initBahtTable() {
    layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
        ...
        var unitTableTitle = [{type: 'checkbox', fixed: 'left'},
            {field: 'htbh', title: '合同编号', hide: 'true'},
            {
                field: 'bdcdyh', title: '不动产单元号', width: 262, templet: '#hthbdcdyhTpl'
            },
            {field: 'bdcqzh', title: '不动产权证号', width: 225},
            {field: 'qllx', title: '权利类型', hide: 'true'},
            {field: 'qllxMc', title: '权利类型名称', hide: 'true'},
            {field: 'zl', title: '坐落', minWidth: 220,sort: true},
            {field: 'qlrmc', title: '权利人',minWidth: 120},
            {field: 'qlrzjh', title: '权利人证件号',minWidth: 140},
            {
        ...
        //提交表单
        form.on("submit(queryBaht)", function (data) {
            // 备案合同查询之前 要先通过参数查询备案合同的接口获取产证号 才能查登记库

            if(data.field.qlrmc == "" && data.field.qlrzjh == "" && data.field.htbh == ""){
                ityzl_SHOW_WARN_LAYER("请输入查询条件！");
                return false;
            }
            var bahtCxParam = queryBaht(data.field);
            if(!bahtCxParam.bdcqzh){
                ityzl_SHOW_WARN_LAYER("未查询到合同备案信息！");
                return false;
            }
            addModel();
            //查询时清除缓存数据
            ...
            tableReload('xmid_hth', cxObj, url);
            return false;
        });
        ...
        // 监听复选框事件，缓存选中的行数据
        table.on('checkbox(xmList_hth)', function (obj) {
            addModel();
            ...
            if (obj.type == 'one') {
                ...
                checkHth(checkData, false);
            }
        });


        //监听排序事件
        setColorAfterZl('xmList_hth');

        //监听单元号点击
        $('.bdc-tab-box').on('click','.bdc-hthbdcdyh-click',function () {
            getSelectHthData($(this).data('json'),true);
        });

        //加载表格
        loadDataTablbeByUrl('#xmList_hth', tableConfig);
        //表格初始化
        table.init('xmList_hth', tableConfig);
        //头工具栏事件
        table.on('toolbar(xmList_hth)', function (obj) {
        ...
        table.on('tool(xmList_hth)', function (obj) { //bdcdyhList为table的lay-filter对应的值
...
function checkHth(checkData, sfcj) {
    var selectDataList = [];
    for (var i = 0; i < checkData.length; i++) {
        ...
        bdcGzYzsjDTO.gzldyid = processDefKey;
        var cxlx = "";
        $("input:radio[name='cxlx']:checked").each(function () {
            cxlx = $(this).val();
        });
        bdcGzYzsjDTO.cxlx = cxlx;
        var obj = {};
...
getSelectHthData = function (elem, cj) {
    addModel();
    checkData = [];
    checkData.push(elem);
    sfcj = cj;
 ...
 * @param data
 */
function queryBaht(data){
    var result = {};
    data.gzldyid = processDefKey;
    $.ajax({
        url: getContextPath() + "/bdcdyh/queryBaht/param",
        type: 'POST',
        contentType: "application/json;charset=UTF-8",
        async: false,
        dataType : "json",
