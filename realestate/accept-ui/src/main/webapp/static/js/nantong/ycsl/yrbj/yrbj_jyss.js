...
//加载房屋信息模块
function generateFwxx(fwxx, jyxx) {
    ...
    if (swxxList != null && swxxList.length > 0) {
        ...
        if (swxx) {
            for (var i = 0; i < swxx.length; i++) {
                jexj.ynsexj += swxx[i].ysx;
                jexj.jmsehj += swxx[i].jcx;
                jexj.sjnsehj += swxx[i].ynse;
                jexj.ybtsehj += swxx[i].ybtse;
            }
        }
        //一人办件处理完税状态字段
        swxxDTO.xxzt = changeDmToMcFilter(swxxDTO.bdcSlHsxxDO.wszt, zdList.swsbzt, "1");
        swxxDTO.xxztdm = swxxDTO.bdcSlHsxxDO.wszt;
        swxxDTO.sbzt = changeDmToMcFilter(swxxDTO.bdcSlHsxxDO.wszt, zdList.swsbzt, "2");
        swxxDTO.sbztdm = swxxDTO.bdcSlHsxxDO.wszt;
        swxxDTO.jszt = changeDmToMcFilter(swxxDTO.bdcSlHsxxDO.wszt, zdList.swsbzt, "3");
        swxxDTO.jsztdm = swxxDTO.bdcSlHsxxDO.wszt;
    }
...
function openJtcyYm(sqrid, sqrlb) {
    var url = getContextPath() + "/view/ycsl/yrbjtcy.html?sqrid=" + sqrid + "&formStateId=" + formStateId + "&sqrlb=" + sqrlb;
    layer.open({
        title: '配偶子女信息',
        type: 2,
        area: ['960px', '400px'],
...
function jddmDz(xzqh, jddm) {
    ...
    if (isNotBlank(jddmList)) {
        $('#fwjyxx').find('#jyss-jddm option').remove();
        var $jddm = $('#fwjyxx').find('#jyss-jddm');
...
//电子签字
function getQzxx(qzrlb) {
    layer.open({
        title: '合同签字',
        type: 2,
        area: ['700px', '430px'],
        content: "/realestate-accept-ui/view/ycsl/yrbjqzb.html?xmid=" + xmid + "&gzlslid=" + processInsId + "&qzrlb=" + qzrlb,
        cancel: function () {
        },
        success: function () {
        }
...
* 字典值过滤后转换字典项名称
* */
function changeDmToMcFilter(dm, zdList, filter) {
    var mc = "";
    if (isNotBlank(zdList)) {
        for (var i = 0; i < zdList.length; i++) {
            var zd = zdList[i];
            if (dm == zd.DM && filter === zd.LX) {
                mc = zd.MC;
                break;
            }
        }
