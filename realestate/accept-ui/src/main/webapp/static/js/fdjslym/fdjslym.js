...
layui.use(['form', 'jquery', 'laydate', 'element', 'layer', 'table', 'laytpl'], function () {
    ...
    $(function () {
        ...
        form.on('submit(saveFdjData)', function (data) {
            saveFdjData();
            return false;
        });
...
function loadFdjData() {
    ...
    $.when(// 获取资金监管信息接口数据
        ...
        .done(function(){
            // 渲染页面
            var tpl = fdjslymTpl.innerHTML, view = document.getElementById('fdjslymView');
            laytpl(tpl).render(json, function (html) {
                view.innerHTML = html;
            });
            form.render();
            renderDate(form, "");
            getStateById(readOnly, formStateId, "fdjslym", "", "");
            disabledAddFa();
            renderSelect();
