...
function initBdcdyhTable() {
    layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
        ...
        var unitTableTitle = [
            ...
            {
                field: 'lx', title: '类型', width: "10%", fixed: "right", templet: function (d) {
                    return getHsLx(d.lx);
                }
            },
            {
        ...
        form.on("submit(queryBdcdyh)", function (data) {
            addModel();
            var dyhzl = $("#bdcdyzl");
            var url = getContextPath() + '/bdcdyh/ls/listBdcdyhByPageJson?loadpage=true';
            var cxObj = data.field;
            cxObj.qjgldm = qjgldm;
            cxObj.dzwtzm = xnbdcdyh.substring(19, 20);
            cxObj.bdcdyfwlx = "";
            cxObj.qlr = cxObj.qlrmc
            cxObj.qlrmh = 4
...
function matchDyh(bdcdyh,gxbdcdyfwlx) {
    ...
    $.ajax({
        url: getContextPath() + "/matchData/matchLsDyh?lsdyh=" + xnbdcdyh + "&bdcdyh=" + bdcdyh +"&qjgldm="+qjgldm+"&gxbdcdyfwlx="+gxbdcdyfwlx,
        type: 'POST',
        async: false,
...
function dismatchDyh(bdcdyh) {
    ...
    $.ajax({
        url: getContextPath() + "/matchData/dismatchLsDyh?bdcdyh=" + bdcdyh + "&xnxmid=" + xnxmid+"&qjgldm="+qjgldm,
        type: 'POST',
        async: false,
        contentType: "application/json",
        success: function (data) {
...
function getHsLx(lx) {
    var hslx = "";
    if (lx === "HS") {
        hslx = '<span class="bdc-sc">实测</span>';
    } else if (lx === "YCHS") {
        hslx = '<span class="bdc-yc">预测</span>';
    } else if (lx === "LJZ") {
        hslx = '<span class="bdc-ljz">逻辑幢</span>';
    } else if (lx === "XMXX") {
        hslx = '<span class="bdc-xmxx">项目内多幢</span>';
    }
    return hslx;
}
