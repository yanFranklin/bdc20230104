...
layui.use(['jquery', 'form', 'laytpl','element','table'], function () {
    ...
    $(function () {
        ...
        function generateMatchData(getMatchData){
            //小写
            var getMatchDataLowerCase = [];
            for (var i=0;i< getMatchData.length;i++){
                var ele = getMatchData[i];
                var getMatchDataItemLowerCase= {};
                for(var k in ele){
                    getMatchDataItemLowerCase[k.toLowerCase()] = ele[k];
                }
                getMatchDataLowerCase.push(getMatchDataItemLowerCase)
            }
            var json={
                data:getMatchDataLowerCase,
                xztzlx:xztzlx,
                jbxxid:jbxxid,
                ppzt:ppzt
            };
            ...
            if(firstLi.data('src1') != undefined && dzwtzm === "F"){
        ...
        $(document).on('click', '.bdc-outer-li a', function () {
            ...
            $('.bdc-tab-content .layui-tab-item:first-child iframe').attr('src',encodeURI($(this).data('src')));
             ...
             //左侧为非土地时，展示土地证列
            if($(this).data('src1') != undefined && dzwtzm !== "W"){
                $('.bdc-tab-content .layui-tab-item:last-child').removeClass('bdc-hide');
