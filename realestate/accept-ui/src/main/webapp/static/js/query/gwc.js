...
layui.use(['element', 'form', 'jquery', 'laydate', 'laypage', 'laytpl', 'layer'], function () {
    ...
    $(document).on("click", ".main-content-item >.main-content-item-content>div", function (e) {
        ...
        editbdcdyh = $(this).parents(".main-content-item").find(".bdcdyh").val();
        ...
        if (data.value == 3) {
            ...
            if (isfirst == true) {
                ...
                $(".main-content-container-right-item").droppable({
                    accept: '.todrag,.layui-colla-title',
...
function loadSelectBdcdyh(page, size, queryData) {
    $.ajax({
        ...
        success: function (data) {
            ...
            //计算所有的面积
            getReturnData("/gwc/jzmj", {jbxxid: jbxxid, djxl: djxl}, "GET", function (data) {
                if (data) {
                    $("#zjzmj").show();
                    $("#sumjzmj").text(data + "m²");
                } else {
...
function queryDzwyt(yt) {
    for (var i = 0; i < zdList.fwyt.length; i++) {
        if (yt == zdList.fwyt[i].DM) {
            yt = zdList.fwyt[i].MC;
        }
    }
    return yt;
