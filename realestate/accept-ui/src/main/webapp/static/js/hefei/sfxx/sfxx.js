...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
    ...
    form.on("submit(saveData)", function (data) {
        if (!isSubmit) {
            ...
            setTimeout(function () {
                try {
                    $.when(saveSfxx(), saveSfxm(), insertXgLog()).done(function () {
                        removeModal();
    ...
    // 是否月结监听修改
    form.on('select(sfyj)', function (data) {
        console.info(data.othis);
        var $select = $(data.othis).find("input");
        if ($select.length > 0) {
...
function generateSfxx(bdcSlSfxxDTO, sqrlb,sfcxjs) {
    if (bdcSlSfxxDTO !== null) {
        ...
        layui.use(['form', 'jquery', 'laytpl', 'element', 'laydate'], function () {
            ...
            form.render('select');
            disabledAddFa();
            getStateById(readOnly, formStateId, formStateName);
            queryXgLog();
        })
    } else {
        if (sqrlb === "1") {
...
function loadWxjjxx() {
    $.ajax({
        url: getContextPath() + "/wxzj/queryWxjjxx",
        type: 'get',
        dataType: 'json',
        data: {gzlslid: processInsId},
        success: function (data) {
...
// 渲染维修基金信息列表
function generateWxjjxx(bdcSlWxjjxxDOList) {
    layui.use(['form', 'jquery', 'laytpl', 'element', 'laydate'], function () {
        var form = layui.form;
        var laytpl = layui.laytpl;
        var json = {
            bdcSlWxjjxxDOList: bdcSlWxjjxxDOList,
            zd: zdList
        };
        var view = document.getElementById('wxjjxx');
        var tpl = wxjjTpl.innerHTML;
...
// 删除维修基金信息列表
function saveAndDeleteWxjjxx(wxjjxxid) {
    if (isNotBlank(wxjjxxid)) {
        var deleteIndex = layer.open({
            ...
            yes: function () {
                ...
                setTimeout(function () {
                    try {
                        $.when(deletewxjj(wxjjxxid)).done(function () {
                            removeModal();
...
// 删除维修基金信息
function deletewxjj(wxjjxxid) {
    $.ajax({
        url: getContextPath() + "/wxzj/deleteWxjjxxByWxjjxxid?wxjjxxid=" + wxjjxxid,
        async: false,
        type: 'DELETE',
        success: function (data) {
            loadWxjjxx();
...
function changeSl(elem) {
    ...
    if (sl === null || sl === "") {
        ...
        $(ysjeEl).val(ysje);
        $(ssjeEl).val(Subtr(parseFloat(ysje),parseFloat(jmje)));
        countMoney(elem);
    }
}
...
//POS缴费原版本
function posjfBak(jkfs){
    addModel();
    $.ajax({
        url: getContextPath() + "/sf/tsdjfxxByGzlslid",
        type: 'POST',
        dataType: 'json',
        data: {
        ...
        success: function (data) {
            ...
            if (jkfs === "2") {
                if (isNotBlank(data)) {
                    if (data.jfurl) {
                        var url = "/realestate-accept-ui/sf/ewm?ewmnr=" + encodeURIComponent(data.jfurl);
            ...
            if(data && isNotBlank(data.statusCode) && isNotBlank(data.jfsbm)){
                if("0000" == data.statusCode){
                    var url = "/realestate-accept-ui/view/slym/posjf_bak.html?jfsbm="+data.jfsbm+"&hsje="+data.hsje+"&processInsId="+processInsId;
                    var index = layer.open({
                        type: 2,
                        area: ['1150px', '400px'],
                        fixed: false, //不固定
                    ...
                    ityzl_SHOW_WARN_LAYER("操作失败" + data.msg);
                }
            }else{
                ityzl_SHOW_WARN_LAYER("操作失败，未获取到订单编号");
            }
        },
        error: function (err) {
            delAjaxErrorMsg(err);
    ...
    gzyzCommon(2, bdcGzYzQO, function (data) {
        ...
        var index = layer.open({
            ...
            yes: function () {
                addModel();
                $.ajax({
                    url: getContextPath() + "/sfss/ddxx/zfddxx",
                    type: 'get',
                    dataType: 'json',
                    data: {gzlslid: processInsId},
                    success: function (data) {
...
function insertXgLog(){
    if($(".bdc-change-input").length > 0){
        ...
        $(".bdc-change-input").each(function (i) {
            var qlrlb = $(this).parents(".layui-form-item").find("input[name='qlrlb']").val();
            xgxxList.push({
        ...
        var id = "sfxx-"+ processInsId;
        if(isNotBlank(xmid)){
            id = xmid;
        }
        getReturnData("/rest/v1.0/slym/addXgLog?gzlslid=" + id, JSON.stringify(xgxxList), "POST", function () {
        }, function (error) {
            delAjaxErrorMsg(error);
        });
    }
...
function queryXgLog() {
    ...
    getReturnData("/rest/v1.0/slym/queryXgLog", {gzlslid: id}, "GET", function (data) {
        if (data && data.value) {
            var modifyData = JSON.parse(data.value);
            for (var i = 0; i < modifyData.length; i++) {
                var $select;
                var name = modifyData[i].name;
                if(modifyData[i].type == "1"){
                    $select = $("#qlrsfxx").find('select[name="' + name + '"]');
                } else {
                    $select = $("#ywrsfxx").find('select[name="' + name + '"]');
                }
                $select.parents('.layui-input-inline').addClass('bdc-change-input');
                $select.css({"background-color": "#eaf4fe"})
            }
        }
    }, function (error) {
    })
...
//缴费推送--生成二维码
function jfts(sfxxid, jfsewmurl) {
...
// 缴费查询
function jfcx(sfxxid) {
    if (isNotBlank(sfxxid)) {
        addModel();
        getReturnData("/sf/jfcx/sc?sfxxid=" + sfxxid, {}, "GET", function (data) {
            removeModal();
            querySfxx();
        }, function (xhr) {
            delAjaxErrorMsg(xhr);
...
// 税费托管推送收费缴库
function sftgTsjkFun() {
    addModel();
    $.ajax({
        url: getContextPath() + "/slym/sw/sftg/tsjk",
        type: 'get',
        dataType: 'json',
        data: {gzlslid: processInsId},
        success: function (res) {
            ...
            if (res && res.success) {
                ...
                ityzl_SHOW_WARN_LAYER("推送失败" + res.fail.message);
            }
        },
        error: function (xhr, status, error) {
            removeModal();
...
function zhjqFun(){
    addModel();
    $.ajax({
        url: getContextPath() + "/slym/sw/sftg/zhjq",
        type: 'get',
        dataType: 'json',
        data: {gzlslid: processInsId},
        success: function (res) {
            ...
            if (res && res.success) {
                layer.open({
                    ...
                    area: ['430px','250px'],
                    btn: ['确定', '取消'],
                    content: $('#bdc-popup-small-zhjq'),
                    yes: function (index, layero) {
                        layer.close(index);
                    }, btn2: function (index, layero) {
                        //取消 的回调
                    }, success: function () {
                        $("#tkje").val(res.data.tkje);
                        $("#lxje").val(res.data.lxje);
                        $("#tkzje").val(res.data.tkzje);
                    }
                });
            } else {
                ityzl_SHOW_SUCCESS_LAYER("账户结清失败" + res.fail.message);
            }
        },
        error: function (xhr, status, error) {
            removeModal();
