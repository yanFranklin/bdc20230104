layui.use(['jquery', 'table', 'element', 'form', 'layer'], function () {
    ...
    $(function () {
        ...
        $('.bdc-car').on('click',function (event) {
            ...
            $('.bdc-gwc').slideUp(300);
            var url = getContextPath() + '/hefei/query/gwc.html?jbxxid=' + jbxxid + '&gzldyid=' + processDefKey+"&zlcsh="+zlcsh+"&slbh="+slbh;
            layer.open({
                type: 2,
                title: "已选信息",
                skin: 'bdc-gwc-layer',
        ...
        if(zlcsh ==="true"){
            $("#csh").hide();
            $("#zlcsh").removeAttr("disabled").css("pointer-events", "auto");
        }else{
            $("#zlcsh").hide();
            $("#csh").removeAttr("disabled").css("pointer-events", "auto");
        }
        //单击购物车表格中的去创建
