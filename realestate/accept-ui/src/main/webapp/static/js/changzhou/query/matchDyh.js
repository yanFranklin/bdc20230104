...
var xnbdcdyh = getQueryString("xnbdcdyh");
var xnxmid = getQueryString("xnxmid");
var xztzlx = getQueryString("xztzlx");
var bdcdyhzl = getQueryString("zl");
var bdcdywybh = getQueryString("bdcdywybh");
...
function loadData(xnxmid) {
    ...
    initBdcdyhTable();
    renderSearchInput();
    if(xnbdcdyh && xnbdcdyh.length==28 && xnbdcdyh.slice(6,12)!='000000'){
        $("#queryBdcdyh").click();
    }
}
...
function initBdcdyhTable() {
    layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
        ...
        var unitTableTitle = [
            ...
            {field: 'zt', title: '状态', width: "10%", templet: '#bdcdyzt', align: "center"},
            {field: 'jzmj', title: '建筑面积', width: "10%"},
            {
        ...
        table.on('toolbar(bdcdyhList)', function (obj) {
            ...
            if (layEvent === "qxlz") {
                //取消落宗
                if(isNotBlank(ppbdcdyh)) {
                    checkMatchData(ppbdcdyh, "", "YZQXPPDYH");
                }else{
                    ityzl_SHOW_WARN_LAYER("没有找到匹配记录");
                }
...
function matchDyh(bdcdyh,gxbdcdyfwlx) {
    if(!isNotBlank(gxbdcdyfwlx)){
        gxbdcdyfwlx =false;
    }
    $.ajax({
        url: getContextPath() + "/matchData/matchDyh?lsdyh=" + xnbdcdyh + "&bdcdyh=" + bdcdyh +"&qjgldm="+qjgldm+"&gxbdcdyfwlx="+gxbdcdyfwlx,
        type: 'POST',
        async: false,
...
function dismatchDyh(bdcdyh) {
    $.ajax({
        url: getContextPath() + "/matchData/dismatchDyh?bdcdyh=" + bdcdyh + "&xnxmid=" + xnxmid+"&qjgldm="+qjgldm,
        type: 'POST',
        async: false,
        contentType: "application/json",
        success: function (data) {
