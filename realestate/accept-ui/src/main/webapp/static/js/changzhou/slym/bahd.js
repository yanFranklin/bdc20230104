...
layui.use(['form', 'jquery', 'laydate', 'element', 'layer', 'table', 'laytpl'], function () {
    ...
    $(function () {
        ...
        form.on('submit(saveBahd)', function (data) {
...
function loadBahd() {
    getReturnData("/slym/bahd", {gzlslid: gzlslid}, "GET", function (data) {
        var json = {
            bahdxx: data
        }

        var tpl = bahdTpl.innerHTML, view = document.getElementById('bahdView');
        //渲染数据
        ...
        form.render();
        renderDate(form, "");
        getStateById(readOnly, formStateId, "bahd", "", "");
        disabledAddFa();
        renderSelect();
...
function saveBahd() {
    ...
    var bahdArray = $('.bahd');
    var bahd = {};
    bahdArray.serializeArray().forEach(function (item, index) {
        bahd[item.name] = item.value;
    });
    bahd.gzlslid = gzlslid;
    getReturnData("/slym/bahd", JSON.stringify(bahd), "POST", function (data) {
        loadBahd();
        removeModal();
