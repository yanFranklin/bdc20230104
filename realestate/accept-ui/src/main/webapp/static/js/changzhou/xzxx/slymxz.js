...
var processInsId = isNotBlank(getQueryString("processInsId")) ? getQueryString("processInsId") : "nogzlslid";
var lclx = "zhlc";
...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl', 'laydate'], function () {
    ...
    addModel();
    //加载页面上方按钮模块
    setTimeout("loadButtonArea('slymxz')", 10);
    setTimeout(function () {
        try {
...
function generateJbxx(bdcxmxx, xmfb, refresh) {
    ...
    if (refresh) {
        ...
        renderForm();
        getStateById(readOnly, formStateId, 'slymxz');
        form.render('select');
        disabledAddFa();
        renderSelect();
    }
...
function generateQlxx(bdcSlQlxxymDTOList) {
    if (isNotBlank(bdcSlQlxxymDTOList) && isNotBlank(bdcSlQlxxymDTOList[0].bdcQl)) {
        for (var i = 0; i < bdcSlQlxxymDTOList.length; i++) {
            var bdcSlQlxxym = bdcSlQlxxymDTOList[i];
            ...
            if (bdcSlQlxxym.bdcXm.qllx == "4" || bdcSlQlxxym.bdcXm.qllx == "6" || bdcSlQlxxym.bdcXm.qllx == "8") {
                getReturnData("/slym/sdq/querySdqyw/" + processInsId, {}, 'GET', function (data) {
                    if (data) {
            ...
            if (isNotBlank(bdcSlQlxxym.bdcXm.djxl)) {
                ...
                getReturnData("/slym/xm/queryDjxlDjyy", {
                    djxl: bdcSlQlxxym.bdcXm.djxl,
                    gzldyid: bdcSlQlxxym.bdcXm.gzldyid
                }, 'GET', function (data) {
                    if (isNotBlank(data)) {
                        djxldjyyList = data;
                    }
...
function generateXmZh(bdcslqlxxym, name, index, qlTableId) {
    var data = bdcslqlxxym.bdcXm;
    var xmfb = bdcslqlxxym.bdcXmFbDO;
    var qxList = bdcslqlxxym.regionDtoList;
    ...
    var json = {
        ...
        qlTableId: qlTableId,
        qxdmList: qxList,
        zsbdcqzh: bdcslqlxxym.zsbdcqzh,
        zsid: bdcslqlxxym.zsid,
        xgzszh: bdcslqlxxym.xgzszh,
        zxlc: zxlc //20191025新增用于页面做注销流程判断
    };
    ydjyy = data.djyy;
    ydjyyArr['djyy' + index] = data.djyy;
    sfchange = false;
    ...
    if ($(".jyxx" + index).length > 0 && isNotBlank(data.xmid)) {
...
function loadSjcl() {
    if (isNotBlank(xmlx) && (xmlx === 2 ||xmlx ===4) && zhsjcl) {
...
function generateQlrxx(data, xmid, name, index, isLoad, sfdydj) {
    ...
    if (isLoad == null || !isLoad) {
        ...
        form.render();
        renderForm();
        getStateById(readOnly, formStateId, "slymxz", 'qllx' + (index));
        //重新渲染select框
        form.render('select');
...
function generateSjcl(data, xmid) {
    ...
    form.render();
    renderForm();
    getStateById(readOnly, formStateId, 'slymxz', 'sjcl');
    form.render('select');
...
function addQlr(qllx, dydj) {
    ...
    var url = getContextPath() + "/view/slym/qlr.html?xmid=" + addXmid + "&lclx=" + lclx + "&processInsId=" + processInsId + "&formStateId=" + formStateId + "&qllx=" + qllx + "&zxlc=" + zxlc + "&xzlc=true";
    if (dydj) {
        layer.open({
            type: 2,
            area: ['960px', '610px'],
...
function showQlr(qlrid, xmid, qlrlb, elem) {
    ...
    var title = '';
    var url = getContextPath() + "/changzhou/slym/qlr.html?processInsId=" + processInsId + "&qlrid=" + qlrid + "&lclx=" + lclx + "&xmid=" + xmid + "&formStateId=" + formStateId + "&qllx=" + qllx + "&readOnly=" + readOnly + "&zxlc=" + zxlc + "&djxl=" + djxl + "&xzlc=true";
...
function getLzfs() {
    var lzfs = '';
    getReturnData('/slym/lzr/lzfs', {gzlslid: processInsId}, "GET", function (data) {
        lzfs = data;
    }, function (xhr) {
        delAjaxErrorMsg(xhr);
        return "";
...
function compareHyxx(elem) {
    ...
    getReturnData("/slym/jtcy/compareHyxx", {
        slbh: slbh,
        xmid: xmid,
        sqrid: sqrid,
        qlrmc: qlrmc,
