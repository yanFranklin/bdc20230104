...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
    ...
    $(function () {
        ...
        setTimeout(function () {
            try {
                // $.when(loadButtonArea(),loadZdData(),loadJbxx(),loadSwxx()).done(function () {
...
function hqWspz(){
    addModel();
    getReturnData( "/slym/sw/queryHsxxZt", {gzlslid: processInsId}, 'GET', function (hsxx) {
        removeModal();
        if(isNotBlank(hsxx)){
            if(hsxx.wszt == "10"){
                addModel();
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(isNotBlank(data)){
                            ...
                            ityzl_SHOW_WARN_LAYER("获取契税完税凭证失败");
                        }
                    },
                    error: function (xhr, status, error) {
                        removeModal();
                ...
                var ztxx = fmtHsSbzt(hsxx.wszt);
                ityzl_SHOW_WARN_LAYER("完税状态为：" + ztxx + "，需缴款完成才能获取。" );
            }
        }else{
            ityzl_SHOW_WARN_LAYER("未获取到核税信息");
        }
    }, function (err) {
        removeModal();
        delAjaxErrorMsg(xhr);
        ...
        if(isNotBlank(hsxx)){
            if(hsxx.wszt == "10"){
                ...
                $.ajax({
                    ...
                    type: "GET",
                    data: {gzlslid: processInsId, fwlx: fwlx,  beanName:"sw_qslxdhq", gxlx: "0"},
                    dataType: "json",
                    contentType: "application/json;charset=UTF-8",
                    success: function (data) {
                        removeModal();
                        if(isNotBlank(data)){
                            ityzl_SHOW_SUCCESS_LAYER("获取契税完税联系单成功");
                            setRedisCache("hqqsxxlxd","true");
                        }else{
                            ityzl_SHOW_WARN_LAYER("获取契税完税联系单失败");
                        }
                    },
                    error: function (xhr, status, error) {
                        removeModal();
