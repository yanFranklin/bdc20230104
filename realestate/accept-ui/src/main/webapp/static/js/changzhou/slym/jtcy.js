layui.use(['jquery', 'layer', 'element', 'form', 'laytpl'],function () {
    ...
    if(isNotBlank(xmid)){
        getReturnData("/slym/sqr/sqrxx",{xmid:xmid, sqrlb:sqrlb},"GET",function (data) {
            if(isNotBlank(data)){
                $.each(data, function(index, val){
                    if(val.sqrid != sqrid){
                        $("#jtcymcselect").append('<option value="'+val.sqrmc+'" data-zjzl="'+val.zjzl+'" data-zjh="'+val.zjh+'" >' + val.sqrmc + '</option>');
                    }
                });
                form.render("select");
            }
    ...
    // 渲染字典项
    $.each(parent.zdList.gj, function (index, item) {
        $('#po_gj').append('<option value="' + item.DM + '">' + item.MC + '</option>');
    });
    ...
    form.on('select(jtcymcselect)', function(data){
        var zjzl = $(data.elem).find("option:selected").data("zjzl");
        var zjh = $(data.elem).find("option:selected").data("zjh");
        $(this).parents('tr').find("input[name='jtcymc']").val(data.value);
        $(this).parents('tr').find("#po_zjzl").val(zjzl);
        $(this).parents('tr').find("input[name='zjh']").val(zjh);
        form.render();
    });
