    ...
    layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
        ...
        form.on("submit(saveData)", function (data) {
            if (!isSubmit) {
                ...
                setTimeout(function () {
                    try {
                        $.when(saveXmxx(), saveZsxx(), saveQlxx(), saveSjcl(), saveQlr(), saveDsQlr(), saveZrzcfxx(), updateFwfsss(), insertXgLog(), saveYjxx(), saveZdjbxx()).done(function () {
                            if (lclx === "zhlc") {
                                refreshQlxx();
                                //修正保存后数据处理
                                handleXzlc();
...
function saveZsxx() {
    var zsid = $("#zsid").val();
    var bdcqzh = $("#zsbdcqzh").val();
    if (isNotBlank(zsid) && isNotBlank(bdcqzh)) {
        getReturnData("/xzxx/gxzs", {zsid: zsid, bdcqzh: bdcqzh, xmid: xmid}, "GET", function (data) {
            console.log("更新证书信息成功");
        }, function (xhr) {
            delAjaxErrorMsg(xhr);
        })
    }
    ...
    if (lclx === "zhlc") {
        ...
        if (index !== null && index.length > 0) {
            for (var i = 0; i < index.length; i++) {
                ...
                if($("#sfyt").length > 0 && $("#sfyt").hasClass("xmfb"+c)){
                    // 同步项目附表用途
                    syncXmfbSfyt(".xmfb"+c);
                }
            }
...
// 组合流程同步项目附表收费用途
function syncXmfbSfyt(formClass){
    var bdcXmfbData = {};
    var bdcXmfbArray = $(formClass);
    if (bdcXmfbArray !== null && bdcXmfbArray.length > 0) {
        bdcXmfbArray.serializeArray().forEach(function (item, index) {
        ...
        bdcXmfbData.gzlslid = processInsId;
        if (Object.keys(bdcXmfbData).length > 1 && isNotBlank(bdcXmfbData.xmid)) {
            getReturnData("/slym/xm/fb/syncsfyt", JSON.stringify(bdcXmfbData), 'POST', function (data) {
            }, function (err) {
                delAjaxErrorMsg(err);
            }, false);
        }
...
function handleXzlc() {
    //处理修正流程数据，重新计算收费，更新证书信息
    getReturnData("/xzxx/dataservice", {gzlslid: processInsId, xmid: xmid, xzlcymlx: xzlcymlx}, "GET", function (data) {
        console.log("修正流程数据处理完成");
    }, function (xhr) {
        delAjaxErrorMsg(xhr);
...
function updateZsxx() {
    addModel();
    //处理修正流程数据，重新计算收费，更新证书信息
    getReturnData("/xzxx/dataservice/zs", {gzlslid: processInsId, xmid: xmid}, "GET", function (data) {
        removeModal();
        ityzl_SHOW_SUCCESS_LAYER("证书更新完成");
    }, function (xhr) {
        delAjaxErrorMsg(xhr);
