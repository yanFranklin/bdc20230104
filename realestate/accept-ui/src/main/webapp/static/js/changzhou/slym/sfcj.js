    ...
    getReturnData("/slym/xm", {processInsId: processInsId}, 'GET', function (data) {
        if (isNotBlank(data)) {
            ...
            getReturnData("/slym/xm/fb",{xmid:xmid},"GET",function (result) {
                ...
                //常州获取邮寄信息渲染到页面
                var yjxx = getYjxx();
                var cfbh =getCfbh(data.xmid,data.qllx);
                generateJbxx(data, yjxx,result,cfbh);
...
function showYqlxxBtn(xmid) {
    //先找到当前项目判断是否为注销流程，注销只展示原权利信息按钮
    //根据xmid找到原权利，原权利存在则展现
    if (zxlc != "true") {
        getReturnData("/slym/ql/queryQlxx", {xmid: xmid,sfyql:true}, 'GET', function (data) {
            if (isNotBlank(data)) {
                showYqlBtn = "showAllBtn";
            } else {
                showYqlBtn = "showQlBtn";
            }
        }, function (err) {
            delAjaxErrorMsg(err);
        }, false);
...
//组织基本信息到页面
function generateJbxx(bdcxmxx, yjxx,xmfb,cfbh) {
    var json = {
        bdcxmxx: bdcxmxx,
        zd: zdList,
        yjxx: yjxx,
...
function buildQlxx(bdcSlQlxxymDTOList) {
    var qlxxList = [];
    qlxxList.push(bdcSlQlxxymDTOList[0]);
    var json = {
        bdcSlQlxxymDTOList: qlxxList,
...
function generateQlxx(bdcSlQlxxymDTOList) {
    if (isNotBlank(bdcSlQlxxymDTOList)) {
        for (var i = 0; i < bdcSlQlxxymDTOList.length; i++) {
            ...
            var xtjgpzList = [];
            //交易价格值乘10000，以元为单位显示
            var jyjg = bdcSlQlxxym.bdcQl.jyjg;
            if (isNotBlank(jyjg)) {
                jyjg = calculateFloat(jyjg * 10000);
            }
            showYqlxxBtn(bdcSlQlxxym.bdcXm.xmid);
            ...
            // }
            bdcSlQlxxym.tableName = 'bdc_sfcj';
            ...
            if (bdcSlQlxxym.cqXmList && bdcSlQlxxym.cqXmList.length > 0) {
                for (i = 0; i < bdcSlQlxxym.cqXmList.length; i++) {
            ...
            if (isNotBlank(bdcSlQlxxym.bdcXm.jyhth)) {
                //加载合同信息模块
                if (bdcSlQlxxym.tableName === 'bdc_sfcj') {
                    generateHtxxZhSfcj(bdcSlQlxxym.bdcXm, "bdchtxx", qlTableId);
                } else {
            ...
            if (bdcSlQlxxym.tableName === 'bdc_sfcj') {
                loadQlrZhSfcj(bdcSlQlxxym.bdcXm.xmid, "sqr", true);
            } else {
                loadQlrZh(bdcSlQlxxym.bdcXm.xmid, "sqr" + (i + 1), i + 1, true);
            }
            if (bdcSlQlxxym.tableName === 'bdc_sfcj') {
                bdcSlQlxxym.qlmc = '司法裁决';
                generateXmZhSfcj(bdcSlQlxxym.bdcXm, "xmzh", qlTableId, bdcSlQlxxym.bdcXmFbDO);
            }
            loadZdjbxx(bdcSlQlxxym.bdcXm.bdcdyh);
            //加载第三权利人
            if (bdcSlQlxxym.tableName === 'bdc_sfcj') {
                generateDsQlrSfcj(bdcSlQlxxym.bdcXm.xmid, $("#" + bdcSlQlxxym.tableName), bdcSlQlxxym.bdcDsQlrDOList);
            } else {
        ...
        for (var j = 0; j < bdcSlQlxxymDTOList.length; j++) {
            ...
            if (bdcSlQlxxym.tableName === 'bdc_sfcj') {
                syncBdcdyxxSfcj(bdcSlQlxxym, bdcSlQlxxymDTOList);
            } else {
...
function loadQlrZh(xmid, name, index,isLoad) {
    getReturnData("/slym/qlr/list/xm", {xmid: xmid, lclx: "zhlc"}, 'GET', function (data) {
        generateQlrxx(filterQlrZh(data), xmid, name, index,isLoad);
    }, function (err) {
        console.log(err);
    },false);
}
...
//加载权利人（入口）司法裁决
//isLoad:是否为页面加载时,默认false
function loadQlrZhSfcj(xmid, name,isLoad) {
    getReturnData("/slym/qlr/list/xm", {xmid: xmid, lclx: "zhlc"}, 'GET', function (data) {
        generateQlrxxSfcj(filterQlrZh(data), xmid, name,isLoad);
    }, function (err) {
        console.log(err);
    },false);
}
...
//组织项目信息到组合页面（司法裁决）
function generateXmZhSfcj(data, name, qlTableId, xmfb) {
    ...
    if (isNotBlank(data.xmid)) {
        // 查看外联证书
        getReturnData("/slym/xm/queryWlzs", {xmid: data.xmid}, 'GET', function (data) {
            if (isNotBlank(data)) {
                wlzs = data;
            }
        }, function (err) {
    ...
    if ($(".jyxx").length > 0 && isNotBlank(data.xmid)) {
        getReturnData("/ycsl/jyxx", {xmid: data.xmid}, "GET", function (data) {
            //数据库查询到交易信息数据以数据库为准
            if (data) {
                data.yhtbh = data.htbh;
                //表单赋值
                form.val("xmForm", data);
            }
...
function loadZdjbxx(bdcdyh) {
    if($(".zdjbxx").length >0){
        getReturnData("/slym/ql/zdjbxx",{bdcdyh:bdcdyh},"GET",function (data) {
            var zdjbxxArray = $(".zdjbxx").serializeArray();
            if(zdjbxxArray.length >0){
                zdjbxxArray.forEach(function (item, index) {
                    var name =item.name;
...
//组织合同信息到组合页面（司法裁决）
function generateHtxxZhSfcj(xmxx, name, qlTableId) {
    cxHtxx(xmxx).done(function(data){
        var bdchtxxTpl;
        if(isNotBlank(data)){
            if(data.htxx.fwlx == "clf"){
...
//组织权利人数据到页面
function generateQlrxx(data, xmid, name, index,isLoad) {
    var sfdydj = false;
...
//组织权利人数据到页面（司法裁决）
function generateQlrxxSfcj(data, xmid, name,isLoad) {
    var sfdydj = false;
    getReturnData("/slym/ql/checkDydj", {xmid: xmid}, "GET", function (data) {
        sfdydj = data;
    }, function () {
    ...
    if(isLoad == null ||!isLoad){
        ...
        form.render();
        renderForm();
        getStateById(readOnly, formStateId, "slymzh", 'qllx');
        //重新渲染select框
        form.render('select');
...
function addQlr(qllx, dydj) {
    ...
    var url = getContextPath() + "/view/slym/qlr.html?xmid=" + addXmid + "&lclx=" + lclx + "&processInsId=" + processInsId + "&formStateId=" + formStateId + "&qllx=" + qllx + "&zxlc=" + zxlc + "&qlrid=" + qlrid;
    if (dydj) {
        layer.open({
            type: 2,
            area: ['960px', '610px'],
...
function showQlr(qlrid, xmid, qlrlb, elem) {
    ...
    var title = '';
    var url = getContextPath() + "/changzhou/slym/qlr.html?processInsId=" + processInsId + "&qlrid=" + qlrid + "&lclx=" + lclx + "&xmid=" + xmid + "&formStateId=" + formStateId + "&qllx=" + qllx + "&readOnly=" + readOnly + "&zxlc=" + zxlc +"&djxl="+djxl;
...
function syncBdcdyxxSfcj(bdcSlQlxxym, bdcSlQlxxymDTOList) {
    var bdcdyh = bdcSlQlxxym.bdcXm.bdcdyh;
    var djxl = bdcSlQlxxym.bdcXm.djxl;
    //不动产单元信息同步
    var $bdcdyxx = $("#bdcdyxxForm").find(".xmxx");
    $bdcdyxx.on('change', function () {
        var id = $(this)[0].id;
        var value = $(this).val();
        //定着物面积与建筑面积同步
        if (id === "dzwmj") {
            $("#qllx").find("input[name=jzmj]").val(value);
        }
        //循环同步其他TAB权利页数据
        for (var i = 0; i < bdcSlQlxxymDTOList.length; i++) {
            var dyh = bdcSlQlxxymDTOList[i].bdcXm.bdcdyh;
    ...
    form.on('select', function (data) {
        ...
        if ($select.length > 0) {
            ...
            //定着物用途与规划用途同步
            if (id === "dzwyt") {
                $("#qllx").find("select[name=ghyt]").val(value);
            }
            ...
            //定着物用途与规划用途同步
            if (name === "ghyt") {
                $("#bdcdyxxForm").find("select[name=dzwyt]").val(value);
                //循环同步其他TAB权利页数据
                for (var i = 0; i < bdcSlQlxxymDTOList.length; i++) {
                    var dyh = bdcSlQlxxymDTOList[i].bdcXm.bdcdyh;
                    if (bdcdyh === dyh) {
    ...
    //建筑面积监听
    var $jzmj = $("#qllx").find("input[name=jzmj]");
    if ($jzmj.length > 0) {
        $jzmj.on('change', function () {
            var value = $(this).val();
            $("#bdcdyxxForm").find("input[name=dzwmj]").val(value);
            //循环同步其他TAB权利页数据
            for (var i = 0; i < bdcSlQlxxymDTOList.length; i++) {
                var dyh = bdcSlQlxxymDTOList[i].bdcXm.bdcdyh;
                if (bdcdyh === dyh) {
...
function lpb(bdcdyh) {
    addModel();
    getReturnData("/rest/v1.0/slym/fwbdcdy",{bdcdyh:bdcdyh},"GET",function (data) {
