...
layui.use(['element', 'form', 'jquery', 'laydate', 'laypage', 'laytpl', 'layer'], function () {
    ...
    $('body').on("click", "#addZhengshu", function () {
        ...
        var djxlOption ="";
        if (zdDjxlList !== null && zdDjxlList.length > 0 && zyhzData !== null && zyhzData.zyhzDjxlList && zyhzData.zyhzDjxlList.length > 0) {
            for (var i = 0; i < zdDjxlList.length; i++) {
                var djxlZd = zdDjxlList[i];
                for(var j=0; j<zyhzData.zyhzDjxlList.length;j++) {
                    if(djxlZd.DM == zyhzData.zyhzDjxlList[j]) {
                        if(djxlZd.DM == djxl) {
                            djxlOption +='<option value="' + djxlZd.DM + '" title="' + djxlZd.MC + '" selected="selected">' + djxlZd.MC + '</option>';
                        } else {
                            djxlOption +='<option value="' + djxlZd.DM + '" title="' + djxlZd.MC + '">' + djxlZd.MC + '</option>';
                        }
                    }
                }
        ...
        var top = 41+185*(newnum - 1);
        //转移换证流程
        var html = '';
        if(zyhzData.zyhzGzldyidList !== null && zyhzData.zyhzGzldyidList.length>0 && zyhzData.zyhzGzldyidList.indexOf(gzldyid)>-1) {
            html = ' <div class="main-content-container-right-item bdc-ryzhzs" data-options="" data-certificate-index="' + newnum + '" name="zsxh">' +
                '<div class="main-content-container-right-item-title">' +
                '<span>证书' + newnum + '</span>' +
                '<div class="bdc-ryzh"  style="top:'+ top +'px">\n' +
                '<div class="bdc-num-box">\n' +
                "<select class='bdc-djxl' name='djxl' lay-search='true' lay-verify='required'\n" +
                'lay-filter="djxl'+ newnum + '" id="djxl">\n' +
                djxlOption +
                '</select>\n' +
                '</div>\n' +
                '</div>' +
        ...
        form.render();
        //select监听事件
        form.on('select(djxl'+num +')', function(data){
...
function loadSelectBdcdyh(page, size, queryData) {
    $.ajax({
        ...
        success: function (data) {
            ...
            getReturnData("/gwc/zyhz",{},"GET",function(data){
                if(data) {
                    zyhzData = data;
                }
            },function (xhr) {
...
function initBdcdyh(bdcYxYwxxDTOList) {
    layui.use(['form', 'jquery', 'laytpl', 'element'], function () {
        ...
        var json = {
            ...
            type: type,
            sfzyhz:sfzyhz,
            zyhzDjxlList:zyhzData.zyhzDjxlList!=null ? zyhzData.zyhzDjxlList : {}
        };
        //获取模板
    ...
    if (zsxhSpanArray != null && zsxhSpanArray.length > 0) {
        ...
        for (var i = 0; i < zsxhSpanArray.length; i++) {
            ...
            $(zsxhSpan).find(".reverse-drag").each(function () {
                var djxl = $(zsxhSpan).find(".bdc-djxl").val();
                var slxm = {};
                slxm.zsxh = zsxh;
