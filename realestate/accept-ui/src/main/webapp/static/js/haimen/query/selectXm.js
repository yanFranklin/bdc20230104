function initXmTable() {
    layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
        ...
        var unitTableTitle = [{type: 'checkbox', fixed: 'left'},
            ...
            {field: 'ghyt', width: 86, title: '规划用途', templet: '#ghyt'},
            {field: 'qlrmc', title: '权利人'},
            {field: 'qlrzjh', title: '权利人证件号'},
            {
                field: 'pplzzt', title: '匹配落宗状态', width: 160, align: "center", templet: function (obj) {
    ...
    getReturnData("/bdcdyh/pplzzt?xztzlx=" + xztzlx, JSON.stringify(checkData), "POST", function (data) {
        ...
        if (data !== null) {
            if(isNotBlank(data.mullz)){
                ...
                if (data.sfpp === "false") {
                    //已落宗未匹配提示
                    showConfirmDialog("提示", "未匹配土地证号，是否创建流程", "cshSelectedXxSingle", "'" + jbxxid + "','" + processDefKey + "'", "checkPp", "'" + JSON.stringify(checkData) + "','" + xztzlx + "'",true);
 ...
 * @description 查询规划用途，项目内多幢查询bdc_fdcq_fdcqxm表多个拼接展示
 * @date : 2020/8/10 10:42
 */
function changeGhyt(dzwyt,bdcdyfwlx,xmid) {
    //项目内多幢查询bdc_fdcq_fdcqxm表多个拼接展示
    if(bdcdyfwlx === 1) {
        getReturnData("/bdcdyh/ghyt",{xmid:xmid},"GET",function (data) {
            dzwyt = data;
        },function (xhr) {
            delAjaxErrorMsg(xhr);
        },false)
    } else {
        for (var i = 0; i < zdGhytList.length; i++) {
            if (dzwyt === zdGhytList[i].DM) {
                dzwyt = zdGhytList[i].MC;
            }
        }
    }
    return dzwyt;
