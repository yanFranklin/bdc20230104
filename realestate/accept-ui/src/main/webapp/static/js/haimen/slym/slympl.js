    ...
    if (qllx === 4 || qllx === 6 || qllx === 8) {
        ...
        if (zxlc === "true") {
            ...
            cols = [
                ...
                // {field: 'bdbzzqse', title: '被担保主债权数额(万元)', minWidth: 200},
                {
...
function addQlr(qllx) {
    ...
    layui.use(['form', 'jquery', 'laytpl', 'element', 'laydate'], function () {
        var form = layui.form;
        var url = getContextPath() + "/nantong/slym/qlr.html?xmid=" + xmid + "&lclx=" + lclx + "&processInsId=" + processInsId + "&formStateId=" + formStateId + "&qllx=" + qllx + "&zxlc=" + zxlc;
        if (dydj) {
            url = url + "&dydj=true";
        }
        layer.open({
