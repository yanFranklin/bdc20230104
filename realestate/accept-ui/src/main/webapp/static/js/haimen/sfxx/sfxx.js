...
layui.use(['jquery', 'laydate', 'form', 'laytpl'], function () {
    ...
    $(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (null != qlrData.bdcSlQlrSfxxDTO){
                        ...
                        //渲染权利人收费项目表格
                        renderSfxm(qlrData.renderList.sfxxid, '#qlrSfxm table','1');
                        //月结的机构推送按钮不可点击
                        if(qlrData.renderList.sfayjs || isNotBlank(qlrData.renderList.jfsbm)) {
                    ...
                    if (ywrData.bdcSlYwrSfxxDTO != null) {
                        ...
                        //sfxxid为空,义务人收费模块隐藏
                        // if (!isNotBlank(ywrData.renderList.sfxxid)) {
                        //     $("#ywrSfxm").hide();
                        // }else{
                            //渲染义务人收费项目表格
                            renderSfxm(ywrData.renderList.sfxxid, '#ywrSfxm table','2');
                        //月结的机构推送按钮不可点击
                        if(ywrData.renderList.sfayjs || isNotBlank(ywrData.renderList.jfsbm)) {
        ...
        //渲染收费项目列表
        function renderSfxm(sfxxid, view, qlrlb) {
            $.ajax({
                    ...
                    data.forEach(function (v) {
                        ...
                        if((isNotBlank(v.sfxmmc) &&v.sfxmmc.indexOf("登记费") >-1 ||v.sfxmmc.indexOf("工本费") >-1 ||v.sfxmmc.indexOf("土地使用权交易服务费") >-1) &&v.sl ===null) {
                            ...
                            if($.isEmptyObject(bdcSlSfxmSlDTO)){
                                getReturnData("/sf/sfxm/sl/nantong",{gzlslid:processInsId,djxl:djxl},"GET",function (data) {
                                    if(data){
                                        bdcSlSfxmSlDTO =data;
                                    }
                                },function (error) {
                        ...
                        getSfxmList.forEach(function (value) {
                            if (value.sfxmdm == v.sfxmdm) {
                                if(isNotBlank(value.bdcSlSfxmSfbzList)){
...
// 生成电子发票
function scdzfp(qlrlb){
    var processInsId = getQueryString("processInsId");
    var xmid = getQueryString("xmid");
    var param = {"qlrlb": qlrlb, "gzlslid": processInsId, "xmid": xmid};
    $.ajax({
        url: getContextPath() + "/sf/xx/hkdzfp",
        type: "POST",
        ...
        success: function (data) {
            removeModal();
            if (data && data.success === true){
                ityzl_SHOW_SUCCESS_LAYER("电子发票生成成功");
            } else {
                warnMsg("电子发票生成失败");
            }
        }, error: function (xhr, status, error) {
...
function zfsfxx(qlrlb){
    ...
    showTfyy().done(function (tfyy) {
        addModel('作废中');
        var sfxx = { xmid: xmid, qlrlb: qlrlb, gzlslid: processInsId, tfyy: tfyy};

        // 判断接口调用模式
        var sftsfsxt =false;
        getReturnData("/sf/nantong/sfmode",{gzlslid: processInsId, configName: "fs.config.zf.beanName"},"GET",function (data){
            sftsfsxt = data;
        },function (error){
            delAjaxErrorMsg(error);
            return false;
...
// 新接口模式，采用beanName调用exchange进行作废
