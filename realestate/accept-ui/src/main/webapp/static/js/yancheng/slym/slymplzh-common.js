            ...
            if ($(this).hasClass('bdc-first-click')) {
                ...
                setTimeout(function () {
                    $.when(loadTabQlr(tabXmid, tabDjxl, '.layui-show'), loadPlQlxx(tabXmid), loadBdcdyh(tabDjxl, tabQllx, tabXmid)).done(function () {
                        generateDsQlr(tabXmid, $('.layui-show').find(".sqr"),"");
                        loadLzrxx(tabXmid, tabDjxl, $('.layui-show').find(".sqr"));
...
function generateTabContent() {
    ...
    if (sfchangeqljbxxTab) {
        $.when(loadTabQlr(qlxx[0].bdcXm.xmid, qlxx[0].bdcXm.djxl, '.layui-show'), loadPlQlxx(qlxx[0].bdcXm.xmid), loadBdcdyh(qlxx[0].bdcXm.djxl, qlxx[0].bdcXm.qllx, qlxx[0].bdcXm.xmid)).done(function () {
            loadLzrxx(qlxx[0].bdcXm.xmid, qlxx[0].bdcXm.djxl, $('.layui-show').find(".sqr"));
            var a = setInterval(function () {
                if ($('.bdc-form-div .layui-show .bdc-table-box .layui-table-main>.layui-table').height() !== 0) {
                    getStateById(readOnly, formStateId, "slympl", "qlxxTab");
                    clearInterval(a);
 ...
 * 加载领证人信息
 */
function loadLzrxx(xmid, djxl, $sqr) {
    getReturnData("/slym/lzr/sfzs", {xmid: xmid}, 'GET', function (data) {
        if (data) {
            getReturnData("/slym/lzr/nt/lzrxx", {xmid: xmid}, 'GET', function (lzrdata) {
                ...
                form.render();
                getStateById(readOnly, formStateId, 'slymplzh', "lzrxx");
                resetSelectDisabledCss();
            }, function (err) {
                console.log(err);
            });
...
function showQlr(qlrid, xmid, qlrlb, djxl) {
    ...
    addModel();
    var qllx = $('.layui-show').find("input[name='qllx']").val();
...
//删除义务人列表
function deleteYwr(qlridlist, sxh, xmid, qlrid, otherXmidList, djxl, ywrmc, ywrzjh) {
    var xmidList;
    if (isNotBlank(otherXmidList)) {
...
function deleteLzr(qlrid, lzrid, djxl) {
    if (isNullOrEmpty(djxl)) {
        djxl = $('.layui-show').data('djxl');
    }
    var url = "/slym/lzr/yc/pllc?lzrid=" + lzrid + "&qlrid=" + qlrid + "&djxl=" + djxl + "&gzlslid=" + processInsId;
    getReturnData(url, '', 'DELETE', function (data) {
        console.log('删除领证人成功');
        loadLzr();
    }, function (xhr) {
    ...
    for (var i = 1; i <= $('.layui-tab-item').length; i++) {
        ...
        if (isNotBlank(tabDjxl)) {
            ...
            if (lzrArrayPllc.length !== 0) {
                saveAllLzr(lzrArrayPllc, tabDjxl);
            }
        }
    }
    //刷新领证人，防止重复新增
...
function saveAllLzr(lzrArray, tabDjxl) {
    var lzr = {};
...
function loadLzr() {
    for (var i = 1; i <= $('.layui-tab-item').length; i++) {
        ...
        var $nowTab = $('.layui-tab-item:nth-child(' + i + ')');
        if (isNotBlank(tabXmid)) {
            loadLzrxx(tabXmid, tabDjxl, $nowTab.find(".sqr"));
        }
    }
}
...
//受理页面-点击保存
function saveSlym() {
    $.when(saveXmxx(),saveFbPlzh(),saveSjcl(), saveLzr(), saveQlxx(), saveQlr(), saveJyxx()).done(function () {
        saveDsQlr();
        removeModal();
