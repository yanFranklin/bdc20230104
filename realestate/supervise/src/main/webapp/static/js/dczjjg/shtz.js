...
layui.use(['form', 'jquery', 'laydate', 'element', 'table'], function () {
    ...
    function zjsh(data) {
        if(!data || !data[0] || isNullOrEmpty(data[0].id)) {
            warnMsg("请选择需要审核的质检记录");
            return false;
        }

        for(var i in data) {
            if(!isNullOrEmpty(data[i].scsj)) {
                warnMsg("存在已审核记录，不可再次审核！");
                return false;
            }
        }
        ...
        layer.open({
            ...
            area: ['960px', '400px'],
            offset: 'auto',
            content: [ "shxx.html?zjxxid=" + data[0].id, 'yes'],
    ...
    function showZgqk(zjxxid) {
        layer.open({
            ...
            area: ['960px', '500px'],
            offset: 'auto',
            content: [ "zgqktz.html?sfsh=1&zjxxid=" + zjxxid, 'yes'],
            success: function (layero, index) {
            },
            end:function(){
            }
