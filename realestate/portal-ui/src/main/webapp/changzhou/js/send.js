                ...
                if (forwardTask.length != 0) {
                    for (var i = 0, len = forwardTask.length; i < len; i++) {
                        ...
                        if (forwardDto.dispatchEnable) {
                            ...
                            // 退回任务并且没有默认人时，不查询参与人信息
                            var backTaskAndNoDefaultUser = forwardTask[i].forwardTaskDto.backTask && forwardTask[i].forwardTaskDto.defaultUserName == "allUser";
                            if(!backTaskAndNoDefaultUser){
                                var queryurl;
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        forwardTask[i].defaultUser = defaultUser;
                                    }, error: function (e) {
                                        response.fail(e, '');
                                    }
                                });
                            }
                        } else if(forwardTask[i].forwardTaskDto.activityId == 'endEvent'){
                            //下个节点为办结
                    ...
                    var allDefaultUser = true;
                    $.each(forwardTask, function(value, index){
                        if(isNullOrEmpty(value.defaultUser)){
                            allDefaultUser = false;
                        }
                    });
                    if (allDefaultUser) {
                        renderZfTable(forwardTask, defaultPersonCol);
...
// 渲染表格
        ...
        table.on('radio(checkboxTableFilter)', function (data) {
            //每次选择前判断
            //1.移除isLock属性，重新根据接口查询
            var sendBtn = $('.bdc-send-btn');
            if (sendBtn && sendBtn.length > 0) {
                for (var i = 0; i < sendBtn.length; i++) {
                    sendBtn[i].classList.remove('unAdmin')
                }
            }
            if (data.data.forwardTaskDto.dispatchEnable) {
                // 判断当前用户是否是管理员
                $.ajax({
                    type: "GET",
                    url: getContextPath() + "/rest/v1.0/user/info",
                ...
                success: function (data) {
                   /* $select.html('<option value="">请选择</option>');*/
                    ...
                    /*if(data.length > 1){
                        ...
                        $select.append('<option value="' + data[0].username + '" selected>' + data[0].alias + '</option>');
                    }*/
                    form.render('select');
                }, error: function (e) {
                    response.fail(e, '');
...
//点击转发
        $('.bdc-send-btn').on('click', function () {
            if ($(this).hasClass('islock')) {
            ...
            } else if ($(this).hasClass('unAdmin')) {
                layer.msg('非管理员，退回件请联系管理员转发');
            } else {
                // 避免重复点击
                var sendBtn = $('.bdc-send-btn');
                ...
                if (isSelectAll) {
                    ...
                    var activityId=selectData[0].forwardTaskDto.activityId;
                    //单选
                    if (checkStatus.data.length == 1) {
                        ...
                        if (isNullOrEmpty(selectUserNames) &&activityId !="endEvent") {
                            layer.msg('请选择参与人');
                        } else {
                            var selectRoleIds;
                            if ($selectTr.find('td:last-child select').length > 0) {
                            ...
                            if (sameNodes) {
                                var workflowPlList = [];
                                if (isNotBlank(bdzf) && bdzf === "true") {
                                    //如果是绑定转发，根据页面填写的受理编号的值，重新请求后台数据作为需要转发的数据
                                    var slbhList = [];
                                    $.each($('.bdslbh'), function (ket, item) {
                                        if (isNotBlank(item.value)) {
                                            slbhList.push(item.value);
                                        }
                                    });
                                    plzfList = getBindProcessData(slbhList);
                                }
       ...
       //点击取消
        ...
        function getBindProcessData(slbhList) {
            var result = {};
            getReturnData("/rest/v1.0/task/forward/bind", {slbh: slbhList.join(',')}, "GET", function (data) {
        ...
        if (bdzf === "true") {
            $('#slbhFilter').removeClass('bdc-hide');
            $('#slbh').val(isNotBlank(slbh) ? slbh : "");
            $('#bind-send').removeClass('bdc-hide');
            $('#normal-send').addClass('bdc-hide');
            $('.bdc-item-add').on('click', function () {
                var $slbhdiv = $('#slbhFilter');
                var slbhtpl = addSlbhTpl.innerHTML;
                var json = {};
                laytpl(slbhtpl).render(json, function (html) {
                    $slbhdiv.after(html);
                    $('.bdc-item-delete').on('click', function () {
