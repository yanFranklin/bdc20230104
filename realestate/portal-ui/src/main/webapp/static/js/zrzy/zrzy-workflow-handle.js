layui.define(['jquery', 'element', 'form', 'response', 'layer', 'laytpl'], function (exports) {
    ...
    function startUpProcess(data) {
        if (data) {
            window.open('/realestate-natural-ui/rest/v1.0/zrzydjdyh?processDefKey='  + data.processDefKey);
        } else {
            layer.msg('创建失败！');
        }
    }
...
//转发操作
    ...
    function forwardauto(checkStatus, url, tableId, currentId, isIndex, processInstanceId, endType) {
        ...
        $.ajax({
            type: "POST",
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/zfyz",
            async: false,
    ...
    function allowDelete(checkStatus, url, tableId, currentId, isIndex, processInstanceId){
        layer.confirm('您确定要删除吗？', {
            ...
            $.ajax({
                type: "GET",
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/lcsc/withCzrz/" + processInstanceId,
                dataType: "json",
                success: function (data) {
                    if (isIndex) {
                        layer.msg('删除成功');
    ...
    function claimProcessDetail(checkStatus, url, tableId, currentId, openpage) {
        ...
        var gzlslidIds = [];
        var workFlowDTOList =[];
        if(checkStatus.data instanceof Array){
            ...
            checkResult = checkStaus(checkStatus.data, 6, '认领');
            if (!checkResult) {
                return false;
            }
            var workFlowDTO ={};
            taskIds.push(checkStatus.data.taskId);
            workFlowDTO.taskId = checkStatus.data.taskId;
            if (gzlslidIds.indexOf(checkStatus.data.processInstanceId) == -1) {
                gzlslidIds.push(checkStatus.data.processInstanceId);
                workFlowDTO.processInstanceId =checkStatus.data.processInstanceId;
            }
            workFlowDTOList.push(workFlowDTO);
        }
        if (taskIds.length == 0) {
        ...
        $.ajax({
            type: "post",
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/task/claim",
            traditional: true,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            ...
            success: function (data) {
                ...
                layer.msg('认领成功');
                $('#todoTab').click();
                var rlUrl = getContextPath() + "/rest/v1.0/zrzy/task/claim/list";
                $.ajax({
                    type: "POST",
                    url: rlUrl,
                    success: function (data) {
    ...
    function cancelClaimProcess(checkData, url, tableId, currentId, isIndex) {
        var logoutIndex = layer.confirm('您确定要取消认领吗？', {
            ...
            $.ajax({
                type: "post",
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/task/claim/cancel",
                traditional: true,
                ...
                success: function (data) {
                    ...
                    $.ajax({
                        type: "post",
                        url: getContextPath() + "/rest/v1.0/zrzy/task/todo",
                        traditional: true,
                        success: function (data) {
                            if (data.totalElements > 99) {
    ...
    function cancelClaimPlProcess(checkData, url, tableId, currentId, isIndex) {
        var logoutIndex = layer.confirm('您确定要批量取消认领吗？', {
            ...
            $.ajax({
                type: "post",
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/task/claim/cancel/pl",
                contentType: "application/json;charset=utf-8",
                dataType: "json",
...
//判断是否可以退回
    function allowBack(taskId) {
        ...
        $.ajax({
            type: "get",
            async: false,
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/back/isAllow",
            data: {
                taskId: taskId
            },
            success: function (data) {
...
//判断是否可以取回
    function allowFetchBack(taskId) {
        ...
        $.ajax({
            type: "get",
            async: false,
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/task/isAllowFetchBack",
            data: {
                taskId: taskId
            },
            success: function (data) {
    ...
    function fetchProcess(checkData, doneUrl, doneTableId, doneCurrentId, isIndex) {
        ...
        var logoutIndex = layer.confirm('您确定要取回吗？', {
            ...
            $.ajax({
                type: "get",
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/task/fetchBack",
                traditional: true,
    ...
    function fetchPlProcess(checkData, doneUrl, doneTableId, doneCurrentId, isIndex) {
        ...
        var logoutIndex = layer.confirm('您确定要取回吗？', {
            ...
            $.ajax({
                type: "get",
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/task/plFetchBack",
                traditional: true,
        ...
        if (checkResult == false) {
            ...
            var logoutIndex = layer.open({
                ...
                ,yes: function(index, layero) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/abandon",
                        contentType: "application/json;charset=utf-8",
                        dataType: "json",
        ...
        layer.open({
            ...
            , yes: function (index, layero) {
                ...
                if (isNullOrEmpty(reason)) {
                    ...
                    $.ajax({
                        type: "post",
                        url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/hang",
                        traditional:true,
                        data: {
        ...
        if (workflowBackList!= null && workflowBackList.length > 0) {
            layer.open({
                ...
                ,yes: function(index, layero){
                    ...
                    var thyjCheck = "";
                    $.ajax({
                        url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/getThyjCheck",
                        dataType: "json",
                        async: false,
                        success: function (data) {
                            thyjCheck = data;
                    ...
                    $.ajax({
                        type: "POST",
                        contentType: "application/json;charset=utf-8",
                        url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/back/pl",
    ...
    function plForward(url, tableId, currentId, statusResults) {
        $.ajax({
            type: "POST",
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/task-handel/batch-forward/complete",
            contentType: "application/json;charset=utf-8",
            ...
            if (sameNodes) {
                ...
                if (end) {
                    ...
                    var send = layer.open({
                        ...
                        title: '任务转发',
                        area: area,
                        content: '../view/zrzy/send.html?currentId=' + currentId + '&name=' + workflow.taskId + "&isEnd=" + end + "&processInstanceId=" + workflow.processInstanceId + "&sameNodes=" + sameNodes + "&plzfkey=" + plzfkey + "&bdzf=" + bdzf + "&slbh=" + checkStatus.data[0].slbh,
                        end: function () {
...
//    批量转发右侧显示异常信息
    ...
    function preformBack(workflowList, isIndex, url, tableId, currentId) {
        ...
        var logoutIndex = layer.open({
            ...
            , yes: function (index, layero) {
                ...
                $.ajax({
                    type: "DELETE",
                    url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/rl",
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
                    data: JSON.stringify(workflowList),
                    success: function (data) {
    ...
    function preformDelte(workflowList, isIndex, url, tableId, currentId) {
        ...
        $.ajax({
            type: "GET",
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/getDeleteReason",
            success: function (data) {
                if (data == '1') {
                    var logoutIndex = layer.open({
                        title: '删除',
                        ...
                        ,yes: function(index, layero){
                            ...
                            $.ajax({
                                type: "DELETE",
                                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances",
                                contentType: "application/json;charset=utf-8",
                                dataType: "json",
                                data: JSON.stringify(workflowList),
                                success: function (data) {
    ...
    function preformStop(workflowList, isIndex, url, tableId, currentId){
        ...
        layer.open({
            ...
            yes: function(index, layero){
                ...
                $.ajax({
                    type: "POST",
                    url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/stopTask",
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
                    data: JSON.stringify(workflowList),
                    success: function (data) {
        ...
        layer.open({
            ...
            , yes: function (index, layero) {
                ...
                if (isNullOrEmpty(reason)) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/activate",
                        traditional:true,
                        data: {
                            processInstanceIds: processInstanceIds,
                            reason: reason
...
//办结
    function endProcess(checkData, url, tableId, currentId, isIndex) {
        ...
        $.ajax({
            type: "POST",
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/end",
            data: {taskId: checkData.taskId},
            async: false,
            success: function () {
                if (isIndex == 'true') {
    ...
    function lockProcess(checkData) {
        if (checkData.state != 3) {
            ...
            $.ajax({
                type: "post",
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/lock",
                data: {
            ...
            success: function () {
                ...
                $('.confirmRemove').click(function () {
                    ...
                    if (alertSize == 0 && $(".confirmRemove").size() == 0) {
                        addModel();
                        callbackFn(checkData, url, tableId, currentId, isIndex, processInstanceId);
                        // 避免转发页面不展示将关闭操作放到后面执行
                        layer.close(warnLayer);
                    }
                });
