layui.use(['jquery', 'table', 'element', 'form', 'laytpl', 'response'], function () {
    ...
    $(function () {
        ...
        $.ajax({
            type: "GET",
            url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/forward/taskAndRole",
            data: {
                taskId: taskId
            },
                    ...
                    for (var i = 0, len = forwardTask.length; i < len; i++) {
                        ...
                        if (forwardDto.dispatchEnable) {
                            ...
                            if (!isNullOrEmpty(forwardTask[i].forwardTaskDto.selectUserNames)) {
                                ...
                                queryurl = getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/forward/user?username=" + defaultUser;
                            } else {
                                var taskId = forwardTask[i].forwardTaskDto.taskId;
                                var activityId = forwardTask[i].forwardTaskDto.activityId;
                                queryurl = getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/forward/users?roleId=" + forwardTask[i].roleDtoList[0]['id']+"&taskId="+taskId+"&activityId="+activityId;;
                            }
                            $.ajax({
                                type: "GET",
                                async: false,
...
//默认渲染第一个角色的，选择角色，重新渲染
        function renderPerson(roleId, $select) {
            ...
            $.ajax({
                type: "GET",
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/forward/users",
                data: {
                    roleId: roleId,
                    taskId:taskId,
                    ...
                    if (checkStatus.isAll) {
                        ...
                        if ($selectTr.find('td:last-child .layui-anim-upbit .layui-this').length == checkStatus.data.length) {
                            ...
                            $.ajax({
                                type: "POST",
                                traditional: true,
                                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/forward/parallel",
                            ...
                            if (sameNodes) {
                                ...
                                zfhyz(zfhyzData,function () {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        traditional: true,
                                        url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/forward",
                                        data: {
       ...
       //点击取消
        ...
        function zfhyz(zfhyzData,zfcallback){
            ...
            $.ajax({
                type: "POST",
                traditional: true,
                url: getContextPath() + "/rest/v1.0/workflow/zrzy/process-instances/forward/zfhyz",
                data: {
