...
function saveCols(data, tableid, url) {
    $.ajax({
        type: 'POST',
        url: '/realestate-portal-ui/rest/v1.0/gxhpz/save/table?url=' + encodeURI(url) + '&tableid=' + tableid,
...
function getGxhpz(tableid) {
    var url;
    if (window.location.pathname != "/realestate-portal-ui/view/dtcx/commonCx.html") {
    ...
    $.ajax({
        type: 'GET',
        url: '/realestate-portal-ui/rest/v1.0/gxhpz/query/table?url=' + encodeURI(url) + '&tableid=' + tableid,
...
function formLoad() {
    ...
    for (var i = 0; i < forms.length; i++) {
        ...
        $.ajax({
            type: 'GET',
            url: '/realestate-portal-ui/rest/v1.0/gxhpz/query/form?url=' + encodeURI(url) + '&formid=' + formid,
            async: false,
            success: function (data) {
                if (data) {
    ...
    $(".bdc-set-search").on('click', function () {
        ...
        if (sameFlag == false) {
            ...
            $.ajax({
                type: 'POST',
                url: '/realestate-portal-ui/rest/v1.0/gxhpz/save/form?url=' + encodeURI(url) + '&formid=' + formid,
        ...
        var index = layer.open({
            ...
            content: "/realestate-portal-ui/view/gxhpz/searchPz.html?formid=" + formid + "&url=" + encodeURI(url),
            btn: ['保存', '取消'],
