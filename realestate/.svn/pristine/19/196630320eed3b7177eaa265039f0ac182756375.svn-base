...
layui.use(['element', 'form', 'jquery', 'laydate', 'laytpl', 'layer', 'formSelects'], function () {
    ...
    // 是否需要保存时候生成PDF文件到附记
    var needPdf = $.getUrlParam('needPdf');
    var xmid;
    ...
    //发证材料补充
    form.on('submit(fzclbc)', function (data) {
        fzclbc(null, true);
    });
    ...
    function addSfzYz(zjzl, elem) {
        ...
        if (zjzl === "1" || zjzl === 1) {
            ...
            if(isNotBlank(zjhVal) && zjhVal.indexOf(",") > 0){
                // 证件号信息为多个人的证件号时，不进行验证
                return;
            }
            //增加身份证验证
    ...
    function generateFzjlPdf(xmid) {
        ...
        // 南通不需要保存时候生成PDF，预留此功能，通过参数控制，默认不生成
        if(!(needPdf === true || needPdf === "true")) {
            return;
        }
