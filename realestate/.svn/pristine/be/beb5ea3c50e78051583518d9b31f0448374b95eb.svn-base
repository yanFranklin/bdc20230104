...
layui.use(['table', 'laytpl', 'laydate', 'layer', 'form'], function () {
    ...
    // 获取参数
    var gzlslid = getQueryString('gzlslid')||getQueryString('processInsId');
    var dangqianGzldyid = getQueryString('processDefKey');
    var bdcdyh = getQueryString('bdcdyh');
    var qllxlist = getQueryString('qllxlist');
    ...
    function generateTable() {
        table.render({
            ...
            where: {"gzlslid": gzlslid, "qllxlist": qllxlist, "sfcxls": '1', "cxlx":cxlx, "version":"changzhou", "dangqianGzldyid": dangqianGzldyid},
            request: {
            ...
            cols: [[
                ...
                {minWidth: 230, sort: false, field: 'fjh', title: '房间号'},
                {minWidth: 230, sort: false, field: 'jzmj', title: '建筑面积'},
                {minWidth: 100, sort: false, field: 'dyfs', title: '抵押方式'},
                {minWidth: 100, sort: false, field: 'fkfs', title: '付款方式'},
                {field: 'djxl', sort: false,width: 100, title: '登记小类',
                    templet: function(d){
                        var result=d.djxl;
 ...
 * @param data
 */
function openCqdz(data){
    var xmid = data.xmid;
    if(xmid){
        $.ajax({
            url: "/realestate-register-ui/rest/v1.0/bdcdy/cqdz?xmid="+xmid,
            type: 'get',
            success: function (data) {
                console.log("产权大证:"+data);
                if(isNullOrEmpty(data)){
                    warnMsg("没有产权大证信息！");
                }
                if(!isNullOrEmpty(data) && '1' == data[0].xmly){
                    window.open('/realestate-portal-ui/view/new-page.html?type=lsgx&processinsid=' + data[0].gzlslid);
                } else {
                    window.open('/realestate-register-ui/view/lsgx/new-page.html?xmid=' + data[0].xmid + '&processInsId=' + data[0].gzlslid + '&version=changzhou');
                }
            },
            error: function (err) {
                warnMsg("产权大证信息查询失败！");
...
// 通过工作流id获取项目不动产单元号
function listBdcdyhByGzlslid(gzlslid){
    var dataObj ;
    $.ajax({
        url: '/realestate-register-ui/rest/v1.0/bdcdy/xmxx/'+ gzlslid,
        type: "GET",
        async: false,
        dataType: "json",
        success: function (data) {
            for(i = 0; i < data.length; i++) {
                if(i == 0){
                    dataObj = data[i].bdcdyh;
                }else{
                    dataObj = dataObj +','+data[i].bdcdyh;
                }
            }
        }
    });
