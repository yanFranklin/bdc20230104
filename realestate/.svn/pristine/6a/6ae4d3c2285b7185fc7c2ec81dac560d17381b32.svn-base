 ...
 */
public class ComputeZqxFzlTask implements Runnable {

    private static final Logger LOGGER = LoggerFactory.getLogger(ComputeZqxFzlTask.class.getName());

    private static final String jdmc = "发证";
    /**
    ...
    public ComputeZqxFzlTask(GzltjQO gzltjQO,
    ...
    public void run() {
        ...
        LOGGER.info("计算周期性发证量开始，耗时：{},  查询参数：{}", System.currentTimeMillis() - startTime, JSON.toJSONString(param));
        if (CommonConstantUtils.GZLTJ_TJWD_RY.equals(param.getDimension())
                && CommonConstantUtils.GZLTJ_TJLX_XM.equals(param.getTjlx())) {
            szlList = this.bdcGzltjXmxxMapper.countRyFzlByXmid(param);
            ...
            szlList = this.bdcGzltjXmxxMapper.countRyFzlByGzlslid(param);
        } else if (CommonConstantUtils.GZLTJ_TJWD_BM.equals(param.getDimension())
                && CommonConstantUtils.GZLTJ_TJLX_XM.equals(param.getTjlx())) {
            szlList = this.bdcGzltjXmxxMapper.countBmFzlByXmid(param);
        } else {
            szlList = this.bdcGzltjXmxxMapper.countBmFzlByGzlslid(param);
        }
        ...
        if (CollectionUtils.isNotEmpty(szlList)) {
            ...
            if (MapUtils.isNotEmpty(resultMap)) {
                ...
                while (bdcGzltjXmxxVOIterator.hasNext()) {
                    ...
                    String key = "";
                    if(CommonConstantUtils.GZLTJ_TJWD_BM.equals(param.getDimension())){
                        key = bdcZqxGzltjXmxxVO.getBmdm();
                    }
                    if(CommonConstantUtils.GZLTJ_TJWD_RY.equals(param.getDimension())){
                        key =  bdcZqxGzltjXmxxVO.getRymc();
                    }
        ...
        LOGGER.info("计算周期性发证量结束，耗时：{},  查询参数：{}", System.currentTimeMillis() - startTime, JSON.toJSONString(param));
    }
    ...
    private Map<String, Integer> convertMap(String tjwd, List<BdcGzltjXmxxDTO> bdcGzltjXmxxDTOList) {
        ...
        if (CommonConstantUtils.GZLTJ_TJWD_RY.equals(tjwd)) {
            return bdcGzltjXmxxDTOList
                    .stream()
                    .filter(t -> StringUtils.isNotBlank(t.getRyid()))
