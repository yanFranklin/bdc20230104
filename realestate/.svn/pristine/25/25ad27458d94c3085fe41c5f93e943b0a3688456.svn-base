...
// 有房证明打印模板
var yffczmDymb = getIP() + "/realestate-inquiry-ui/static/printModel/yffczm.fr3";
// 无房证明打印模板
var wffczmDymb = getIP() + "/realestate-inquiry-ui/static/printModel/wffczm.fr3";
// (土地)权属证明打印模板
...
// 有无土地信息打印模板
var tdYwtdxx = getIP() + "/realestate-inquiry-ui/static/printModel/ywtdxx.fr3";
// 证明验证对应的组合规则标识
// 有房无房证明
...
// 是否是勾选打印有房无房证明  s 是  f 否
var sfgxyfwf = getQueryString("sfgxyfwf");
//获取页面版本
...
var mhlx = {
    ...
    "fwbhmhlx": 0, "zhlshmhlx": 0, "ybdcdyhmhlx": 0, "yxtcqzhmhlx": 0, "zsyzhmhlx": 0, "htbhmhlx": 0
};
// 是否重新获取缓存数据
var sfhqhc = true;
...
layui.use(['form', 'jquery', 'element', 'table'], function () {
    ...
    $(function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: 'djyy', width: 200, title: '登记原因', sort: false,},
                {
                    ...
                    templet: function (d) {
                        return zdGhytToMc('fwyt', d.ghyt, zdList,d.zdzhyt);
                    }
                },
                ...
                {
                    field: 'qszt', width: 110, title: '权属状态', fixed: 'right', sort: false,
                    templet: function (d) {
                ...
                {title: '操作', fixed: 'right', toolbar: bar, width: 240}
            ]],
            data: [],
            page: true,
        ...
        table.on('tool(pageTable)', function (obj) {
            ...
            } else if (obj.event === 'djlsck') {
                if(!isNullOrEmpty(data) && '1' == data.xmly){
                    window.open('/realestate-portal-ui/view/new-page.html?type=lsgx&processinsid=' + data.gzlslid);
                } else {
                    window.open('/realestate-register-ui/view/lsgx/new-page.html?xmid=' + data.xmid + '&processInsId=' + data.gzlslid + '&slbh=' + data.slbh);
                }
            }
        });

        //根据bdcdyh查询现势的不动产单元登记历史关系
        function queryXmls(bdcdyh){
            var datals = {};
            $.ajax({
                url: "/realestate-inquiry-ui/rest/v1.0/bdcdy/getlsgx?bdcdyh=" + bdcdyh,
                // url: '/realestate-register-ui/rest/v1.0/bdcdy/lsgx?qszt=1&bdcdyh=' + bdcdyh,
                type: "GET",
                async: false,
                dataType: "json",
                success: function (data) {
                    datals =  data.content[0];//查看最新的一条记录，sql根据按照受理时间倒序排序
                },
        ...
        var dylxArr = ["yffczm", "wffczm", "yfwfzm", "fwqszm", "tdqszm","ywtdxx"];
        ...
        table.on('toolbar(pageTable)', function (obj) {
            ...
            switch (obj.event) {
                ...
                case 'yfwfzm':
                    ...
                    break;
                case 'ywtdxx':
                    ywtdxx(checkStatus.data, obj.config.cols[0]);
                    break;
                case 'qszm':
                    qszm(checkStatus.data, obj.config.cols[0]);
                    break;
                case 'addToDataArr':
                    addToDataArr(checkStatus.data);
                    break;
                case 'clearDataArr':
                    clearDataArr();
                    break;
         ...
         * 打印有房无房证明
         */
        function dyyfwf(data) {
            checkApplyRegistration(data, ZHCX_YFWFZM_ZHGZ, 'ZHCX').fail(function () {
                return;
            }).done(function () {
                var qlr = "";
                var zjh = "";
                for (let i=0;i<data.length;i++){
                    qlr = qlr+data[0].qlrmc+",";
                    zjh = zjh+data[0].qlrzjh+",";
                }
                printFczm(qlr.slice(0,-1),zjh.slice(0,-1),data);
            });
        }
        /**
         * 根据已知的值删除数组元素
        ...
        function addData(d) {
            var data = new Array();
            d.jzmj = getFjxxVal(d.xmid, "jzmj");
            d.tdsyqmj = getFjxxVal(d.xmid, "tdsyqmj");
            d.zh = getFjxxVal(d.xmid, "zh");
            d.fjh = getFjxxVal(d.xmid, "fjh");
            d.ghyt = zdGhytToMc('fwyt', d.ghyt, zdList);
            data.push(d);
            checkAndContinueAddData(data);
        }
        function addToDataArr(d) {
            if (d.length == 0) {
                warnMsg("请选择数据！");
                return false;
            }
            ...
            for (let i = 0;i < d.length;i++){
                d[i].jzmj = getFjxxVal(d[i].xmid, "jzmj");
                d[i].tdsyqmj = getFjxxVal(d[i].xmid, "tdsyqmj");
                d[i].zh = getFjxxVal(d[i].xmid, "zh");
                d[i].fjh = getFjxxVal(d[i].xmid, "fjh");
                d[i].ghyt = zdGhytToMc('fwyt', d[i].ghyt, zdList);
            }

            checkAndContinueAddData(d);
        }
        ...
        function yfwfzm(d,cols) {
            ...
            if (qlrmc!=saveqlr || qlrzjh != savezjh){
                warnMsg("请先查询进行核实数据");
                return;
            }
            //为空则检查是否需要输入证件号
            if (isNullOrEmpty(qlrzjh)){
                var flag = checkZjh(qlrmc);
                if (flag){
                    if (qlrmc.split(",").length<=1 && qlrmc.length <= 4){
                        warnMsg("权利人证件号不可为空!");
                        return;
                    }
                    if (qlrmc.split(",").length>1){
                        for (var i=0;i<qlrmc.split(",").length;i++){
                            if (qlrmc.split(",")[i].length <=4){
                                warnMsg("权利人证件号不可为空!");
                                return;
                            }
                        }
            ...
            if (qlrmc != '' && qlrzjh != '' && qlrmc.split(",").length != qlrzjh.split(",").length){
                warnMsg("权利人证件号需一一对应！");
                return;
            }
            var checkedData = layui.sessionData('checkedData');
            ...
            printFczm(qlrmc,qlrzjh,data);

        }
        // 有无土地信息,按照搜索条件中权利人信息进行查询有无土地信息
        function ywtdxx(d,cols){
            var qlrmc = $("#qlrmc").val();
            var qlrzjh = $("#qlrzjh").val();
            var ysqrgx = $("#ysqrgx").val();
            if (isNullOrEmpty(qlrmc)){
                warnMsg("请填写权利人姓名!");
                return;
            }
            ...
            // 获取当前登录用户信息
            getReturnData("/rest/v1.0/zszm/userinfo",{},"GET",function (data) {
                if(data) {
                    userInfo = data;
                }
            ...
            var bmmcList ="";
            for(var i=0;i<userInfo.orgRecordList.length;i++){
                bmmcList = bmmcList + userInfo.orgRecordList[i].name + ",";
            }
            if (bmmcList.lastIndexOf(",")===bmmcList.length-1) {
                bmmcList = bmmcList.substring(0, bmmcList.length - 1);
            }
            // 权利人证件号不为空，需要判断和名称数量是否一致
            var qlrmcArray = qlrmc.split(",");
            var qlrzjhArray = qlrzjh.split(",");
            var ysqrgxArray = ysqrgx.split(",");
            var qlrxxList = [];
            ...
            $.ajax({
                ...
                success: function (dbkey) {
                    if (!isNullOrEmpty(dbkey)) {
                        ...
                        // 保存日志
                        var xml = getXmlStr(dataUrl);
                        savePrintInfo(mburl, dataUrl, {'zmbh': dbkey, "printType": "有无土地信息"}, xml);
                        // 记录打印有无土地信息数据库记录
                        saveZhcxLogToDataBase(getSearchData(), xml, 1, "ywtdxx");
                    } else {
                        failMsg("打印出错，请重试！");
                    }
                },
        ...
        //检查证件号，若权利人类型不为1则可以不填证件号
        function checkZjh(data) {
            var result;
            $.ajax({
                url: "/realestate-inquiry-ui/rest/v1.0/zhcx/checkZjh",
                data: {
                    qlrmc : data
                },
                type: 'GET',
        ...
        function viewTableRender() {
            table.render({
                ...
                cols: [[
                    ...
                    {field: 'qlrmc', title: '权利人名称', minWidth: 100},
                    {field: 'qlrzjh', title: '权利人证件号', minWidth: 100},
                    {field: 'bdcqzh', title: '不动产权证号（明）', minWidth: 120},
                    {field: 'jzmj', title: '建筑面积', minWidth: 100},
                    {field: 'ghyt', title: '规划用途', minWidth: 100},
                    {field: 'zl', title: '坐落', minWidth: 50},
                    {fixed: 'right', title: '操作', toolbar: '#barDemoIn', width: 80}
                ]],
        ...
        // 打印有房无房证明
        function printFczm(qlrmc, qlrzjh,data) {
            //ghyt代码转名称
            for (var i = 0; i < data.length; i++) {
                data[i].ghyt = zdGhytToMc('fwyt', data[i].ghyt, zdList);
                data[i].jzmj = getFjxxVal(data[i].xmid, "jzmj");
            }
            ysqrgx = $('#ysqrgx').val();
            $.ajax({
                url: "/realestate-inquiry-ui/rest/v1.0/zhcx/bzb/yfwfzm",
                type: "POST",
                data: JSON.stringify({"qlrmc": qlrmc, "zjh": qlrzjh, "jbr": useralias,"zfxxDTOList":data, "moduleCode":moduleCode, "ysqrgx": ysqrgx}),
                contentType: 'application/json',
                dataType: "text",
                success: function (key) {
                    if (key.indexOf('REDIS') == -1){
                        if(JSON.parse(key).code == -1){
                            layer.msg('<img src="../../static/image/warn-small.png" alt="">' + JSON.parse(key).msg + ' <a class="bdc-show-more-tips" href="javascript:;">详情</a>', {
                                time: 4000,
                                success: function () {
                                    if($('#otherTips').length == 0){
                                        $('body').append('<div class="bdc-other-tips-box bdc-hide">\n' +
                                            '    <div class="bdc-other-tips">\n' +
                                            '        <p>漏选提示：<span class="bdc-close">不再提示</span></p>\n' +
                                            '        <div id="otherTips">\n' +
                                            '        </div>\n' +
                                            '    </div>\n' +
                                            '</div>');
                                    }
                                    var exceptionMsg = JSON.parse(key).detailMsg;
                                    if (exceptionMsg == '') {
                                        exceptionMsg = '<p>暂无详细信息</p>'
                                    }
                    ...
                    if (key) {
                        var yfwfmb, dylx;
                        if(!isNullOrEmpty(sfgxyfwf) && sfgxyfwf === "f") {
                            // 按照权利人名称、证件号精确查询
                            var yfwfbs = getFcxx(qlrmc, qlrzjh);
                            yfwfmb = yfwfbs === "WF" ? wffczmDymb : yffczmDymb;
                            dylx = yfwfbs === "WF" ? "wffczm" : "yffczm";
                        } else {
                            // 根据勾选打印
                            yfwfmb = $.isEmptyObject(data) ? wffczmDymb : yffczmDymb;
                            dylx = $.isEmptyObject(data) ? "wffczm" : "yffczm";
                        }

                        var dataUrl = getIP() + "/realestate-inquiry-ui/rest/v1.0/print/zhcx/bzb/yfwfzm/" + key + "/xml";
                        printChoice(dylx, "realestate-inquiry-ui", dataUrl, yfwfmb, false, sessionKey);

                        // 记录打印有房无房数据库记录
                        var dataXml =  getXmlStr(dataUrl);
                        saveZhcxLogToDataBase(getSearchData(), dataXml, 1, "yfwfzm");
                        savePrintInfo(fczmDymb, dataUrl, {'zmbh': key, "printType": "有房无房证明", "qlr": qlrmc}, dataXml);
                    } else {
                        failMsg("打印失败，请重试！");
                    }
                },
        ...
        function getFcxx(qlrmc, qlrzjh) {
            ...
            $.ajax({
                url: "/realestate-inquiry-ui/rest/v1.0/zhcx/nantong/yfwfzm/zfxx?moduleCode=" + moduleCode,
                type: "POST",
                data: JSON.stringify(qlrxxList),
                contentType: 'application/json',
                async: false,
        ...
        function qszm(d, cols) {
            ...
            checkApplyRegistration(checkedData, ZHCX_QSZM_ZHGZ, 'QSZM').fail(function () {
                ...
                $.each(checkedData, function (key, value) {
                    ...
                    if(isNullOrEmpty(param.bdcdyh)){
                        param.bdcdyh = value.fwbh;
                    }
            ...
            layer.open({
                ...
                success: function (layero, index) {
                    ...
                    if (needsearch && searchData) {
                        ...
                        savezjh = searchData.qlrzjh;
                        saveqlr = searchData.qlrmc;
                        table.reload("pageTable", {
                            url: "/realestate-inquiry-ui/rest/v1.0/zszm"
                            , where: searchData
                            , page: {
        ...
        function zjhyz(obj) {
            var pattern1 = /[\u4e00-\u9fa5]+/g;
            var patt = /^[a-zA-Z]+$/;
            if (pattern1.test(obj.qlrzjh) || patt.test(obj.qlrzjh)){
                return true;
            }
            return false;
        }
        ...
        function getZhcxFjxx(pageData) {
            if (pageData && pageData.length > 0) {
                ...
                $.each(pageData, function (index, item) {
                    ...
                    var jzmj = isNullOrEmpty(item.zdzhmj) ? "" : item.zdzhmj;
    ...
    //获取页面与申请人关系
    window.getYsqrgxVal = function () {
        var ysqrgxVal = $('#ysqrgx').val();
        return ysqrgxVal;
    }
     ...
     * @returns {string|*}
     */
    function zdGhytToMc(zdname, zdDm, zdListName,zdzhyt) {
        try {
            if (zdDm) {
                var zdVals = eval(zdListName.fwyt);
                if (!zdVals) {
            ...
            return isNullOrEmpty(zdzhyt) ? "" : zdzhyt;
        } catch (e) {
            return isNullOrEmpty(zdzhyt) ? "" : zdzhyt;
        }
    }
     ...
     * @param data 新选择的数据
     */
    function checkAndContinueAddData(data) {
        // 1.先和当前已有的数据比较去重
        var flag = continueAddData(data);
        if (flag){
