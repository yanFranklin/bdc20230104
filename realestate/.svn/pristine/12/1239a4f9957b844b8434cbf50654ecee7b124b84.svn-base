...
var SpfClfJyxx = function () {
    ...
    function openPage(fwfclx) {
        var beanName = fwfclx === "clf" ? "yrb_zd_clfhqjg" : "yrb_htxx";
    ...
    function queryJyxx(fwfclx) {
        ...
        var beanName = fwfclx === "clf" ? "ntyth_clfwqxx_zh" : "yrb_htxx";
        var param = {
            htbh: $('#contractNo').val(),
            zjh: $('#mfzjh').val(),
            beanName: beanName
    ...
    function formatJyxx(data) {
        ...
        var zrfArray = [];
        var zcfArray = [];
        $.each(data.bdcSlSqr, function (index, sqr) {
            if (sqr.sqrlb == "1") {
        ...
        if(isNotBlank(data.jyxxMap) && isNotBlank(data.jyxxMap.cmf)){
            zcfArray.push(data.jyxxMap.cmf);
        }
        fcjyxx.push({
            zrf: zrfArray.join(","),
            zcf: zcfArray.join(","),
            zl: data.bdcSlJyxx.zl != null ? data.bdcSlJyxx.zl : "",
            htbh: data.bdcSlJyxx.htbh,
            htdjsj: data.bdcSlJyxx.htbasj,
            fcjyBaxxDTO: data
        });
        return fcjyxx;
    }
    ...
    function linkCheck(data, fwfclx) {
        ...
        //         var sqr = value.bdcSlSqrDO;
        //         if(sqr.sqrlb == 1 && sqr.sqrmc == msrxx.qlrmc && sqr.zjh == msrxx.zjh){
        //             checkFlag = true;
        //         }
        //     });
        //     if(!checkFlag){
    ...
    function checkHtbhLink(fcjyBaxx, fwfclx) {
        if (chooseBdcXm) {
            ...
            getReturnData("/ycsl/jyxx/checkHtbhLinked", {
                htbh: fcjyBaxx.bdcSlJyxx.htbh,
                fwyt: fwyt
            }, "GET", function (data) {
                removeModal();
                if (data) {
    ...
    function proxyDealDjxx(data, fwfclx, xmid) {
        if (fwfclx === "clf") {
            ...
            //统一处理附件信息
            dealFjxx(data);
            //成功带入信息后反馈状态给交易 1为已获取，2表示删件
            backJyxx(data.bdcSlJyxx.htbh, $("#mfzjh").val());
        } else {
            ...
            if (bdcslFwxx.xzzt == "0" && bdcslFwxx.dyzt == "0") {
                ...
                ityzl_SHOW_WARN_LAYER("存在限制或者抵押无法带入交易信息");
            }
        }
    }

    function backJyxx(htbh, zjh) {
        getReturnData("/ycsl/jyxx/backjyzt", {htbh: htbh, zjh: zjh}, "GET", function () {
            console.log("反馈交易状态结束");
        }, function (xhr) {
            delAjaxErrorMsg(xhr);
        })
    }
...
function importXmHtxx(htxx) {
    addModel();
    getReturnData("/ycsl/jyxx/checkHtbhImport", {
        htbh: htxx.fcjyBaxx.bdcSlJyxx.htbh,
