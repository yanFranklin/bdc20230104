 ...
 */
layui.extend({
    custom_tree: '../../static/ztree/custom_tree'
});
var custom_tree;
layui.use(['layer','custom_tree'],function () {
    ...
    //下拉面板宽高位置
    (function(){
        $('.org_select_tree').css({'width': '280px', 'height':'200px', 'left':60, 'top':40});
    })();
...
function loadMultiOrgTree(treeId, onCheckFn){
    layui.custom_tree.load({
        treeId: treeId,
        commonUrl: '/realestate-supervise/rest/v1.0/ztree/orgs',
 ...
 * @param onCheckFn 多选框选中事件方法
 */
function loadAsyncMultiOrgTree(treeId, onCheckFn){
    $.ajax({
        url:"/realestate-supervise/rest/v1.0/ztree/orgs/all",
        type: "POST",
        ...
        success: function (data) {
            ...
            layui.custom_tree.getZTreeObj().expandAll(true);
        },
...
function onCheck(e, treeId, treeNode) {
    var zTree = $.fn.zTree.getZTreeObj(treeId); //ztree对象
