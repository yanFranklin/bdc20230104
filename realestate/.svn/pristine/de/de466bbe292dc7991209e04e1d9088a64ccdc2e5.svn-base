...
layui.use(['form', 'jquery', 'element', 'table', 'laydate'], function () {
    ...
    table.render({
        ...
        cols: [[
            ...
            {field: 'principal', title: '用户名',width: 100, hide: true},
            {field: 'alias', title: '用户名',width: 100},
            {field: 'eventName', title: '日志类型',width: 130},
            {field: 'gzlslid', title: '工作流实例ID', width: 120, hide:true},
            {field: 'dbzt', title: "登簿状态", width: 100},
            {field: 'sfyws', title: '是否完税',width: 100, templet: function(d){
                    if(d.sfyws){
                        return "已完税";
                    }else{
                        return "";
            ...
            {field: 'access', title: '是否上报',width: 110, templet: function(d){
                    if(d.access){
                        return "已执行上报";
                    }else{
                        return "";
            ...
            {field: 'gzlslid', title: '工作流实例ID', width: 120},
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (index, item) {
                $('#gzldymc').append(new Option(item.name, item.name));// 下拉菜单里添加元素
            });
