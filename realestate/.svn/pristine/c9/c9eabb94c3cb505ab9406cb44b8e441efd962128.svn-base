function initXmTable() {
    layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
        ...
        var unitTableTitle = [{type: 'checkbox', fixed: 'left'},
            ...
            {field: 'xb', title: '小班', width: "5%", hide: gllq == false ? false : true},
            {field: 'zdzhmj', title: '宗地面积', width: "8%", hide: gllq == false ? false : true},
        ...
        form.on("submit(queryXm)", function (data) {
            ...
            if (isNotBlank(bdcslxztzpz.qllx)){
                var qllx =bdcslxztzpz.qllx.split(',');
                for (var i=0; i<qllx.length; i++) {
                    var index = lqlx.indexOf(qllx[i]);
                    if (isNotBlank(index) && index != -1){
                        cxObj.gllq = true;
                        gllq = true;
                        break;
                    }
