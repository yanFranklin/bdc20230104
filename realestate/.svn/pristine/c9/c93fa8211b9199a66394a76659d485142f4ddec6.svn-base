...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            // 获取查询条件
            var gmsfhm = $(this).parents('.layui-inline').prev().find('#gmsfhm').val();
            // 判空
            if (!isNotBlank(gmsfhm)) {
                warnMsg('请输入必要查询条件');
                return false;
            }
            // 组织查询条件
            ...
            searchData.cxywcs.push({"gmsfhm": gmsfhm});
        ...
        function search() {
            ...
            if(searchData.cxywcs &&searchData.cxywcs.length >0) {
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    // 获取查询参数
                    var param = {
                        gmsfhm : searchData.cxywcs[i].gmsfhm
                    };
                    // 重新请求
                    $.ajax({
                        url:"/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/zhrkkdsrrkxx",
                        type: "POST",
                            ...
                            if (data.code == 200) {
                                ...
                                if(data.data  && data.data.length>0) {
                                    for(var i=0;i<data.data.length;i++){
                                        data.data[i].gmsfhm = param.gmsfhm;
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijzhrkkdsrrkxx","综合法人库-低收入人口信息查询",key,"市级共享交换平台");
        });
    });
});
