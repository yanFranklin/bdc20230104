...
setTimeout(function () {
    if(gzlslid) {
        queryParam.gzlslid = gzlslid;
        $.ajax({
            url: "/realestate-inquiry-ui/dtcx/get/gglcckym",
            type: "get",
            success: function (data) {
                if(!data || isNullOrEmpty(data.cxid)) {
                    warnMsg("未配置公告流程台账自定义查询，请联系管理员");
                } else {
...
function sjcl(data) {
    ...
    addModel();
    // 打开项目信息
    if (isNotBlank(data) && isNotBlank(data.data.GZLSLID)) {
        //打开的是项目列表资源
...
function edit(data) {
    layer.open({
        ...
        shadeClose: true,
        isOutAnim: false,
        content: "../../changzhou/ggtzlr/edit.html?gzlslid="+data.data.GZLSLID,
        area: ['960px', '500px'],
        title: '公告信息录入',
    });
}
 ...
 */
function addGg() {
    var gglxZdList = getSlMulZdList("slgglx");
    var gglxHtml = gglxSelectOpt(gglxZdList);
...
function ts(ggxx) {
    ...
    var gzlslid = ggxx.data.GZLSLID;
    if (isNotBlank(qzbs)) {
        removeModal();
        ityzl_SHOW_WARN_LAYER("已推送签章");
    } else {
        getReturnData("/rest/v1.0/ggxx/tsqz", {gzlslid: gzlslid}, "GET", function (data) {
            removeModal();
