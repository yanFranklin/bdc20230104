...
layui.use(['jquery', 'table', 'form', 'layer'], function () {
    ...
    $(function () {
        ...
        var zl = $.getUrlParam('zl');
        if(zl){
            $('input[name="zl"]').val(zl);
        }
        ...
        var col = [
            {
                field: 'bdcdyh', title: '不动产单元号', width: "35%", templet: function (obj) {
                    if (!isNullOrEmpty(obj.bdcdyh)) {
                        ...
                        return '<span style="color:#1d87d1; cursor:pointer" title="' + obj.bdcdyh + '" onclick="selectBdcdyh(' + "'" + obj.bdcdyh + "'" + ','+"'"+qjgldm+"','"+qsxz+"'"+')">' + formatBdcdyh(obj.bdcdyh) + '</span>'
                    } else {
                        return '<span></span>';
                    }
                }
 ...
 */
function selectBdcdyh(bdcdyh,qjgldm,yqllx) {
    addModel();
    // yxmid 值存在，属于复制功能
    if (!isNullOrEmpty(yxmid)) {
        ...
        if (cqxzbdcdyh === 'cq') {
            cqXzBdcdyhYz(bdcdyh,qjgldm,yqllx);
        } else {
            xzBdcdyhReload(bdcdyh,qjgldm,yqllx);
        }
    }
}
 ...
 * @param bdcdyh
 */
function cqXzBdcdyhYz(bdcdyh,qjgldm,yqllx) {
    addModel();
    var selectDataList = [];
    var bdcGzYzsjDTO = {};
...
//选择不动产单元后刷新父页面
function xzBdcdyhReload(bdcdyh,qjgldm,yqllx){
