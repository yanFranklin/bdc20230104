...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            // 获取查询条件
            var jdcsyrgmsfhm = $(this).parents('.layui-form-item').find('#jdcsyrgmsfhm').val();
            // 判空
            if (!isNotBlank(jdcsyrgmsfhm)) {
                warnMsg('请输入必要查询条件');
                return false;
            }
            // 组织查询条件
            ...
            searchData.cxywcs.push({"jdcsyrgmsfhm": jdcsyrgmsfhm});
                    ...
                    if(data && data.length>0){
                        ...
                        for (var i = 0; i < data.length; i++) {
                            searchData.cxywcs.push({
                                "jdcsyrgmsfhm":data[i].zjh
                            });
                            if (data[i].dlrzjh){
                                searchData.cxywcs.push({
                                    "jdcsyrgmsfhm":data[i].dlrzjh
                                });
                            }
                        }
                        // 展示查询条件
        ...
        function search() {
            ...
            if(searchData.cxywcs &&searchData.cxywcs.length >0) {
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    // 获取查询参数
                    var param = {
                        jdcsyrgmsfhm : searchData.cxywcs[i].jdcsyrgmsfhm
                    };
                    // 重新请求
                    $.ajax({
                        url:"/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/zhrkkrkjbxx",
                        type: "POST",
                            ...
                            if (data.code == 200) {
                                ...
                                if(data.data  && data.data.length>0) {
                                    for(var i=0;i<data.data.length;i++){
                                        data.data[i].jdcsyrgmsfhm = param.jdcsyrgmsfhm;
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijzhrkkrkjbxx","盐城市工业和信息化局-人口基本信息",key,"市级共享交换平台");
        });
    });
});
