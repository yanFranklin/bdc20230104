...
function renderWaitTable(url, tableId, currentId, toolbar) {
    ...
    if(ywlbXmly) {
        tableTitle = [
            {type: 'checkbox', width: 50, fixed: 'left'},
            {field: 'sply', title: '项目来源', minWidth: 150, templet: function(d){
                        return getXmlymc(d);
                }
            },
            {field: '', title: '流程状态', templet: '#stateTpl', minWidth: 90},
    ...
    layui.table.render({
        ...
        parseData: function (res) { //res 即为原始返回的数据
            ...
            if (isNotBlank(res) && res.numberOfElements === 1 && flag === true && jtslbh === true){
                var obj = {data: res.content[0]};
                openWindow(obj);
                flag = false;
            }
            return {
...
function renderRlTable(tableId, currentId, toolbar) {
    var tableTitle = [
        ...
        {field: 'sqbmmc', minWidth: 150, title: '外网申请部门'},
        {field: 'qlr', minWidth: 100, title: '权利人'},
        {field: 'ywr', minWidth: 100, title: '义务人'},
        {field: 'zl', minWidth: 200, title: '坐落'},
