...
layui.use(['jquery','formSelects','form','response'],function () {
    ...
    formSelects.render('selectSqbmAdd', {
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data){
                    ...
                    formSelects.data('selectSqbmAdd','local',{arr:data.sqbm});
                    formSelects.value('selectSqbmAdd', [String(datas.sqbmMc)]);
                    formSelects.data('selectSfbcAdd','local',{arr:data.sf});
    ...
    function loadXh() {
        var obj ={};
        obj.djxl =djxl;
        getReturnData("/rest/v1.0/lcpz/querySjclPzMaxSxh",JSON.stringify(obj),"POST",function (data) {
            if(data ||data ===0){
