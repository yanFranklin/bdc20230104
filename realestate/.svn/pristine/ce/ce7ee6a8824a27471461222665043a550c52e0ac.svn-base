...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
    ...
    $(function () {
        ...
        // 监听分屏验证
        $(document).on('click','#fpyz',function () {
            var xmid = $('#tableUi li:nth-child(2)').data('xmid');
            showFpyz(xmid);
        });
    });
...
function generateTabContent() {
            ...
            if (isNotBlank(data)) {
                ...
                getReturnData("/slym/ql/pllc", {processInsId: processInsId, sfcxql: false}, 'GET', function (data) {
                    if (isNotBlank(data)) {
                        ...
                        //存放不动产单元号
                        if (isNotBlank(qlxx)) {
                            var bdcXmDOList = qlxx.bdcXmDOList;
                            if (isNotBlank(bdcXmDOList)) {
                                for (var i = 0; i < bdcXmDOList.length; i++) {
                                    var bdcdyh = bdcXmDOList[i].bdcdyh;
    ...
    layui.use(['form', 'jquery', 'laytpl', 'element', 'laydate'], function () {
        ...
        if(isNotBlank(qlxx) && isNotBlank(qlxx.bdcXmFbDO) && qlxx.bdcXmFbDO.sfsftg == 1){
            // 添加税费托管业务展示说明标签
            $("#sfsftg-tips").show();
        }
    });
...
function getQlCols() {
    ...
    if (qllx === 4 || qllx === 6 || qllx === 8) {
        ...
        cols = [
            ...
            {field: 'yxtcqzh', title: 'TT系统证号', minWidth: 250, hide: zxlc == "true" ? false : true},
            {field: 'bdcdyh', title: '不动产单元号', minWidth: 270, templet: '#bdcdyhTpl'},
            {field: 'ysfwbm', title: '预售房屋编码', minWidth: 140},
            {field: 'zl', title: '坐落', minWidth: 240},
            {field: 'dzwmj', title: '定着物面积(㎡)', minWidth: 240},
...
function setSplyBtnAttr(){
    ...
    if(sply ===3 || sply ===5 || sply ===11){
        ...
        // 税费托管流程
        if(sfsftg == 1){
            $('.sslc').css({'color':'#2d5de1','pointer-events':'auto'});
        }
    }
    //互联网涉税
    if(sply ===5 || sply ===11){
