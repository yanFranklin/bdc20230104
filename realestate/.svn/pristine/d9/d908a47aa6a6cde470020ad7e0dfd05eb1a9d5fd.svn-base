...
public class BdcForwardController extends BaseController {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(BdcForwardController.class);
    @Autowired
    ...
    @LogNote(value = "转发流程任务", action = LogConstans.LOG_TYPE_LCZF, custom = LogConstans.LOG_TYPE_LCZF)
    @ResponseStatus(HttpStatus.OK)
    public void forwardTask(ForwardTaskDto forwardTaskDto) {
        ...
        LOGGER.info("转发任务complete：{}",JSONObject.toJSONString(forwardTaskDto));
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "forwardTaskDto", value = "转发流程任务", required = true, dataType = "ForwardTaskDto")})
    @LogNote(value = "转发流程并行任务", action = LogConstans.LOG_TYPE_LCZF, custom = LogConstans.LOG_TYPE_LCZF)
    ...
    @ResponseStatus(HttpStatus.OK)
    @LogNote(value = "获取转发的用户", action = LogConstans.LOG_TYPE_LCZF, custom = LogConstans.LOG_TYPE_LCZF)
