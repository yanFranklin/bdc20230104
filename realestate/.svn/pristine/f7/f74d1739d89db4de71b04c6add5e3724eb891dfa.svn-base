    ...
    try {
        ...
        if (objTest.ReadCard()) {
            layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
                ...
                $(forms).find("input[name='xm']").val(trimStr(pName));
 ...
 * @param element
 */
function appendReadIDCardNewQlrAndZjh(element) {
    try {
        var objTest = readIdCard();
        //  var objTest = {"Name":"李四", "ID":"13212131233"};
        if (objTest.ReadCard()) {
            layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
                var pName = objTest.Name;
                var pCardNo = objTest.ID;
                ...
                ["qlr", "qlrmc", "singleQlr"].forEach(function (item) {
                    var qlrxx = $(forms).find("input[name='" + item + "']").val();
                    if(!isNullOrEmpty(qlrxx)) {
                        $(forms).find("input[name='" + item + "']").val(qlrxx + "," + trimStr(pName));
                    } else {
                        $(forms).find("input[name='" + item + "']").val(trimStr(pName));
                    }
                });

                ["qlrzjh", "zjh", "singleZjh"].forEach(function (item) {
                    var zjhxx = $(forms).find("input[name='" + item + "']").val();
                    if(!isNullOrEmpty(zjhxx)) {
                        $(forms).find("input[name='" + item + "']").val(zjhxx + "," + trimStr(pCardNo));
                    } else {
                        $(forms).find("input[name='" + item + "']").val(trimStr(pCardNo));
                    }
                });
...
function ReadIDCardNewJfrAndZjh(element) {
    try {
        var objTest = readIdCard();
        if (objTest.ReadCard()) {
            layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
                ...
                var pAddress = objTest.Address;
                var forms = element.parentNode.parentNode.parentNode;
                $(forms).find("input[name='jfr']").val(trimStr(pName));
                $(forms).find("input[name='jfrmc']").val(trimStr(pName));
                $(forms).find("input[name='jfrxm']").val(trimStr(pName));

                $(forms).find("input[name='jfrzjh']").val(trimStr(pCardNo));
