...
public class ISO7064Util {
     ...
     * 校验字符系统验证器
     */
    public static class CheckCharacterSystemValidator {
         ...
         * 纯系统校验
         */
        static boolean validatePureSystem(String inputString, Designation designation) {
            int M = 0; // 模数
            int r = 0; // 基数
         ...
         * 混合系统校验
         */
        static boolean validateHybridSystem(String inputString, Designation designation) {
            int M = 0; // 模数1
            List<String> mapping = null;
    ...
    private static class CheckCharacterComputor {
         ...
         * 通过递归法法计算混合系统一位校验字符
         * 参照开发文档地址如下
         * https://blog.csdn.net/liyongzhi1992/article/details/82426265
         * https://doc.mbalib.com/view/57c8589e38a37dceca2bf220586d6cd2.html
         */
        static String recursiveMethod4HybridSystemWith1CheckChar(
            ...
            if (mapping != null) {
                for (int i = n; i >= 2; i--) {
                    ...
                    //System.out.println("乘积：" + P + "    下一符值：" + mapping.indexOf(String.valueOf(strArray[n - i])) + "   中间和：" + S + "  结果：" + ((S % M == 0 ? M : S % M) * 2) % Mplus1);
                    P = ((S % M == 0 ? M : S % M) * 2) % Mplus1;
                }
                /*System.out.println("最后M：" + M);
                System.out.println("最后P：" + P);
                System.out.println("P % M：" + P % M);
                System.out.println("P % M：" + P % M);
                System.out.println("M + 1 - P % M：" + (M + 1 - P % M));
                System.out.println("((M + 1 - P % M) % M) - 1：" + (((M + 1 - P % M) % M) - 1));*/

                //说白了最后一步计算校验码就是P+校验码除以M余数是1
                return mapping.get((M + 1 - P % M) % M);


                //return mapping.get(((M + 1 - P % M) % M) - 1);
