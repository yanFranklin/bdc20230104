 ...
 * describe: 新增海门版本，不动产单元登记历史关系处理JS
 */
layui.use(['table', 'laytpl', 'laydate', 'layer', 'form'], function () {
    ...
    table.render({
        ...
        where: {"bdcdyh": bdcdyh, "qszt": 1, "sfcxls": '2'},
        request: {
        ...
        cols: [[
            ...
            {minWidth: 230, sort: false, field: 'cfwh', title: '查封文号'},
            {width: 270, sort: false, field: 'bdcqzh', title: '不动产权证号'},
            {width: 120, sort: false, field: 'qlr', title: '权利人'},
            {width: 100, sort: false, field: 'bdbzzqse', title: '被担保主债权数额'},
    ...
    table.on('tool(lsgxTable)', function (obj) {
        ...
        if (obj.event === 'process') {
            ...
            var configInfo = getConfigInfo(data.xmid);

            if(!configInfo || isNullOrEmpty(configInfo.hmDadyUrl)) {
                failMsg("未配置档案调用地址，请联系管理员!");
                return;
            }
            window.open(configInfo.hmDadyUrl + data.slbh);
        }
    });
