            ...
            success: function (data) {
                ...
                if (resourceData
                    ...
                    if (resourceData.info.ljzTitle
                        ...
                        $("#ljzTitle").html("坐落:<span>" + titleVal + "</span>");
                    }
                    // 盐城展示顶部信息栏，隐藏页面坐落、备注
                    if (showljzxx && showljzxx == '1') {
                        showLcTitle(resourceData.info);
                        // 调整备注高度
                        // chandeBzHeight();
                    }
                    if (defultShowLjzxx && defultShowLjzxx == '0') {
                        $('.bdc-operate-show').click();
                    }
...
function showLcTitle(data) {
    ...
    if (data.lhdyqlzt && data.lhdyqlzt.value>0){
        $(".bdc-lc-title #lhdyqlzt").show();
    }
    if (data.sfxxmzt && data.sfxxmzt.value == 1 ){
        $(".bdc-lc-title #sfxxmzt").show();
    }
