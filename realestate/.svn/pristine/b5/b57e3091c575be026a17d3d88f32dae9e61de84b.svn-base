...
layui.use(['jquery', 'table', 'layer'], function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        ...
                        // 判断是否锁定证书
                        $.ajax({
                            url: getContextPath() + '/rest/v1.0/blxx/bllc/sdyy?gzlslid='+ obj.gzlslid,
                            dataType: 'json',
                                ...
                                if (res === true) {
                                    ...
                                    var logoutIndex = layer.open({
                                        ...
                                        , yes: function (index, layero) {
                                            var reason = $("#sdreason").val();
                                            if (isNullOrEmpty(reason)) {
                                                layer.msg('请输入锁定原因!');
                                                return false;
                                            }
                                            bdcBlxxDTO["sdyy"] = reason;
    ...
    function csh(bdcBlxxDTO) {
        ...
        $.ajax({
            type: "POST",
            url: getContextPath() + "/rest/v1.0/blxx/bllc/modify",
            contentType: "application/json;charset=utf-8",
    ...
    function renderblTable(url, tableId, toolbar) {
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: 'qszt', title: '权属状态', templet: '#qsztTemplate', minWidth: 90},
                {field: 'gzlslid', title: '流程ID', width: 90, hide: true},
                {field: 'xzzt', title: '限制状态', templet: '#xzqlZtTemplate', minWidth: 90},
            ]],
