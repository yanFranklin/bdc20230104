...
layui.use(['form', 'jquery', 'element', 'table'], function () {
    ...
    function initSjzsTab(queryData, showEchart) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                data= [{"lxmc":"交接单类型","sl":"904"},{"lxmc":"交接单类型2","sl":"40",},{"lxmc":"交接单类型3","sl":"200"},{"lxmc":"交接单类型4","sl":"977"},{"lxmc":"交接单类型5","sl":"51"},{"lxmc":"交接单类型6","sl":"700"},{"lxmc":"交接单类型7","sl":"500"}]
                if(data){
                    //此处填写对于接口返回的数据处理
    ...
    function changeChart() {
        ...
        var autoHeight = 0;
        if (yData.length > 2) {
            autoHeight = yData.length* setHeight*2;//此处几组数据就取值几
            if(autoHeight < 180) {
                autoHeight = 180;
            }
            $('#chartdemo').height(autoHeight);
        }
        //设置柱状图x轴的长度 这里如果返回的结果集里单个字符长度大于14需要调整宽度
        yData.forEach(function(item){
            if(item.length>14){
                $('#chartdemo').width(1000);
                return ;
            }else{
                $('#chartdemo').width(600);
                return ;
...
function exportExcel(d, obj){
    var cols = obj.cols[0];
    var checkedData = layui.sessionData('checkedDatbdcdyLista');
