 ...
 * @description 从原项目加载数据到建设用地使用权 测试
 */
...
public class InitYxmToBdcJsydsyqServiceImplTest {
    @Autowired
    private InitYxmToBdcJsydsyqServiceImpl initYxmToBdcJsydsyqService;

    @Transactional(transactionManager = "transactionManager")
    @Test
    @Rollback
    @DatabaseSetup(value = "/data/InitYxmToBdcJsydsyqSetup-data.xml")
    public void initQlxx() throws Exception {
        ...
        BdcJsydsyqDO bdcJsydsyqDO = (BdcJsydsyqDO) initYxmToBdcJsydsyqService.initQlxx(initServiceQO);
        ...
        Assert.assertEquals("532.78", bdcJsydsyqDO.getQdjg().toString());
        Assert.assertEquals(sdf.parse("2018-11-13 01:45:54"), bdcJsydsyqDO.getSyqqssj());
        Assert.assertEquals(sdf.parse("2088-11-13 01:45:54"), bdcJsydsyqDO.getSyqjssj());
        Assert.assertEquals("52.66", bdcJsydsyqDO.getSyqmj().toString());
        Assert.assertEquals(sdf.parse("2018-11-14 01:45:54"), bdcJsydsyqDO.getSyqqssj2());
        Assert.assertEquals(sdf.parse("2088-11-14 01:45:54"), bdcJsydsyqDO.getSyqjssj2());
        Assert.assertEquals(sdf.parse("2018-11-15 01:45:54"), bdcJsydsyqDO.getSyqqssj3());
        Assert.assertEquals(sdf.parse("2088-11-15 01:45:54"), bdcJsydsyqDO.getSyqjssj3());
        Assert.assertEquals("342.33", bdcJsydsyqDO.getDytdmj().toString());
        Assert.assertEquals("568.6", bdcJsydsyqDO.getFttdmj().toString());
        Assert.assertEquals("875.44", bdcJsydsyqDO.getZrjg().toString());
        Assert.assertEquals("9", bdcJsydsyqDO.getQlsdfs().toString());
        Assert.assertEquals("54", bdcJsydsyqDO.getQlxz().toString());
    }
