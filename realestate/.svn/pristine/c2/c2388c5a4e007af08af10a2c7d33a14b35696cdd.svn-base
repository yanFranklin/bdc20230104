...
import cn.gtmap.realestate.certificate.core.model.encrypt.SM2;
import cn.gtmap.realestate.certificate.core.model.encrypt.SM2KeyPair;
import cn.gtmap.realestate.certificate.core.model.encrypt.Signature;
...
public class SM2Util {
    ...
    public static String encrypt(String inputStr, String publicKeyStr){
        ...
        if (StringUtils.isNotBlank(inputStr) && StringUtils.isNotBlank(publicKeyStr)) {
            ECPoint publicKey = SM2.curve.decodePoint(Base64Util.decodeBase64StrToByte(publicKeyStr));
            byte[] data = SM_2.encrypt(inputStr, publicKey);
            result = Base64Util.encodeByteToBase64Str(data);
        }
        return result;
    }
    ...
    public static String decrypt(String encryptData, String privateKeyStr){
        ...
        if (StringUtils.isNotBlank(encryptData) && StringUtils.isNotBlank(privateKeyStr)) {
            BigInteger privateKey = new BigInteger(Base64Util.decodeBase64StrToByte(privateKeyStr));
            if (null != privateKey) {
                result = SM_2.decrypt(Base64Util.decodeBase64StrToByte(encryptData), privateKey);
            }
        }
     ...
     * @return
     */
    public static boolean SM2Verify(String M, String signature, String IDA, String publicKeyStr){
        if (StringUtils.isNotBlank(IDA) && StringUtils.isNotBlank(publicKeyStr)) {
            byte[] pubArr = DataUtil.hexStringToBytes(publicKeyStr);
            if (null != pubArr) {
                ECPoint publicKey = SM2.curve.decodePoint(pubArr);
                Signature s = Signature.getDecode(Base64Util.decodeBase64StrToByte(signature));
                if (null != s) {
                    return SM_2.verify(M, s, IDA, publicKey);
                }
            }
        }
        return false;
