...
function renderformSelects(data) {
    ...
    // 渲染字典表下拉
    formSelects.data('cxmdhyt', 'local', {arr: [{MC: "不动产构成利害关系", DM: 1}, {MC: "诉讼", DM: 2}, {MC: "仲裁", DM: 3}]});
...
function saveCdxx(data) {
    ...
    cdxxObj.bdcdyh = trimAllStr(cdxxObj.bdcdyh);
...
function getQueryParam() {
    ...
    param["xcxr"] = $("#xcxr").val();
    param["xcxrzjh"] = $("#xcxrzjh").val();
    param["bdcdyh"] = $("#bdcdyh").val();
    param["qlrzjh"] = '';
    param["qlrmc"] = '';
    param["qlrzjzl"] = '';
    var zjh = "";
...
function jumpQuery(dylx) {
    ...
    if (dylx == "zlhgxcdjg" || dylx == "lhgxcdjg") {
        ...
        if (isNullOrEmpty(param.zl) && isNullOrEmpty(param.bdcqzh) && isNullOrEmpty(param.xcxr) && isNullOrEmpty(param.xcxrzjh) && isNullOrEmpty(param.bdcdyh)) {
            ityzl_SHOW_WARN_LAYER("缺失查询参数，请检查");
        } else {
            url = url + "&zl=" + param.zl + "&bdcqzh=" + param.bdcqzh + "&qlrmc=" + param.xcxr + "&qlrzjh=" + param.xcxrzjh + "&bdcdyh=" + trimAllStr(param.bdcdyh);
            window.open(url);
        }
    } else {
