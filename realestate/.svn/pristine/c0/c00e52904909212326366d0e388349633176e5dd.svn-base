...
layui.use(['form', 'jquery', 'laydate', 'element', 'layer'], function () {
    ...
    $(function () {
        ...
        form.on("submit(save)", function (data) {
            ...
            setTimeout(function () {
                try {
                    $.when(saveXmxx(), saveXmfbpl(), saveQlxx()).done(function () {
                        removeModal();
...
//权利信息保存
function saveQlxx() {
    var xmids = sessionStorage.getItem('plxg_cqxmids');
    if (isNotBlank(tableName)) {
        var qlData = {};
...
function saveXmfbpl() {
    var xmids = sessionStorage.getItem('plxg_cqxmids');
    var bdcXmData = {};
    var bdcdyxxArray = $(".xmfb").serializeArray();
    bdcdyxxArray.forEach(function (item, index) {
        var name = item.name;
        if (isNotBlank(item.value)) {
            bdcXmData[name] = item.value;
