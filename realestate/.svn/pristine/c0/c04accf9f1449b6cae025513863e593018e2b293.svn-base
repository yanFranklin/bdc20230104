...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
    ...
    form.on('radio(zstdcrj)', function (data) {
        ...
        if(data.value =="1"){
            ...
            $zspmTab.find('select').find("option:eq(0)").attr("selected","selected");
            form.render("select");
            resetSelectDisabledCss();
        }
    });
    ...
    $(function () {
        ...
        $(document).on('click', '.getJtcyxx', function () {
            ...
            getReturnData("/slym/jtcy/hefei/getJtcyxx",{sply:sply,qlrmc:qlrmc,qlrzjh:qlrzjh,sqrid:sqrid,slbh:slbh,xmid:xmid,hybeanName:"acceptHbHyxx"},"GET",function (data) {
                if(sply == 5 || sply == 11){
                    jtcyList = data;
        ...
        $(document).on('click', '.tsSwxx', function () {
            var id =$(this)[0].id;
            var tsswZhbs = $(this).data('zhbs');
            if(isNullOrEmpty(tsswZhbs)){
                tsswZhbs ="tsSwxx";
            }
            var beanName =id;
            if(id ==="tsClfSwxx"){
            ...
            } else if(id === "tsClfSwxx_dk_pl"){
                // 存量房 批量推送税务(德宽)
                beanName="tsSwxxClf_dk_pl";
            }else if(id === "tsSpfSwxx_dk_pl"){
                // 商品房 批量推送税务(德宽)
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (beanName.indexOf("pl") == -1) {
                            ...
                            // 批量推送税务
                            if (data && data.swxxResponseDTOS.length > 0) {
                                var responseList = data.swxxResponseDTOS;
                                var successNum = 0;
                                var failNum = 0;
                                var failMsg = [];
                                for (var i = 0; i < responseList.length; i++) {
                                    if (responseList[i].responseMsg.indexOf("成功") > -1) {
                                        successNum = successNum + 1;
                                    } else {
                                        failNum = failNum + 1;
                                        failMsg.push(responseList[i].responseMsg);
                                    }
                                }
                                // 有失败的
                                if (failNum > 0) {
                                    showAlertDialog("推送失败，失败原因:" + failMsg.toString());
                                } else {
                                    ityzl_SHOW_SUCCESS_LAYER("推送次数:" + successNum + "次,推送成功");
                                }
                            } else {
                                showAlertDialog("推送失败,无返回值");
                            }
                        }
                        loadTsJsSwsj();
                    }, error: function (xhr, status, error) {
        ...
        // 批量查询税务信息
        $(document).on('click', '#querySwxxDk_pl', function () {
            addModel();
            $.ajax({
                url: getContextPath() + "/slym/sw/getPlSwxx",
                type: 'GET',
                dataType: 'json',
                data: {slbh: slbh,beanName: 'swCxxx_dk'},
                success: function (data) {
                    removeModal();
                    if (data && data.responseCode === "200") {
                        if (data.jbrxx != null) {
        ...
        //批量取消作废税务信息
        $(document).on('click', '#qxzfSwxxDk_pl', function () {
            //调用规则验证
            var bdcGzYzQO ={};
            bdcGzYzQO.zhbs = "qxzfSwxx";
            var gzyzParamMap ={};
            ...
            gzyzCommon(2,bdcGzYzQO,function (data) {
                ...
                var submitIndex = layer.open({
                    ...
                    yes: function () {
                        ...
                        $.ajax({
                            url: getContextPath() + "/slym/sw/plQxzfSwxx",
                            type: 'GET',
                            dataType: 'json',
                            data: {slbh: slbh, system:'dk'},
                            success: function (data) {
                                removeModal();
                                if (data && data.responseCode === "200") {
                                    loadYcslxx();
        ...
        //删除征收项目
        $(document).on('click', '.bdc-delete-zspm', function () {
            var tdcrjid =$(this).parents(".bdc-zspm").find("[name=tdcrjid]").val();
            deleteTdcrj(tdcrjid,$(this).parents(".bdc-zspm"))
...
//打开家庭成员页面
function openJtcyYm() {
    var url = getContextPath() + "/hefei/ycsl/jtcy.html";
    layer.open({
        title: '家庭成员信息',
        type: 2,
        area: ['960px'],
...
//新增征收品目
function addZspm(){
    var getTpl = addZspmTpl.innerHTML;
    laytpl(getTpl).render({zd:zdList}, function(html){
        $('.bdc-tdcrj').append(html);
    });
    form.render('select');
...
//删除土地出让金
function deleteTdcrj(tdcrjid,$zspm){
    var url = getContextPath() + "/rest/v1.0/tdcrj?tdcrjid="+tdcrjid;
    var deleteIndex = layer.open({
        type: 1,
        skin: 'bdc-small-tips',
        title: '确认删除',
