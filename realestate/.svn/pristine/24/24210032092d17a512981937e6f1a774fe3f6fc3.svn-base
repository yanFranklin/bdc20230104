...
$(document).ready(function () {
                ...
                if (data && data.length > 0) {
                    ...
                    getZd(function (data) {
                        ...
                        $.each(data.fwyt, function (index, item) {
                            $('.dzwyt').append('<option value="' + item.DM + '">' + item.MC + '</option>');
                        });
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data) {
                                $(".dzwyt").val(data.dzwyt);
                                // 如果用途是其他，显示用途名称
                                if (data.dzwyt == 80){
                                    $('input[name = "dzwytmc"]').val(data.dzwytmc);
                                }else{
                                    // 隐藏用途名称那一列
                                    $(".dzwytmc").css("display","none");
                                    $("#ytmc").css("display","none")
                                }
    ...
    //用于select联动，共有信息定着物用途目前仅一种用途
    form.on('select(dzwyt)', function (data) {
        $('.dzwyt').val(data.value);
        // 80是其他，选择其他时，显示用途名称
        if (data.value==="80"){
            // 显示用途名称那一列
            $(".dzwytmc").css("display","");
    ...
    function syncUpdateXmxx(data, gyxxData){
        if(isNotBlank(xmid) && gyxxData.length > 0){
            ...
            // 如果不是其他，保存用途字典项的值
            if (data.dzwyt != "80") {
                data.dzwytmc = $(".layui-select-title input").val();
            }else{
                 // 如果是其他，保存用途名称,取第一行的
                var dzwytmc = $('input[name = "dzwytmc"]');
                data.dzwytmc = dzwytmc[0].value;
            }
