...
//根据权利人类型判断必填项
function sfczrbyqlrlx() {
    var qlrlb = $("#qlrlb").val();
    if (qlrlb === "1") {
        var sfczr = false;
        $.ajax({
            url: getContextPath() + "/slym/qlr/sfczr",
            async : false,
            type: 'GET',
            dataType: 'json',
            data: {qlrid: qlrid,xmid:xmid,qllx:qllx},
            success: function (data) {
                sfczr = data;
            }
        });
        if (isNullOrEmpty(sfczr)) {
            $("input[name='sfczr'][value='0']").attr("checked",true);
            $("input[name='sfczr'][value='1']").attr("checked",false);
        }else if(sfczr){
            $("input[name='sfczr'][value='1']").attr("checked",true);
            $("input[name='sfczr'][value='0']").attr("checked",false);
        }else {
            $("input[name='sfczr'][value='0']").attr("checked",true);
            $("input[name='sfczr'][value='1']").attr("checked",false);
        }
