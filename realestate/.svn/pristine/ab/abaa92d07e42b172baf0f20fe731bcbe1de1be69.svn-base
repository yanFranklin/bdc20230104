...
function generateQlxx(bdcSlQlxxymDTOList) {
    if (isNotBlank(bdcSlQlxxymDTOList)) {
        for (var i = 0; i < bdcSlQlxxymDTOList.length; i++) {
            ...
            djyyforywr = bdcSlQlxxym.bdcXm.djyy;
        ...
        for (var j = 0; j < bdcSlQlxxymDTOList.length; j++) {
            ...
            if(sply && splyList && splyList.indexOf(sply) > -1){
                $(".sqr"+(j+1)).prop("disabled","off");
                $(".sqr"+(j+1)).find(":input").prop("disabled","off");
                $(".sqr"+(j+1)).find(":button[name='qlrxx']").removeAttr("disabled");
                $(".sqr"+(j+1)).find(":button[name='ywrxx']").removeAttr("disabled");
                $(".sqr"+(j+1)).find(":button[name='addSqr']").removeAttr("disabled");
                $(".sqr"+(j+1)).find(":button[name='qlrdelete']").hide();
                $(".sqr"+(j+1)).find(":button[name='addSqr']").hide();
                form.render("select");
                disabledAddFa();
            }
        }
