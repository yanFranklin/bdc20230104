 ...
 * @param sessionKey 打印配置session
 */
function batchZmdPrint(dylx,modelUrl, zsidList, sessionKey) {
    $.ajax({
        url: "/realestate-register-ui/rest/v1.0/print/batchzsprintsavetoredis",
        type: "POST",
        data: JSON.stringify({"listZsidsStr":zsidList}),
        ...
        success: function (key) {
            if (key) {
                var dataUrl = getIP() + "/realestate-register-ui/rest/v1.0/print/batchZmdDy/" + key;
                var appName = "realestate-register-ui";
...
/**
 * 校验领证方式
 * @param zsidArr,gzlslid
 */
function jylzfs(zsidArr,gzlslid) {
    var jyjg = true;
    // 获取当前流程证书领证方式
    $.ajax({
        url: "/realestate-register-ui/rest/v1.0/zs/jylzfs?gzlslid=" + gzlslid,
        type: "POST",
        data: JSON.stringify(zsidArr),
        async: false,
        contentType: 'application/json',
