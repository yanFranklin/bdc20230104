...
layui.use(['table', 'laytpl', 'laydate', 'layer', 'form'], function () {
    ...
    table.render({
        ...
        cols: [[
            ...
            {field: 'sjlxMc', width: 150, title: '收件类型'},
            {field: 'sqbmMc', width: 150,title: '收件部门'},
    ...
    $('#search').on('click', function () {
        // 获取查询内容
        var clmc=$('#clmc').val();
        var obj = {'sjlx':'','clmc':replaceBracket(clmc),'djxl':djxl};
    ...
    function addBdcSjclpz(djxl) {
        ...
        layer.open({
            ...
            type: 2,
            area: ['960px', '300px'],
            content: ["sjclpz/addOrEditBdcSjcl.html?action=add&djxl="+djxl, 'yes']
            , end: function () {
                removeModal();
                table.reload('sjclpzTable');
            }
    ...
    function editBdcSjclpz(data) {
        ...
        layer.open({
            ...
            area: ['960px', '300px'],
            offset: 'auto',
            content: ["sjclpz/addOrEditBdcSjcl.html?action=edit", 'yes'],
