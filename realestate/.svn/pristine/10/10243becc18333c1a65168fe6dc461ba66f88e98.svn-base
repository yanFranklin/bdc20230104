...
var Toolbar = function () {
  ...
  _createClass(Toolbar, [{
    ...
    value: function _bindListeners() {
      ...
      items.print2.addEventListener('click', function () {
        eventBus.dispatch('print2');
        // adobe
        var ii = layer.load();
        setTimeout(function(){
          layer.close(ii);
        ...
        try {
          var queryString = document.location.search.substring(1);
          var params = (0, _ui_utils.parseQueryString)(queryString);
          var adobeurl = 'file' in params ? params.file : "";
          console.log("打印PDF地址" + adobeurl);

          if(adobeurl && adobeurl.indexOf('/office/download') != -1) {
            ...
            //   读取缓存到磁盘PDF文件
            window.open(result + '/office/download?fileName=' + localFileName);
          }
        } catch (e) {
          console.error(e);
           alert("仅支持IE，并请确保已安装Adobe，开启Adobe加载项！");
        }
      });
