...
function search() {
    var xm = $("#xm").val();
    var cxrzjhm = $("#cxrzjhm").val();

    if (isNullOrEmpty(xm) || isNullOrEmpty(cxrzjhm)) {
        warnMsg("请输入查询接口完整条件！");
        return;
    }
    addModel();
    var searchData = {"xm": xm, "cxrzjhm": cxrzjhm, "beanName": "bengbu_swxx"};
    // 查询婚姻登记信息
    $.ajax({
        url: "/realestate-inquiry-ui/rest/v1.0/queryByExchange/bengbu/swxx",
        type: 'GET',
        dataType: 'json',
        contentType: "application/json;charset=UTF-8",
        // data: encodeURI(JSON.stringify(searchData)),
        data: {
            "xm": xm, "cxrzjhm": cxrzjhm, "beanName": "bengbu_swxx"
        },
        success: function (data) {
            if (data) {
