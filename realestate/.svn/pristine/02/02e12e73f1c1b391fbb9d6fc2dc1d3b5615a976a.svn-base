...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            // 获取查询条件
            var tyshxydm = $(this).parents('.layui-inline').prev().find('#tyshxydm').val();
            // 判空
        ...
        function search() {
            ...
            if(searchData.cxywcs.length >0){
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    ...
                    // 重新请求
                    $.ajax({
                        url: "/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/dqjgcx",
                        type: "POST",
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijdqjgcx","党群机关信息查询",key,"市级共享交换平台");
        });
