 ...
 */
function generateQlxx(ycslYmxxDTO, bdcSlXm,$nowTab){
...
function saveSqr(ywlx){
    for (var i = 1; i <= $('.layui-tab-item').length; i++) {
        ...
        var $nowTab = $('.layui-tab-item:nth-child(' + i + ')');
        saveSqrTab($nowTab, tabXmid, tabDjxl,ywlx);
    }
        ...
        if($(".zrzcf").length != 0){
            ...
            for (var zrfnum = 0; zrfnum < zrfData.length; zrfnum++) {
                ...
                if ((zrfnum + 1 < zrfData.length && zrfData[zrfnum + 1].name === 'sqrid') || zrfnum + 1 == zrfData.length) {
                    ...
                    bdcSlSqrSingleData.sfzcqr = isNotBlank(zrf.sfzcqr) ? zrf.sfzcqr : "";
                    bdcSlSqrSingleData.zrfcsfgx = zrf.zrfcsfgx;
                    if(zrf.sqrlb != "1"){ // 卖方需要记录满五唯一、满二、是否直接亲属交易信息
                        bdcSlSqrSingleData.sfzxqs=zrf.sfzxqs;
                        sfzxqs = zrf.sfzxqs;
                    }
        ...
        if($(".zcf").length != 0){
            ...
            for (var zcfnum = 0; zcfnum < zcfData.length; zcfnum++) {
                ...
                if ((zcfnum + 1 < zcfData.length && zcfData[zcfnum + 1].name === 'sqrid') || zcfnum + 1 == zcfData.length) {
                    ...
                    bdcSlSqrSingleData.sfzcqr = isNotBlank(zcf.sfzcqr) ? zcf.sfzcqr : "";
                    sqrlist.push(bdcSlSqrSingleData);
                    bdcSlSqrSingleData = {};
                }
            }
...
// 页面上方保存按钮 保存权利人
function saveSqrTab($nowTab, tabXmid, tabdjxl,ywlx) {
    var bdcSlSqrData = [];
    var sqrlist = [];
    var qlrnum = 0;
    var ywrnum = 0;
    ...
    //验证婚姻信息比对结果
    if (!checkHyxxBdjg(sqrlist) &&ywlx !=="yrbj") {
