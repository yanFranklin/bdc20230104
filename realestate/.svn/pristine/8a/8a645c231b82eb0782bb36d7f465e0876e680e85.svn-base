...
function generateTableContent() {
    layui.use(['form', 'jquery', 'laytpl', 'element', 'laydate', 'table'], function () {
        ...
        if (bdcslxztzpz.xztzlxList !== null && bdcslxztzpz.xztzlxList.length > 0) {
            ...
            pplzztBdcqzhBs = getPplzztBdcqzhBs();
...
function checkBdcdyh(checkdData, sfcj) {
    ...
    for (var i = 0; i < checkdData.length; i++) {
        ...
        if(isNotBlank(selectData.qlr)) {
            bdcGzYzsjDTO.qlrmc = selectData.qlr;
        }
        if(isNotBlank(selectData.qlrzjh)) {
...
//添加不动产单元
function addBdcdyhShoppingCar(addData, sfcj) {
    if (sfcj) {
        //如果购物车存在数据，先清除购物车数据
...
function initLc(jbxxid, processDefKey, isFrame, needYz) {
    if (zlcsh == "true") {
        ...
        //判断是否为一窗创建,对照表存在数据,即为一窗流程
        queryYcslDjywDzb();
        if (!$.isEmptyObject(ycslDjywDzb) || zdjbdblc) {
            //一窗创建
            ycslCsh(jbxxid, processDefKey);
        } else {
            setTimeout(function () {
                lcCsh(jbxxid, processDefKey, isFrame, needYz)
            }, 50);
        }
    }
}
...
* 正常流程创建
* */
function lcCsh(jbxxid, processDefKey, isFrame, needYz) {
    $.ajax({
        ...
        success: function (data) {
            removeModal();
            if (data.msg === "success") {
                var portalUrl = "/realestate-portal-ui/view/new-page.html?taskId=" + data.taskId;
                if (isFrame) {
                    var index = parent.layer.getFrameIndex(window.name);
                    parent.layer.close(index);
                    setTimeout(function () {
                        parent.location.href = portalUrl;
                    }, 500);
                    parent.location.href = portalUrl + "&timestamp=" + new Date().getTime();
                } else {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (zllx === 'single') {
                ...
                var newUrl = changeURLPar(getUrl,'timestamp',new Date().getTime());
...
//页面刷新或者关闭时如果没有创建流程清空所有的冗余数据(监听页面关闭方法)
function clear() {
    //外联证书,关闭不执行清空操作
    if(xztztype ==="wlzs"){
        return false;
 ...
 * 供给楼盘表的js回调方法
 */
function xzbdcdyhSczsCallBack(lpbData){
    var sfsczs = 1;
    var zszl = 1;
    xzbdcdyhUpdateFzzt(lpbData, sfsczs, zszl);
 ...
 * 供给楼盘表的js回调方法
 */
function xzbdcdyhSczmdCallBack(lpbData){
    var sfsczs = 1;
    var zszl = 3;
    xzbdcdyhUpdateFzzt(lpbData, sfsczs, zszl);
 ...
 * 供给楼盘表的js回调方法
 */
function xzbdcdyhBfzCallBack(lpbData) {
    var sfsczs = 0;
    var zszl = null;
    xzbdcdyhUpdateFzzt(lpbData, sfsczs, zszl);
...
* 选择楼盘表页面只登记不登簿按钮回调
* */
function xzbdcdyhDjbdbCallBack(lpbData) {
    var sfsczs = 0;
    var zszl = 4;
    xzbdcdyhUpdateFzzt(lpbData, sfsczs, zszl);
}
...
* 楼盘表页面添加按钮回调方法
* */
function addGwcCallBack(lpbData) {
    addModel("添加中");
    var lpbcheckedData = [];
    $.each(lpbData, function (i, value) {
        if (isNotBlank(value.bdcdyh)) {
            lpbcheckedData.push({
                bdcdyh: value.bdcdyh,
                zl: value.zl,
                qjid: value.fwHsIndex,
                yt: value.ghyt,
                qlr: value.qlr,
                dyhcxlx: "1",
                qjgldm:value.qjgldm
    ...
    checkData = lpbcheckedData;
    checkBdcdyh(lpbcheckedData,false);
}

function xzbdcdyhUpdateFzzt(lpbData, sfsczs, zszl){

    if(lpbData.length == 0){
        ityzl_SHOW_WARN_LAYER("未获取到不动产单元号信息。");
        return;
    }
    var msg ="确认添加已选户室并生成证书吗？";
    if(sfsczs ===0) {
        msg = "确认添加已选户室并且只登簿不发证吗？";
        if (zszl === 4) {
            msg = "确认添加已选户室并且只登记不登簿吗？";
        }
    }else if(zszl ===3){
        msg ="确认添加已选户室并生成证明书吗？";
    }
    layer.confirm(msg, {title:'提示'}, function(index) {
        ...
        $.each(lpbData, function (i, value) {
            if (isNotBlank(value.bdcdyh)) {
                ...
                var bdcdyhObj ={};
                bdcdyhObj.bdcdyh =value.bdcdyh;
                bdcdyhList.push(bdcdyhObj);
            }
        });
        addModel();
        checkData = lpbcheckedData;
        sfcxjzlpb = true;
        //1.先更新已添加的发证状态
        var bdcdyhArr =updateFzztData(bdcdyhList, sfsczs, zszl);
        //排除掉已经添加更新的dyh
        lpbcheckedData=[];
        $.each(lpbData, function (i, value) {
            if (isNotBlank(value.bdcdyh) &&(bdcdyhArr.length ===0 ||bdcdyhArr.indexOf(value.bdcdyh) < 0)) {
                lpbcheckedData.push({
                    bdcdyh: value.bdcdyh,
                    zl: value.zl,
                    qjid: value.fwHsIndex,
...
function sczmbybdcdyh(bdcdyhList) {
    var sfsczs = 1;
    var zszl = 3;
    updateFzztData(bdcdyhList, sfsczs, zszl);
}
...
function updateFzztData(bdcdyhList, sfsczs, zszl){
    var bdcdyhArr =[];
    ...
    addModel();
    $.ajax({
        url: getContextPath() + "/gwc/updateSlxmFzzt/"+jbxxid+"?gzldyid="+processDefKey,
        type: 'PUT',
        dataType: 'json',
        async:false,
        contentType: "application/json;charset=UTF-8",
        data: JSON.stringify(bdcSlXmList),
        success: function (data) {
            if(data &&data.length >0){
                bdcdyhArr =data;
            }
...
/**
 * 获取配置的bdcqzh标识 用于判断匹配落宗状态
 * @returns {Array}
 */
function getPplzztBdcqzhBs(){
    var bdcqzhBsArr = [];
    $.ajax({
        url: getContextPath() + "/rest/v1.0/slym/pplzztBdcqzhBs",
        type: 'GET',
        async: false,
        success: function (data) {
            bdcqzhBsArr = data
        }, error: function (xhr, status, error) {
            console.log("获取配置的bdcqzh标识失败");
            bdcqzhBsArr = ['不动产']
        }
    });
    return bdcqzhBsArr;
...
function ydslcxtj(){
    ...
    $.ajax({
        url: getContextPath() + "/bdcdyh/ydslcxtj",
        type: 'GET',
        data : queryParam,
        dataType: 'text',
        async: false,
        success: function (data) {
            if(isNotBlank(data)){
                ...
                // 产权证展示的查询条件
                var cqzshowCxtjCount=0;
                // 外联证书展示的查询条件
                var wlzsshowCxtjCount=0;
                // 查封展示的查询条件
                var cfshowCxtjCount=0;
                // 获取所有产权证查询条件并且遍历
                $(".cqzcxtj").each(function(){
                ...
                // 产权证查询条件小于等于3时需要将下面的表格往上移动
                if(cqzshowCxtjCount <= 3){
                    $(".bdc-cqz").css("padding-top","57px");
                }
                // 获取所有外联证书查询条件并且遍历
                $(".wlzscxtj").each(function(){
                    if(!(showCxtj.indexOf($(this).attr("id")) > -1)){
                        $(this).hide();
                    }else{
                        wlzsshowCxtjCount++
                ...
                // 产权证查询条件小于等于3时需要将下面的表格往上移动
                if(wlzsshowCxtjCount <= 3){
                    $(".bdc-wlzs").css("padding-top","57px");
                }
                // 获取所有外联证书查询条件并且遍历
                $(".cfcxtj").each(function(){
                    if(!(showCxtj.indexOf($(this).attr("id")) > -1)){
                        $(this).hide();
                    }else{
                        cfshowCxtjCount++
                ...
                // 产权证查询条件小于等于3时需要将下面的表格往上移动
                if(cfshowCxtjCount <= 3){
                    $(".bdc-xzcf").css("padding-top","57px");
                }
                // 异地受理角色隐藏高级查询按钮
                $("#seniorSearch").hide();
            }
