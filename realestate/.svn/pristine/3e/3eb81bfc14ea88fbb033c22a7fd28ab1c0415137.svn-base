...
layui.use(['jquery', 'form', 'layer'], function () {
    ...
    $(function () {
        ...
        form.on('submit(copyBdcdyh)', function (data) {
            plblSelectBdcdyh(processInsId,qllx,xmid);
        });

        form.on('submit(copycq)', function (data) {
            plblCq(processInsId,qllx,xmid);
        });
...
function plblCq(processInsId,qllx,xmid) {
    var yxmid = xmid;
 ...
 * 选择不动产单元号批量补录
 */
function plblSelectBdcdyh(processInsId,qllx,xmid){
    var yxmid = xmid;
    var qllx = qllx;
    if (isNotBlank(yxmid)) {
        $.ajax({
            ...
            success: function (gzldyid) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (!isNullOrEmpty(data)) {
                            xztzlx = data.xztzlx;
                            checkIsOpenSelectBdcdyh("1", '../xxbl/plxxbl_selectbdcdyh.html?gzldyid=' + gzldyid+'&yxmid=' + yxmid);
                        } else {
                            warnMsg("选择台账未配置，请检查！")
                        }
                    }, error: function (xhr, status, error) {
 ...
 * 判断是否可以打开选择不动产单元台账
 */
function checkIsOpenSelectBdcdyh(tzlx, content) {
