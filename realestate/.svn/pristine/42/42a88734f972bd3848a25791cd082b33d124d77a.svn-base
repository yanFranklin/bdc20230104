...
function showMatchData(data, xztzlx, lx) {
    var index = parent.layer.open({
        ...
        content: getContextPath() + '/view/query/matchData.html?xztzlx=' + xztzlx + "&jbxxid=" + jbxxid + "&lx=" + lx,
...
* 先自动取消匹配，再跳转到匹配页面
* */
function reloadMatch(elem, jbxxid, xztzlx, xnxmid,qjgldm) {
    $.ajax({
...
function checkMullzPp(checkdata, xztzlx,lx,type){
    showConfirmDialog("提示", "项目存在多条匹配关系，是否重新匹配？", "reloadMatch", "'" + JSON.stringify(checkdata) + "','" + jbxxid + "','" + xztzlx + "','" + checkdata[0].xmid + "'");
