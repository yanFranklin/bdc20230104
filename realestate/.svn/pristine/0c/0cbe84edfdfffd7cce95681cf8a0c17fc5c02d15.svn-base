...
layui.use(['layer', 'element', 'form', 'table', 'jquery', 'laydate', 'laytpl'], function () {
    ...
    $(document).on('dblclick', 'input', function () {
        ...
        if ($(this).attr('disabled') == 'disabled') {
            if(isNullOrEmpty(bdsjshxgzd)){
                // 没有配置可修改字段，排除写死disabled的字段input.no-modify
                if ($(this).hasClass('no-modify')) {
                    ityzl_SHOW_WARN_LAYER("该字段不可编辑！");
                    return false;
                }
            }else{
                //配置可修改字段，不在配置中的字段中不能修改
                var bdsjshxgzdArr = bdsjshxgzd.split(",");
                if($.inArray($(this).attr('name'),bdsjshxgzdArr) == -1){
                    return false;
                }
            }
            //暂不支持受理table填报
            ...
            var $tabdDjxlCollection = $('.layui-tab-title').find('[name="djxl"]');
            var tabDjxlValues = [];
            for (let i = 0; i < $tabdDjxlCollection.length; i++) {
                tabDjxlValues[i] = $tabdDjxlCollection.eq(i).val();
            }
            if(tabDjxlValues.length < 1 || isNullOrEmpty(xgnrglxs) ){
                isGlxsDjxl = false;
            }else{
                for (let i = 1; i < tabBdcdyhValues.length; i++) {
                    if (xgnrglxs.indexOf(tabDjxlValues[i]) === -1) {
                        isGlxsDjxl = false;
                        break;
                    }
                }
            ...
            if (isLayerOpened) {
                ...
                if(isGlxsDjxl){
                    renderChange("", form, $(".bdc-sjsh-form")[0].getAttribute("id"));
                }
