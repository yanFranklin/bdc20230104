 ...
 * @description
 */
@Service(value = "httpPostForBengbuQi")
public class HttpPostRequestForBengbuQiServiceImpl extends InterfaceRequestService<HttpReqPropBO> {
     ...
     * 分配的appId
     */
    @Value("${bengbu.qi.appId:}")
    private String appid;

    /**
     * 私钥
     */
    @Value("${bengbu.qi.secret:}")
    private String secret;

    /**
     * RSA公钥
     */
    @Value("${bengbu.qi.rasPublicKey:}")
    ...
    public void sendRequest(InterfaceRequestBuilder builder) {
        ...
        LOGGER.info("蚌埠供气接口请求requestBody:" + JSONObject.toJSONString(requestBody));
        ...
        if (CheckParameter.checkAnyParameter(requestBody) && StringUtils.isNotBlank(prop.getUrl())) {
            ...
            if (CollectionUtils.isNotEmpty(requestParamMap.keySet())) {
                for (String key : requestParamMap.keySet()) {
                    if (Arrays.asList(HEADER_PARAMS).contains(key)) {
                        ...
                        try {
                            paramMap.put(key, SecurityUtil.aes128Encrypt((String) requestParamMap.get(key),aesSecret));
                        } catch (Exception e) {
                            LOGGER.info("蚌埠供气接口请求参数加密失败！:{}", e);
                            e.printStackTrace();
                        }
                    }
                }
            ...
            LOGGER.info("蚌埠供气paramMap：" + JSONObject.toJSONString(paramMap));
            String response = "";
            Exception logE = null;
            ...
            httpPost.setHeader("secretKey", encodeAesSecret);
            httpPost.setHeader("nonce", nonce);
            httpPost.setHeader("sign", sign);
            LOGGER.info("蚌埠供气的加密请求头参数，time:{},appId:{},aesSecret:{},secretKey:{},nonce:{},sign:{}", time, appid, aesSecret, encodeAesSecret, nonce, sign);
            ...
            try {
                LOGGER.info("蚌埠供气开始执行忽略证书http请求，方法为：{}", httpPost.getMethod());
                RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(30000).setConnectTimeout(30000).build();
                httpPost.setConfig(requestConfig);
                response = httpClientService.doPost(httpPost, "UTF-8");
                LOGGER.info("蚌埠供气response：{}", response);
            } catch (Exception e) {
                logE = e;
                LOGGER.error("蚌埠供气httpPost 请求异常：url:{},reqMap:{}", prop.getUrl(), JSONObject.toJSONString(requestBody), e);
                throw new AppException("蚌埠供气httpPost 请求异常");
            } finally {
                try {
