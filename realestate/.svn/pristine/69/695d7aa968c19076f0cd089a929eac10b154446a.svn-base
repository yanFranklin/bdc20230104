 ...
 */
public class ComputeRySzlTask implements Runnable {

    private static final Logger LOGGER = LoggerFactory.getLogger(ComputeRySzlTask.class.getName());

    /**
     * 工作量信息：线程共享变量，计算并设置 BdcGzltjXmxxVO 中的 szl(缮证量)参数
     */
    ...
    public ComputeRySzlTask(GzltjQO gzltjQO, List<BdcRyGzltjXmxxVO> bdcRyGzltjXmxxVOList, BdcGzltjXmxxMapper bdcGzltjXmxxMapper){
    ...
    public void run() {
        ...
        // 1、根据统计类型，获取人员缮证量信息
        List<BdcGzltjXmxxDTO> szlList = this.bdcGzltjXmxxMapper.countRySzlxx(gzltjQO);

        // 2、将人员办件量信息转换成 Map 对象 ( Key:人员ID, Value: 缮证量 )
        if(CollectionUtils.isNotEmpty(szlList)){
            Map<String, Integer> resultMap = szlList.stream().filter(t-> StringUtils.isNotBlank(t.getRyid()))
            ...
            while (bdcGzltjXmxxVOIterator.hasNext()) {
                ...
                if(StringUtils.isNotBlank(bdcRyGzltjXmxxVO.getRyid()) && resultMap.containsKey(bdcRyGzltjXmxxVO.getRyid())){
                    bdcRyGzltjXmxxVO.setSzl(resultMap.get(bdcRyGzltjXmxxVO.getRyid()));
                }
            }
        }
        LOGGER.info("计算人员缮证量结束，耗时：{}， 查询参数：{}" , System.currentTimeMillis() - startTime, JSON.toJSONString(gzltjQO));
    }
