        ...
        if (!isNullOrEmpty(cxdh)) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#jkff").val(data.jkff);
                    $("#jk").parent().attr("style", "width:800px");
...
function checkJson(json) {
    var checkJson = json.replace(/\s+/g,"");
    if (isNullOrEmpty(checkJson)) {
        layui.layer.msg("请配置查询条件json");
        return false;
    }

    var result = true;
    $.ajax({
        url: '/realestate-inquiry-ui' + "/rest/v1.0/jk/dtcx/check/config/cxjson",
        type: "post",
        async: false,
        data: {
            cxjson: cxsql,
...
function checkCxtj() {
    ...
    if (check === true) {
        $.ajax({
            url: '/realestate-inquiry-ui' + "/rest/v1.0/jk/dtcx/check/config/cxtj",
            type: "post",
            async: false,
            data: {
                cxjson: cxsql,
...
function saveAction() {
    ...
    if (checkJson(cxsql) && checkCxtj() && checkCxjg()) {
        if (dhPassCheck) {
            saveAll();
        } else {
...
function saveAll() {
    ...
    $.ajax({
        url: '/realestate-inquiry-ui' + "/rest/v1.0/jk/dtcx/save/all?" + $("#cxxx").serialize(),
        type: "post",
        async: true,
        data: {
            cxsql: cxsql,
