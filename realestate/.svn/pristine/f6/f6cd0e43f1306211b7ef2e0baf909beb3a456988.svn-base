package cn.gtmap.realestate.exchange.core.dto.ggxypt.zrr;

import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;

import java.util.Date;

/**
 * @author <a href="mailto:haungjian@gtmap.cn">huangjian</a>
 * @Date 2022/8/31
 * @description 企业抵押信息DTO
 */
@ApiModel(description = "个人抵押信息DTO")
public class ZrrDyaDTO {


    /**
     * dyrmc : 泰州市房产局
     * sfzjlx : 泰州恒祥兴置业有限公司
     * dyrzjh : 泰州市自然资源和规划局
     * bdcdjzh : 苏（2020）泰州市不动产权第0096403号
     * bdcdyh :
     * qllx :
     * dyqrmc :
     * dyqrzjlx : 住宅
     * dyqrzjhm :
     * djsj : 2020-01-13 00:00:00
     * lydw : 统一社会信用代码
     * sjgxsj : 2020-12-19 00:00:00
     * recid : b6c8bac84e8b316de055000000000001
     */
    @ApiModelProperty(value = "抵押人名称")
    private String dyrmc;
    @ApiModelProperty(value = "证件类型")
    private String sfzjlx;
    @ApiModelProperty(value = "抵押人证件号")
    private String dyrzjh;
    @ApiModelProperty(value = "抵押人证件号")
    private String bdcdjzh;
    @ApiModelProperty(value = "不动产单元号")
    private String bdcdyh;
    @ApiModelProperty(value = "权利类型")
    private String qllx;
    @ApiModelProperty(value = "抵押权人名称")
    private String dyqrmc;
    @ApiModelProperty(value = "抵押权人证件类型")
    private String dyqrzjlx;
    @ApiModelProperty(value = "抵押权人证件号码")
    private String dyqrzjhm;
    @ApiModelProperty(value = "登记时间")
    private Date djsj;
    @ApiModelProperty(value = "来源单位")
    private String lydw;
    @ApiModelProperty(value = "数据更新时间")
    private Date sjgxsj;
    @ApiModelProperty(value = "记录ID")
    private String recid;

    public String getDyrmc() {
        return dyrmc;
    }

    public void setDyrmc(String dyrmc) {
        this.dyrmc = dyrmc;
    }

    public String getSfzjlx() {
        return sfzjlx;
    }

    public void setSfzjlx(String sfzjlx) {
        this.sfzjlx = sfzjlx;
    }

    public String getDyrzjh() {
        return dyrzjh;
    }

    public void setDyrzjh(String dyrzjh) {
        this.dyrzjh = dyrzjh;
    }

    public String getBdcdjzh() {
        return bdcdjzh;
    }

    public void setBdcdjzh(String bdcdjzh) {
        this.bdcdjzh = bdcdjzh;
    }

    public String getBdcdyh() {
        return bdcdyh;
    }

    public void setBdcdyh(String bdcdyh) {
        this.bdcdyh = bdcdyh;
    }

    public String getQllx() {
        return qllx;
    }

    public void setQllx(String qllx) {
        this.qllx = qllx;
    }

    public String getDyqrmc() {
        return dyqrmc;
    }

    public void setDyqrmc(String dyqrmc) {
        this.dyqrmc = dyqrmc;
    }

    public String getDyqrzjlx() {
        return dyqrzjlx;
    }

    public void setDyqrzjlx(String dyqrzjlx) {
        this.dyqrzjlx = dyqrzjlx;
    }

    public String getDyqrzjhm() {
        return dyqrzjhm;
    }

    public void setDyqrzjhm(String dyqrzjhm) {
        this.dyqrzjhm = dyqrzjhm;
    }

    public Date getDjsj() {
        return djsj;
    }

    public void setDjsj(Date djsj) {
        this.djsj = djsj;
    }

    public String getLydw() {
        return lydw;
    }

    public void setLydw(String lydw) {
        this.lydw = lydw;
    }

    public Date getSjgxsj() {
        return sjgxsj;
    }

    public void setSjgxsj(Date sjgxsj) {
        this.sjgxsj = sjgxsj;
    }

    public String getRecid() {
        return recid;
    }

    public void setRecid(String recid) {
        this.recid = recid;
    }

    @Override
    public String toString() {
        return "QyDyaDTO{" +
                "dyrmc='" + dyrmc + '\'' +
                ", sfzjlx='" + sfzjlx + '\'' +
                ", dyrzjh='" + dyrzjh + '\'' +
                ", bdcdjzh='" + bdcdjzh + '\'' +
                ", bdcdyh='" + bdcdyh + '\'' +
                ", qllx='" + qllx + '\'' +
                ", dyqrmc='" + dyqrmc + '\'' +
                ", dyqrzjlx='" + dyqrzjlx + '\'' +
                ", dyqrzjhm='" + dyqrzjhm + '\'' +
                ", djsj=" + djsj +
                ", lydw='" + lydw + '\'' +
                ", sjgxsj=" + sjgxsj +
                ", recid='" + recid + '\'' +
                '}';
    }
}
