...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl', 'upload'], function () {
    ...
    $.ajax({
        url: getContextPath() + '/rest/v1.0/qjgl/list/qjgldm',
        type: 'GET',
        dataType: 'json',
        async: false,
        success: function (data) {
            if(data &&data.length >0) {
                ...
                $.each(data, function (index, item) {
                    $('#qjgldm').append(new Option(item.mc, item.dm));// 下拉菜单里添加元素
                });
                form.render("select");
            }
    ...
    form.on("submit(query)", function (data) {
        ...
        tableReloadWithUrl('ljzList', postData,"../ljz/listbypage?sfyc="+sfyc);
    ...
    var tableConfig = {
        toolbar: "#toolbarDemo"
        , url: '../ljz/listbypage?sfyc=' + sfyc+"&qjgldm="+qjgldm //数据接口
        , cols: [[
            {type: 'checkbox', fixed: 'left', align: 'center', width: '4%'},
            {
                field: 'ljzh', title: '逻辑幢号', width: '13%'
                , templet: function (d) {
                    var view = d.ljzh;
                    if (!view) {
                        view = "查看";
                    ...
                    return '<div><a href="javascript:void(0);" ' +
                        'lay-event="editLjz" class="layui-table-link ljzh">' + view + '</a></div>';
                }
            },
            {field: 'fwmc', title: '房屋名称', width: '13%'},
    ...
    table.on('toolbar(dataTable)', function (obj) {
        ...
        if (data && data.length === 1) {
            ...
            if (obj.event === "importLpb") {
                importLpb(data[0].fw_dcb_index,data[0].qjgldm);
                return false;
            }
            if (obj.event === "importYcLpb") {
                importYcLpb(data[0].fw_dcb_index,data[0].qjgldm);
                return false;
            }
            if (obj.event === "exportLpb") {
                exportLpb(data[0].fw_dcb_index,data[0].qjgldm);
                return false;
            }
            if (obj.event === "importScjzmj") {
                importScjzmj(data[0].fw_dcb_index,data[0].qjgldm);
                return false;
            }
    ...
    table.on('tool(dataTable)', function (obj) {
        ...
        if (data && data.fw_dcb_index) {
            ...
            if (obj.event == "toLpbView") {
                saveSessionParam(getParamObject());
                toView(getIP() + '/lpb/view?code=bdcres&fwDcbIndex=' + data.fw_dcb_index+"&qjgldm="+data.qjgldm,{tabname:"楼盘表"});
            }
            if (obj.event === "fwlxBg") { //房屋类型变更
                var index = layer.open({
                    ...
                    area: ['400px', '310px'],
                    fixed: false, //不固定
                    content: '../bdcdyfwlxbg/view?fwDcbIndex=' + data.fw_dcb_index+"&qjgldm="+data.qjgldm
            ...
            if (obj.event === "ycToSc") {//预测转实测
                yczsc(data.fw_dcb_index,data.qjgldm);
            }
            if (obj.event === "ycScGl") {
                saveSessionParam(getParamObject());
                ycscgl(data.fw_dcb_index,data.qjgldm);
            }
            if (obj.event == "zlsx") {
                zlsx(data.fw_dcb_index,data.qjgldm);
            }
            if (obj.event === "jsjzmj") { // 计算建筑面积
                jsLjzJzmj(data.fw_dcb_index,data.qjgldm);
            }
            if (obj.event === "jsfttdmj") { // 计算分摊土地面积
                jsLjzFttdmj(data.fw_dcb_index,data.qjgldm);
            }
            ...
            if (obj.event === "djdcb") { // 地籍调查表
                if (data.bdcdyh) {
                    window.open('/building-ui/djdcb/fromljz?bdcdyh=' + data.bdcdyh + "&qjgldm=" + data.qjgldm);
                } else {
    ...
    function deleteLjzFun(data) {
        ...
        $.ajax({
            url: "../ljz/delbyindex",
            dataType: "json",
            data: "fwDcbIndexList=" + encodeURI(fwDcbIndexList) +"&qjgldm="+data[0].qjgldm,
            success: function (data) {
                layer.closeAll();
                removeModal();
