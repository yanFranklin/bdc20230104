...
layui.use(['form','laytpl','layer'],function () {
    ...
    $(function () {
        $.ajax({
            type: 'GET',
            url: '/realestate-exchange/rest/v1.0/gxhpz/query/form?url=' + encodeURI(url) + '&formid=' + formid,
            async: false,
...
function saveAction() {
    $.ajax({
        type: 'POST',
        url: '/realestate-exchange/rest/v1.0/gxhpz/save/form?url=' + encodeURI(url) + '&formid=' + formid,
        data: JSON.stringify(searchData),
        contentType: 'application/json',
        async: false
    });
