...
$(document).ready(function () {
    ...
    getZd(function (data) {
        ...
        $.each(data.cbtdyt, function (index, item) {
            $('#jyyt').append('<option value="' + item.DM + '">' + item.MC + '</option>');
        });
    ...
    if (readOnly || !isNullOrEmpty(formStateId)) {
        getStateByIdQlxx(readOnly, formStateId, "tdjyq");
    }

    var showXgLog = $.getUrlParam("showXgLog");
    if(showXgLog){
        queryXgLog(xmid);
        renderChange("",form,"tdjyq");
        renderDate(form,"tdjyq");
    }
});