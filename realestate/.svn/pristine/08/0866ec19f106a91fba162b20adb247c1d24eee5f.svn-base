...
function formatXzzt(data) {
    ...
    if (data != undefined) {
        if (data.cf === true) {
            //南通样式
            //value += '<span class="bdc_change_red" style="color:#EE0000;">查封 </span>';
        ...
        if (data.lqlzzt === true) {
            value += '<span class="" style="color:#13b1c4;">流转 </span>';
        }
        if (value === '') {
...
// 格式化限制状态(包含次数)
function formatXzztxx(data) {
    var value = '';
    if (data != undefined) {
        if (data.cf === true) {
            //南通样式
            //value += '<span class="bdc_change_red" style="color:#EE0000;">查封 </span>';
            value += '<span class="" style="color:#EE0000;">查封('+data.xscfcs+') </span>';
        }
        if (data.ycf === true) {
            value += '<span class="" style="color:#ee717a;">预查封('+data.xsycfcs+') </span>';
        }
        if (data.dya === true || data.zjgcdy === true) {
            value += '<span class="" style="color:#FF00FF;">抵押('+data.xsdyacs+') </span>';
        }
        if (data.ydya === true) {
            value += '<span class="" style="color:#aacd06;">预抵押('+data.xsydyacs+') </span>';
        }
        if (data.yy === true) {
            value += '<span class="" style="color:#ef7106;">异议('+data.xsyycs+') </span>';
        }
        if (data.yg === true) {
            value += '<span class="" style="color:#ffb60c;">预告('+data.xsygcs+') </span>';
        }
        if (data.dyi === true) {
            value += '<span class="" style="color:#855e6e;">地役('+data.xsdyics+') </span>';
        }
        if (data.sd === true) {
            value += '<span class="" style="color:#e50971;">锁定('+data.xssdcs+') </span>';
        }
        if (data.jzq === true) {
            value += '<span class="" style="color:#13b1c4;">居住('+data.xsjzqcs+') </span>';
        }
        if (data.lqlzzt === true) {
            value += '<span class="" style="color:#13b1c4;">流转 </span>';
        }
