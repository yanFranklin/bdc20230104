 ...
 * 证书证明打印验证组合规则标识
 */
var ZSZM_PRINT_ZHGZ = "ZSZM_PRINT_ZHGZ";
...
function zsPrint(modelUrl, zsid, zslx, sessionKey) {
    checkBeforePrint("", zsid, ZSZM_PRINT_ZHGZ).fail(function () {
        return;
    }).done(function () {
...
function batchZsPrint(modelUrl, zsidList, zslx) {
    checkBeforePrint("", zsidList, ZSZM_PRINT_ZHGZ).fail(function () {
        return;
    }).done(function () {
 ...
 * @param modelUrl 打印模板地址
 * @param zslx 打印的证书类型
 * @param sessionKey 打印配置session
 */
function ntBatchZsPrint(modelUrl, zsidList, zslx, sessionKey) {
    checkBeforePrint("", zsidList, ZSZM_PRINT_ZHGZ).fail(function () {
        return;
    }).done(function () {
        $.ajax({
            ...
            success: function (key) {
                if (key) {
                    var dataUrl = getIP() + "/realestate-register-ui/rest/v1.0/print/nantong/batchZs/" + zslx + "/" + key;
                    var appName = "realestate-register-ui";
                    printChoice(zslx, appName, dataUrl, modelUrl, false, sessionKey);
                } else {
                    layer.msg('<img src="../../static/lib/bdcui/images/error-small.png" alt="">系统异常，请重试！');
...
function allZsPrint(modelUrl, gzlslid, zslx,zsmodel) {
    checkBeforePrint(gzlslid, "", ZSZM_PRINT_ZHGZ).fail(function () {
        return;
    }).done(function () {
        var dataUrl = getIP() + "/realestate-register-ui/rest/v1.0/print/allZs/" + zslx + "?gzlslid=" + gzlslid;
        print(modelUrl, dataUrl, false);
    });
}
 ...
 * @param gzlslid 工作流实例ID
 * @param zslx 证书类型
 * @param sessionKey 页面打印sessionKey
 */
function ntAllZsPrint(modelUrl, gzlslid, zslx, sessionKey) {
    checkBeforePrint(gzlslid, "", ZSZM_PRINT_ZHGZ).fail(function () {
        return;
    }).done(function () {
        var appName = "realestate-register-ui";
...
function saveZszmPrintInfo(zslx, gzlslid, zsid) {
    var zslxcode;
    if ("zs" == zslx || "zs1" == zslx || "zs2" == zslx || "zs3" == zslx || "zs1and2" === zslx
...
// 证书打印按钮
$('.title-btn-area').on('click', '.bdc-print-btn', function (event) {
    return false;
});
$('.title-btn-area').on('mouseenter', '.bdc-print-btn', function (event) {
    if ($(".print").hasClass("layui-btn-disabled")) {
        return false;
    }
    $('.bdc-yzh-more').hide();
    $('.bdc-yl-more').hide();
 ...
 * @param zsid
 */
function printZdt(zsid, sessionKey) {
 ...
 * @param zsid
 */
function printHst(zsid, sessionKey) {
...
function setSfyj(zsid){
    if($("#sfyj").length >0) {
        $.ajax({
            url: "/realestate-register-ui/rest/v1.0/sfxyjg?zsid=" + zsid,
            type: "GET",
            success: function (data) {
                if (1 === data) {
                    var $sfyj = $('#sfyj')[0];
                    $sfyj.className = 'sfyj-tip';
                    $sfyj.innerText = "已生成电子证照，无需打印及获取印制号";
                }
 ...
 * @param data 证书相关信息
 * @param gzlslid 工作流实例ID
 * @param maxShowNum 限制展示地块数量
 * @param zsid 证书ID
 */
function showDklb(data, gzlslid, maxShowNum, zsid) {
    if(isNullOrEmpty(gzlslid) && data && data.bdcXmDOList) {
        gzlslid = data.bdcXmDOList[0].gzlslid;
    }

    if(!isNullOrEmpty(zsid)) {
        // 默认按照zsid查询，即查询当前证书关联的地块，避免用gzlslid查询时候分别持证情况下查询错误
        gzlslid = "";
    }

    if(!isNullOrEmpty(data.qllx) && TDCBJYQ_QLLX == data.qllx) {
        $.ajax({
            url: '/realestate-register-ui/rest/v1.0/djbxx/qlxx/tdcbjyq/dkxx',
            data: JSON.stringify({"gzlslid": gzlslid, "zsid": zsid}),
            contentType: "application/json;charset=utf-8",
            type: "POST",
            dataType: "json",
            success: function (dklbxx) {
                if (dklbxx && dklbxx.length > 0) {
                    var showLength = dklbxx.length;
                    if(showLength > maxShowNum) {
                        showLength = maxShowNum;
                        warnMsg("地块过多，只做部分展示，请打印地块列表贴图");
                    }

                    var html = "";
                    for (var i = 0, len = showLength; i < len; i++) {
                        html += getSzHtml("", "", "", "", "东", dklbxx[i].zdszd);
                        html += getSzHtml(dklbxx[i].zl, dklbxx[i].bdcdyh, dklbxx[i].cbmj, dklbxx[i].scmj, "西", dklbxx[i].zdszx);
                        html += getSzHtml("", "", "", "", "南", dklbxx[i].zdszn);
                        html += getSzHtml("", "", "", "", "北", dklbxx[i].zdszb);
                        html += "<tr><td></td></tr>";
                    }
                    $("#dklbxx").append(html);
                    // 常州土地承包经营权证书显示地块列表
                    $("#dklb").show();
                    if ($("#dklb").height() > 348) {
                        var height = $("#dklb").height() + 100;
                        $(".bdc-notes").css("height", height + "px");
                    }
                }
            },
            error: function () {
...
/**
 * 预览 常州土地承包经营权证书显示地块列表
 * @param data 证书相关信息
 * @param gzlslid 工作流实例ID
 * @param maxShowNum 限制展示地块数量
 * @param xmid 项目ID
 */
function showDklbYl(data, gzlslid, maxShowNum, xmid) {
    if(isNullOrEmpty(xmid)) {
        // 项目ID为空则当前是批量一本证直接可以根据gzlslid查询地块
        showDklb(data, gzlslid, maxShowNum, "");
        return ;
    }

    if(!isNullOrEmpty(data.qllx) && TDCBJYQ_QLLX == data.qllx) {
        $.ajax({
            url: '/realestate-register-ui/rest/v1.0/djbxx/qlxx/tdcbjyq/dkxx/beforeinit/' + xmid,
            contentType: "application/json;charset=utf-8",
            type: "GET",
            dataType: "json",
            success: function (dklbxx) {
 ...
 * @param sz 四至信息
 */
function getSzHtml(zl, bdcdyh, cbmj, scmj, wz, sz) {
    if(isNullOrEmpty(cbmj)) {
        cbmj = "";
    } else {
        cbmj += "亩";
    }

    if(isNullOrEmpty(scmj)) {
        scmj = "";
    } else {
        scmj += "亩";
    }
    ...
    var html = "<tr>";
    html += " <td class=\"set-back-color\">" + zl + "</td>";
    html += " <td class=\"set-back-color\">" + bdcdyh + "</td>";
    html += " <td class=\"set-back-color\">" + cbmj + "</td>";
    html += " <td class=\"set-back-color\">" + scmj + "</td>";
    html += " <td class=\"set-back-color\">" + wz + "：" + sz + "</td>";
    html += "</tr>";
    return html;
}
 ...
 * 常州打印证书证明之前进行规则验证
 * @param gzlslid 工作流实例ID
 * @param zsids 证书ID（批量打印时候多个拼接）
 * @param zhbs 组合规则标识
 * @returns {*}
 */
function checkBeforePrint(gzlslid, zsids, zhbs) {
    if (isNullOrEmpty(zhbs)) {
        warnMsg("未配置验证规则，请联系管理员！");
    }

    var deferred = $.Deferred();
    if (isNullOrEmpty(gzlslid) && isNullOrEmpty(zsids)) {
        return deferred.resolve();
    }

    var bdcGzYzsjDTOList = new Array();
    if(!isNullOrEmpty(zsids)) {
        $.each(zsids.split(","), function (index, value) {
    ...
    $.ajax({
        url: "/realestate-register-ui/rest/v1.0/print/gzyz/" + zhbs,
        type: 'POST',
        dataType: 'json',
        contentType: "application/json;charset=UTF-8",
        data: JSON.stringify({
...
/**
 * 常州农村不动产确权登记显示不动产家庭成员信息列表
 * @param data 证书相关信息
 * @param gzlslid 工作流实例ID
 * @param zsid 证书ID
 */
function showJtcylb(data,gzlslid,zsid) {
    if(!isNullOrEmpty(data.qllx) && NCBDCQQDJ_QLLX == data.qllx && !isNullOrEmpty(data.lzrzjh)) {
            $.ajax({
                url: '/realestate-register-ui/rest/v1.0/djbxx/qlxx/ncbdcqqdj/jtcyxx/'+data.lzrzjh,
                contentType: "application/json;charset=utf-8",
                type: "GET",
                dataType: "json",
                success: function (jtcyxxList) {
                    if (jtcyxxList && jtcyxxList.length > 0) {
                        zdList = getZdList();
                        var showLength = jtcyxxList.length;
                        var html = "";
                        for (var i = 0, len = showLength; i < len; i++) {
                            html += "<tr>";
                            html += " <td class=\"set-back-color\">" + jtcyxxList[i].jtcymc + "</td>";
                            var xb = changeDmToMc(jtcyxxList[i].xb,'',zdList.xb);
                            var yhzgx = changeDmToMc(jtcyxxList[i].yhzgx,'',zdList.yhzgx);
                            html += " <td class=\"set-back-color\">" + xb + "</td>";
                            html += " <td class=\"set-back-color\">" + yhzgx + "</td>";
                            html += " <td class=\"set-back-color\">" + jtcyxxList[i].jtcyzjh + "</td>";
                            html += "</tr>";

                        }
                        $("#jtcy").append(html);
                        // 常州农村不动产确权登记证书显示家庭成员信息列表
                        $("#jtcy").show();
                        if($("#jtcy").height() > 348){
                            var height = $("#jtcy").height() + 100;
                            $(".bdc-notes").css("height",height+"px");
                        }
                    }
                },
 ...
 * @param gzlslid 工作流实例ID
 * @returns {number} 同一工作流实例ID不动产单元号数量
 */
function getBdcdyhCount(gzlslid) {
    var bdcdyhCount = 0;
    $.ajax({
        type: "GET",
        url: "/realestate-register-ui/rest/v1.0//xmxx/" + gzlslid + "/bdcqzh/count",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
