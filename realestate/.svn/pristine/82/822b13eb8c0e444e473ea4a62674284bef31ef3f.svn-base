 ...
 */
@Service
public class InitYywrToBdcQlrServiceImpl extends InitBdcQlrAbstractService {
    @Override
    public String getVal() {
        return Constants.QLRSJLY_YYWR.toString();
    }
    ...
    public List<BdcQlrDO> initQlr(InitServiceQO initServiceQO){
        ...
        if(initServiceQO!=null && StringUtils.isNotBlank(initServiceQO.getYxmid())
            ...
            if (initServiceDTO != null && CollectionUtils.isNotEmpty(initServiceDTO.getBdcQlrList())) {
                // 通过组合业务中生成的原项目义务人信息为权利人信息赋值
                bdcQlrDOList = bdcQlrService.inheritYxmBdcYwr(initServiceDTO.getBdcQlrList(),initServiceQO.getXmid(),CommonConstantUtils.QLRLB_QLR);
            } else {
                //获取上一手项目义务人，继承为当前项目的权利人
                bdcQlrDOList = bdcQlrService.inheritYxmBdcYwr(initServiceQO.getYxmid(),initServiceQO.getXmid(),CommonConstantUtils.QLRLB_QLR);
            }
        }
        return bdcQlrDOList;
    }
