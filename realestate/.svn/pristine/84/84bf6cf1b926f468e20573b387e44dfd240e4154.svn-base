...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        function search() {
            ...
            if(searchData.cxywcs.length >0){
                ...
                for (var paramlength = 0; paramlength < searchData.cxywcs.length; paramlength++) {
                    var xm = searchData.cxywcs[paramlength].xm;
                    var zjh = searchData.cxywcs[paramlength].zjh;
                    // 重新请求
                    $.ajax({
                        url:"/realestate-inquiry-ui/rest/v1.0/sjpt/rkkjzxxcx"+
                            "?xm=" + encodeURI(xm) +
                            "&zjh=" + encodeURI(zjh),
                        type: "GET",
                        ...
                        success: function (data) {
                            if (data.head.status == '0') {
                                successMsg(data.head.msg);
                                // 结果数据不为空，重新加载表格
                                if(data.data && data.data.cxjg.length>0) {
                                    for(var i=0;i<data.data.cxjg.length;i++) {
                                        var cxjg = data.data.cxjg[i];
                                        cxjg.xm = xm;
                                        cxjg.zjh = zjh;
                                        resultData.push(cxjg);
                                    }
                                }
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijrkkjzxxcx","国家公安部—人口库基准信息查询",key,"公安部");
        });
    });
