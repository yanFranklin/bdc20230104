package cn.gtmap.realestate.common.core.vo.config.ui;

/**
 * @author <a href="mailto:zhuyong@gtmap.cn">zhuyong</a>
 * @version 1.0, 2019/2/1
 * @description 证书印制号VO实体对象
 */
public class BdcYzhVO {
    /**
     * 年份
     */
    private String nf;
    /**
     * 证书类型
     */
    private Integer zslx;
    /**
     * 省区代码
     */
    private String sqdm;
    /**
     * 编号位数
     */
    private Integer bhws;
    /**
     * 起始编号
     */
    private Integer qsbh;
    /**
     * 结束编号
     */
    private Integer jsbh;
    /**
     * 区县代码
     */
    private String qxdm;
    /**
     * 领取人员
     */
    private String lqr;
    /**
     * 领取人员ID
     */
    private String lqrid;
    /**
     * 领取人账户名
     */
    private String lqrName;
    /**
     * 领取部门
     */
    private String lqbm;
    /**
     * 领取部门ID
     */
    private String lqbmid;

    /**
     * 使用部门 added by cyc at 2019-07-19
     */
    private String sybmmc;
    /**
     * 使用人 added by cyc at 2019-07-19
     */
    private String syr;

    public String getSybmmc() {
        return sybmmc;
    }

    public void setSybmmc(String sybmmc) {
        this.sybmmc = sybmmc;
    }

    public String getLqrName() {
        return lqrName;
    }

    public void setLqrName(String lqrName) {
        this.lqrName = lqrName;
    }

    public String getSyr() {
        return syr;
    }

    public void setSyr(String syr) {
        this.syr = syr;
    }

    public String getNf() {
        return nf;
    }

    public void setNf(String nf) {
        this.nf = nf;
    }

    public Integer getZslx() {
        return zslx;
    }

    public void setZslx(Integer zslx) {
        this.zslx = zslx;
    }

    public String getSqdm() {
        return sqdm;
    }

    public void setSqdm(String sqdm) {
        this.sqdm = sqdm;
    }

    public Integer getBhws() {
        return bhws;
    }

    public void setBhws(Integer bhws) {
        this.bhws = bhws;
    }

    public Integer getQsbh() {
        return qsbh;
    }

    public void setQsbh(Integer qsbh) {
        this.qsbh = qsbh;
    }

    public Integer getJsbh() {
        return jsbh;
    }

    public void setJsbh(Integer jsbh) {
        this.jsbh = jsbh;
    }

    public String getQxdm() {
        return qxdm;
    }

    public void setQxdm(String qxdm) {
        this.qxdm = qxdm;
    }

    public String getLqr() {
        return lqr;
    }

    public void setLqr(String lqr) {
        this.lqr = lqr;
    }

    public String getLqrid() {
        return lqrid;
    }

    public void setLqrid(String lqrid) {
        this.lqrid = lqrid;
    }

    public String getLqbm() {
        return lqbm;
    }

    public void setLqbm(String lqbm) {
        this.lqbm = lqbm;
    }

    public String getLqbmid() {
        return lqbmid;
    }

    public void setLqbmid(String lqbmid) {
        this.lqbmid = lqbmid;
    }

    @Override
    public String toString() {
        return "BdcYzhVO{" +
                "nf='" + nf + '\'' +
                ", zslx=" + zslx +
                ", sqdm='" + sqdm + '\'' +
                ", bhws=" + bhws +
                ", qsbh=" + qsbh +
                ", jsbh=" + jsbh +
                ", qxdm='" + qxdm + '\'' +
                ", lqr='" + lqr + '\'' +
                ", lqrid='" + lqrid + '\'' +
                ", lqbm='" + lqbm + '\'' +
                ", lqbmid='" + lqbmid + '\'' +
                ", sybmmc='" + sybmmc + '\'' +
                ", syr='" + syr + '\'' +
                '}';
    }
}
