...
function saveCols(data, tableid, url) {
    $.ajax({
        type: 'POST',
        url: '/realestate-natural-ui/rest/v1.0/gxhpz/save/table?url=' + encodeURI(url) + '&tableid=' + tableid,
...
function getGxhpz(tableid) {
    var url;
    if (window.location.pathname != "/realestate-natural-ui/view/dtcx/commonCx.html") {
    ...
    $.ajax({
        type: 'GET',
        url: '/realestate-natural-ui/rest/v1.0/gxhpz/query/table?url=' + encodeURI(url) + '&tableid=' + tableid,
...
function formLoad() {
    if ($('.bdc-search-box').length > 0) {
        ...
        $searchBox.each(function (i) {
            $($searchBox[i]).append('<a href="javascript:;" title="自定义配置查询条件" class="bdc-set-search" style="position: absolute;top: 0;right: 0;">' +
    ...
    for (var i = 0; i < forms.length; i++) {
        ...
        $.ajax({
            type: 'GET',
            url: '/realestate-natural-ui/rest/v1.0/gxhpz/query/form?url=' + encodeURI(url) + '&formid=' + formid,
            async: false,
            success: function (data) {
                if (data) {
    ...
    $(".bdc-set-search").on('click', function () {
        ...
        if (sameFlag == false) {
            ...
            $.ajax({
                type: 'POST',
                url: '/realestate-natural-ui/rest/v1.0/gxhpz/save/form?url=' + encodeURI(url) + '&formid=' + formid,
        ...
        var index = layer.open({
            ...
            content: "/realestate-natural-ui/view/gxhpz/searchPz.html?formid=" + formid + "&url=" + encodeURI(url),
            btn: ['保存', '取消'],
