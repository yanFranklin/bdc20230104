...
layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl', 'laydate'], function () {
    ...
    // 监听林地使用（承包期限）为长期
    form.on('radio(syqx)', function () {
        syqxchange = true;
        var syqx = $("input[id='lq_syqx']:checked").val();
        var $ldsysj = $('.lq-ldsysj');
        var ldsyqssj = document.getElementById('lq-ldsyqssj');
        var ldsyjssj = document.getElementById('lq-ldsyjssj');
        if(syqx==='长期'){
            ldsyqssj.value = "";
            ldsyjssj.value = "";
            ldsyjssj.disabled = true;
            ldsyqssj.disabled = true;
            disabledAddFa();
            form.render('select');
        }else if(syqx!=='长期'){
            ldsyjssj.disabled = false;
            ldsyqssj.disabled = false;
            $ldsysj.each(function(i){
                $($ldsysj[i]).removeAttr('disabled');
                $($ldsysj[i]).parent().removeClass('bdc-date-disabled');
                $($ldsysj[i]).siblings('img').remove();
            });
            form.render('select');
        }
    });
...
function loadButtonArea(ymlx) {
    ...
    //验收文件
    $("#yswjhq").click(function () {
        yswjhq();
    });

    //公证处文件查询
    $("#gzcwjcx").click(function () {
        gzcwjcx();
    });
}
...
*        zwjmc    需要生成的子文件夹名称
* */
function queryDchyjg(beanName, clmc, cllx, zwjmc) {
    addModel("");
    getReturnData("/slym/sjcl/dchy", {
        beanName: beanName,
        clmc: clmc,
        zwjmc: zwjmc,
        cllx: cllx,
...
function titleShowUi() {
    ...
    $(".secondary-clbgsjcl-btn").click(function (e) {
        var e = e || window.event; //浏览器兼容性
        if (e.stopPropagation) {
            e.stopPropagation();
        } else {
        ...
        $("#third-prinbtn-sqs").hide();
        $("#third-chjg-bth").show();
        setThirdUiWidth($(this), $("#third-chjg-bth"));
    });
    ...
    $(".mk-btn li").click(function () {
        $("#mk").hide();
    });

    $(".chcg-btn li").click(function () {
        $("#chcg").hide();
    });

    $("#third-chjg-bth li").click(function () {
        $("#chcg").hide();
    });
...
function showDchyCllx(element) {
    $("#third-prinbtn-sjd").hide();
    $("#third-prinbtn-sqs").hide();
    $("#third-chjg-bth").show();
    setThirdUiWidth($(element), $("#third-chjg-bth"));
}
...
function generateQlxx(bdcSlQlxxymDTOList) {
    if (isNotBlank(bdcSlQlxxymDTOList)) {
        for (var i = 0; i < bdcSlQlxxymDTOList.length; i++) {
            ...
            generateLzrxx(bdcSlQlxxym.bdcLzrDOList, "lzrxx" + (i + 1), i + 1, bdcSlQlxxym.bdcXm.xmid, bdcSlQlxxym.bdcXmFbDO, true);
            //加载第三权利人
...
function saveXmfbPl(formClass,djxl) {
    var bdcXmfbData = {};
    var syqx = $("input[name='syqxcq']:checked").val();
    ...
    if (bdcXmfbArray !== null && bdcXmfbArray.length > 0) {
        ...
        if(syqx === "长期" && syqxchange){
            bdcXmfbData['syqx'] = syqx;
        }
    }
...
function saveQl(formClass, className, index) {
    if (formClass !== null && formClass.length > 0) {
        ...
        if (qlData !== {}) {
            ...
            var syqx = $("input[name='syqxcq']:checked").val();
            // 林权权利界面新增选项林地使用（承包）时间为长期
            if((isNullOrEmpty(qlData.ldsyqssj))){
                qlData.ldsyqssj = '';
            }
            if((isNullOrEmpty(qlData.ldsyjssj))){
                qlData.ldsyjssj = '';
            }
            // 林权使用时间验证
            checkldsysj(syqx, qlData.ldsyqssj, qlData.ldsyjssj);
...
function queryFcjyClfHtxx(fwlx,currxmid) {
    //小弹出层
    addModel();
    generateYcslxx($("#htbh").val(), '', fwlx, currxmid);
}
...
function generateYcslxx(contractNo, index,fwlx,currxmid) {
    ...
    layer.close(index);
    $.ajax({
        url: getContextPath() + "/ycsl/fcjyxx/bdcdyh",
        type: 'GET',
...
function addQlr(qllx, dydj) {
    ...
    var url = getContextPath() + "/view/slym/qlr.html?xmid=" + addXmid + "&lclx=" + lclx + "&processInsId=" + processInsId + "&formStateId=" + formStateId + "&qllx=" + qllx + "&zxlc=" + zxlc + "&sfdrjyxx=" + sfdrjyxx + "&djxl=" + djxl;
    if (dydj) {
        url = url + "&dydj=true";
    }
    layer.open({
...
* 加载领证人信息
*/
function generateLzrxx(lzrdata, id, index, xmid, xmfb, isLoad) {
...
function loadLzrxx(id, index, xmid) {
    getReturnData("/slym/lzr/nt/lzrxx", {xmid: xmid}, 'GET', function (data) {
        generateLzrxx(data, id, index, xmid, null, false);
    }, function (err) {
        console.log(err);
    });
}
    ...
    $(".dsqlr-tr").each(function () {
        ...
        if (dsqlrArray !== null && dsqlrArray.length > 0) {
            ...
            dsqlrArray.forEach(function (item, index) {
                ...
                // 删除权利页面申请人表格中qlrmc和zjh中的空格
                if (item.name == 'qlrmc' || item.name == 'zjh') {
                    if(!isNullOrEmpty(dsqlr.qlrmc)) {
                        dsqlr.qlrmc = deleteWhitespace(dsqlr.qlrmc, 'all');
                    }
                    if (!isNullOrEmpty(dsqlr.zjh)) {
                        dsqlr.zjh = deleteWhitespace(dsqlr.zjh, 'all');
                    }
                }
            });
...
function loadYjxx(){
    ...
    console.log(djxl);
    var slbh = $('#sjbh').val();
    var url = getContextPath() + "/xuancheng/slym/yjxx.html?processInsId=" + processInsId + "&djxl=" + djxl + "&slbh=" + slbh + "&xmid=" +xmid + "&lclx=zh";
    layer.open({
        type: 2,
        area: ['960px', '300px'],
        fixed: false, //不固定
...
//组织申请书信息
function generateSqsxx(data){
    var cols = [];
    cols = [[
        {field: 'qlmc', title: '申请书类型'},
...
//新摩科宣城附件材料推送签字
function signMk(fjlx, dylx, djxl) {
    getReturnData('/pjq/mk/sign', {
            gzlslid: processInsId,
            zxlc: zxlc,
            fjlx: fjlx,
            djxl: djxl,
...
layui.use(['table'], function() {
    ...
    table.on('tool(sqsTable)', function (obj) {
        var qlxx = obj.data; //获得当前行数据
        var djxl = qlxx.bdcXm.djxl;
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        if (layEvent === 'Mk') { //推送
            addModel();
            signMk("不动产登记申请书" + djxl,"", djxl);
        }
    });
})
