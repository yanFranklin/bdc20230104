...
layui.use(['element', 'form', 'jquery', 'laydate', 'laytpl', 'layer', 'response'], function () {
    ...
    $(function () {
        ...
        // 存储打印配置的sessionKey
        // 当前页的打印类型
        var dylxArr = ["zs", "zsfb", "hst", "zdt","jtcyfb","zmdlb","plzmddy"];
        var  qllx;
        ...
        $(document).on('click', '#openZsView', function () {
            layer.open({
                ...
                resize: true,
                closeBtn: 1,
                content: '/realestate-register-ui/view/zsxx/zsView.html?zsid=' + zsid+ "&qjgldm=" + qjgldm,
                scrollbar: false,
                btn: '打印',
        ...
        // 证明单打印
        $(document).on('click', '#printZmdBtn', function () {
            var zsidArr = [];
            zsidArr.push(zsid);
            batchZmdPrint("plzmddy",zmdModelUrl, zsidArr, sessionKey)
            // 禁止提交后刷新
            return false;
        });

        // 证明单列表打印
        $(document).on('click', '#printZmdLbBtn', function () {
            var dataUrl = getIP() + "/realestate-register-ui/rest/v1.0/print/zmdlb/" + processInsId + "/xml";
            var modelUrl = zmdlbModelUrl;
            printChoice("zmdlb", "realestate-register-ui", dataUrl, modelUrl, false, sessionKey);
            // 禁止提交后刷新
            return false;
        });
    });
