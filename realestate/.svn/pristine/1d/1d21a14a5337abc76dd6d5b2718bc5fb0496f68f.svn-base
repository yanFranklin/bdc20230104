...
var ysqrgxZdList = getMulZdList("ysqrgx");
...
layui.use(['form', 'jquery', 'element', 'formSelects', 'layer', 'laytpl'], function () {
    ...
    $(function () {
        ...
        form.on('submit(submitBtn)', function(data) {
            ...
            var ysqrgx = new Array();

            $("select[name='ysqrgx']").each(function(){
                var gx = $(this).val();
                var gxmc = getGxmc(gx);
                if(gx && '' != gx){
                    ysqrgx.push(gxmc);
            ...
            val.ysqrgx = parent.getYsqrgxVal();
            if(ysqrgx && ysqrgx.length > 0){
                if (val.ysqrgx) {
                    val.ysqrgx =val.ysqrgx + "," + ysqrgx.join();
                } else {
                    val.ysqrgx = ysqrgx.join();
                }
...
window.setSearchData = function (data) {
    ...
    // 设置默认的字段模糊类型
    var mhlx = {"bdcdyhmhlx":3, "bdcqzhmhlx":0, "qlrmcmhlx":0, "qlrzjhmhlx":0, "ywrmcmhlx":0,
        "ywrzjhmhlx":0, "zlmhlx":3, "ycqzhmhlx":0, "zhlshmhlx":0, "fjhmhlx":0, "slbhmhlx":0,
        "fwbhmhlx":0, "zslshmhlx":0, "ybdcdyhmhlx":0, "yxtcqzhmhlx":0,"htbhmhlx":0};
 ...
 * 获取关系字典代码值对应的名称
 */
function getGxmc (gxdm) {
    var mc = "-1";
    var zdMapList = ysqrgxZdList.ysqrgx;
    for (i = 0; i < zdMapList.length; i++) {
        var zdMap = zdMapList[i];
        if (gxdm == zdMap.DM) {
            return  zdMap.MC;
        }
    }
