...
function saveCols(data, tableid, url) {
    $.ajax({
        type: 'POST',
        url: '/realestate-accept-ui/rest/v1.0/gxhpz/save/table?url=' + encodeURI(url) + '&tableid=' + tableid,
...
function getGxhpz(tableid) {
    var url;
    if (window.location.pathname != "/realestate-accept-ui/view/dtcx/commonCx.html") {
    ...
    $.ajax({
        type: 'GET',
        url: '/realestate-accept-ui/rest/v1.0/gxhpz/query/table?url=' + encodeURI(url) + '&tableid=' + tableid,
...
function formLoad() {
    ...
    $(".bdc-set-search").on('click', function () {
        ...
        var index = layer.open({
            ...
            content: "/realestate-accept-ui/view/gxhpz/searchPz.html?formid=" + formid + "&url=" + encodeURI(url),
            btn: ['保存', '取消'],
