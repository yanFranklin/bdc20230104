function initLjzTable() {
    layui.use(['jquery', 'layer', 'element', 'form', 'table', 'laytpl'], function () {
        ...
        var unitTableTitle = [{type: 'checkbox', fixed: 'left'},
            ...
            {field: 'zt', title: '状态', width: 120, templet: '#ljzzt', align: "center"},
...
//选择楼盘表
function showLpb(fwDcbIndex,bdcdyh) {
    var result = sfdz(fwDcbIndex);
    if(result) {
        ...
        var index = layer.open({
            ...
            content: '/building-ui/lpb/view?code=accept&fwDcbIndex=' + fwDcbIndex + "&zlcsh=" + zlcsh + '&paramJson=' + encodeURI(JSON.stringify(param)),
            cancel: function () {
                addGwc();
            }
...
function sfdz(fwDcbIndex) {
    var result = false;
    if(isNotBlank(fwDcbIndex)) {
        //先判断是否为独幢，独幢提示未找到楼盘表
        getReturnData("/bdcdyh/sfdz",{fwDcbIndex:fwDcbIndex},"GET",function (data) {
            result = data
        },function (err) {
            throw err;
        },false)
...
function getAllHsByFwdcbIndex(){
    ...
    $.ajax({
        ...
        dataType: 'json',
        async: false,
        data: {fwdcbIndexs:fwdcbIndexs.join(","),bdcdyfwlx:bdcslxztzpz.bdcdyfwlx},
        success: function (data) {
            if(data.length >0){
                checkData =data;
            }
