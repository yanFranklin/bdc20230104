...
import cn.gtmap.realestate.exchange.core.domain.exchange.QlfQlDyaqDO;
        ...
        if (!ObjectUtils.isEmpty(dataModel) && StringUtils.isNotBlank(ywh)) {
            ...
            if (null != xmDO) {
                if (CollectionUtils.isNotEmpty(qlfQlDyaqList)) {
                    for (QlfQlDyaqDO dyaqDO : qlfQlDyaqList) {
                        if (newDefault) {
                        ...
                        if (StringUtils.isNotBlank(excludeDjxl) && !Arrays.asList(excludeDjxl.split(",")).contains(xmDO.getDjxl())) {
                            LOGGER.info("抵押注销业务！djxl{}", xmDO.getDjxl());
                    ...
                    setData(dataModel, qlfQlDyaqList);
                }
            }
