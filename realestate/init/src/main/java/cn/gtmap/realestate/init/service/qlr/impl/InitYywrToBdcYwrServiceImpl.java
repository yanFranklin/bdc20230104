 ...
 */
@Service
public class InitYywrToBdcYwrServiceImpl extends InitBdcYwrAbstractService {
    ...
    public List<BdcQlrDO> initYwr(InitServiceQO initServiceQO){
        ...
        if(initServiceQO!=null && StringUtils.isNotBlank(initServiceQO.getYxmid())
            ...
            if (initServiceDTO != null && CollectionUtils.isNotEmpty(initServiceDTO.getBdcQlrList())) {
                // 通过组合业务中生成的原项目义务人信息为义务人信息赋值
                bdcQlrDOList = bdcQlrService.inheritYxmBdcYwr(initServiceDTO.getBdcQlrList(),initServiceQO.getXmid(),CommonConstantUtils.QLRLB_YWR);
            } else {
