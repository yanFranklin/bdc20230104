...
public class InitYxmToBdcYgServiceImplTest {
    @Autowired
    private InitYxmToBdcYgServiceImpl initYxmToBdcYgService;

    @Transactional(transactionManager = "transactionManager")
    @Test
    @Rollback
    @DatabaseSetup(value = "/data/InitYxmToBdcYgSetup-data.xml")
    public void initQlxx() throws Exception {
        ...
        BdcYgDO bdcYgDO = (BdcYgDO)initYxmToBdcYgService.initQlxx(initServiceQO);
        ...
        Assert.assertEquals("tdsyqr",bdcYgDO.getTdsyqr());
        Assert.assertEquals("3",bdcYgDO.getGhyt().toString());
        Assert.assertEquals("8",bdcYgDO.getFwxz().toString());
        Assert.assertEquals("645.76",bdcYgDO.getJzmj().toString());
        Assert.assertEquals("7",bdcYgDO.getMjdw().toString());
        Assert.assertEquals("65",bdcYgDO.getSzc().toString());
        Assert.assertEquals("987",bdcYgDO.getZcs().toString());
        Assert.assertEquals("65",bdcYgDO.getSzmyc());
        Assert.assertEquals("3245.0",bdcYgDO.getFttdmj().toString());
        Assert.assertEquals(sdf.parse("2018-11-13 1:45:54"),bdcYgDO.getTdsyqssj());
        Assert.assertEquals(sdf.parse("2068-11-13 1:45:54"),bdcYgDO.getTdsyjssj());
        Assert.assertEquals("987",bdcYgDO.getWlc().toString());
    }
