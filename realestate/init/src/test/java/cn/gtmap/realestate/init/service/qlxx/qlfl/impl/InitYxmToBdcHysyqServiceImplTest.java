...
public class InitYxmToBdcHysyqServiceImplTest {
    @Autowired
    private InitYxmToBdcHysyqServiceImpl initYxmToBdcHysyqService;

    @Transactional(transactionManager = "transactionManager")
    @Test
    @Rollback
    @DatabaseSetup(value = "/data/InitYxmToBdcHySetup-data.xml")
    public void initQlxx() throws Exception {
        ...
        BdcHysyqDO bdcHysyqDO = (BdcHysyqDO)initYxmToBdcHysyqService.initQlxx(initServiceQO);

        Assert.assertEquals(bdcXmDO.getSlbh(), bdcHysyqDO.getSlbh());
        Assert.assertEquals(bdcXmDO.getDjlx(), bdcHysyqDO.getDjlx());
        Assert.assertEquals(bdcXmDO.getDjyy(), bdcHysyqDO.getDjyy());
        ...
        Assert.assertEquals(sdf.parse("2018-11-13 01:45:54"),bdcHysyqDO.getSyqqssj());
        Assert.assertEquals(sdf.parse("2088-11-13 01:45:54"),bdcHysyqDO.getSyqjssj());
        Assert.assertEquals("6323.33",bdcHysyqDO.getSyzje().toString());
        Assert.assertEquals("syjbzyj",bdcHysyqDO.getSyjbzyj());
        Assert.assertEquals("syjjnqk",bdcHysyqDO.getSyjjnqk());
        Assert.assertEquals("423.22",bdcHysyqDO.getSyqmj().toString());
        Assert.assertEquals("xmmc",bdcHysyqDO.getXmmc());
