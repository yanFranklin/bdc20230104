...
//判断是否可以取回
    ...
    function forwardPlProcess(checkStatus, url, tableId, currentId) {
        ...
        if (sameNodes) {
            addModel('批量转发中');
            preformPlForward(checkStatus, url, tableId, currentId, sameNodes);
        } else {
            var logoutIndex = layer.confirm('您确定要批量转发吗？', {
                ...
                addModel('批量转发中');
                layer.close(index);
                preformPlForward(checkStatus, url, tableId, currentId);
            });
        }
    }
    ...
    function preformPlForward(checkStatus, url, tableId, currentId, sameNodes) {
        // 对于转发项目的状态进行判断
            ...
            if (sameNodes) {
                ...
                if (end) {
                    ...
                    var send = layer.open({
                        ...
                        title: '任务转发',
                        area: ['960px', '490px'],
                        content: '../view/send.html?currentId=' + currentId + '&name=' + workflow.taskId + "&isEnd=" + end + "&processInstanceId=" + workflow.processInstanceId + "&sameNodes=" + sameNodes + "&plzfkey=" + plzfkey,
                        end: function () {
...
//办结
    ...
    function addRemoveLog(data,gzlslid) {
        getReturnData("/rest/v1.0/check/addIgnoreLog", {data: data,gzlslid:gzlslid}, "GET", function () {
        }, function () {
        })
    }
