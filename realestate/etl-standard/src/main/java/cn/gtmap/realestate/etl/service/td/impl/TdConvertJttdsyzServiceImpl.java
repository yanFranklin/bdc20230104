 ...
 * @description 集体土地使用证类型土地数据转换服务
 */
@Service
public class TdConvertJttdsyzServiceImpl implements TdConvertAbstractService {

    private Logger logger = LoggerFactory.getLogger(this.getClass());
    /**
     * 不动产单元国标字样
     */
    private final static String BDCDYH_JB = "JB";
    /**
     * 土地不动产单元号后缀
     */
    ...
    public void afterPropertiesSet() throws Exception {
        TdConvertStrategyFactory.register(7, this);
    }
    ...
    public void convertTdDataAndImoprtBdcDj(TdsqQO tdsqQO) {
        ...
        try{
            // 1、生成项目信息
            List<BdcXmDO> bdcXmDOList = this.tdMapper.queryJttdsyzXmxx(tdsqQO);
            ...
            // 2、生成建设用地使用权信息
            List<BdcJsydsyqDO> bdcJsydsyqDOList = this.tdMapper.queryJttdsyzJsydsyqxx(tdsqQO);
            if(CollectionUtils.isNotEmpty(bdcJsydsyqDOList)){
                for(BdcJsydsyqDO bdcJsydsyqDO : bdcJsydsyqDOList){
                    // 土地表查询出来的BDCDYH为 DJH, 需要转换后，重新赋值BDCDYH
                    String bdcdyh = djhAndBdcdyhMap.get(bdcJsydsyqDO.getBdcdyh());
                    bdcJsydsyqDO.setBdcdyh(bdcdyh);
                }
                this.bdcEntityMapper.batchSaveSelective(bdcJsydsyqDOList);
            }

            // 3、生成查封信息
            List<BdcCfDO> bdcCfDOList = this.tdMapper.queryJttdsyzCfxx(tdsqQO);
            if(CollectionUtils.isNotEmpty(bdcCfDOList)){
                for(BdcCfDO bdcCfDO : bdcCfDOList){
                    // 土地表查询出来的BDCDYH为 DJH, 需要转换后，重新赋值BDCDYH
                    String bdcdyh = djhAndBdcdyhMap.get(bdcCfDO.getBdcdyh());
                    bdcCfDO.setBdcdyh(bdcdyh);
                }
                this.bdcEntityMapper.batchSaveSelective(bdcCfDOList);
            }

            // 4、生成抵押权信息
            List<BdcDyaqDO> bdcDyaqDOList = this.tdMapper.queryJttdsyzDyaqxx(tdsqQO);
            if(CollectionUtils.isNotEmpty(bdcDyaqDOList)){
                for(BdcDyaqDO bdcDyaqDO : bdcDyaqDOList){
                    // 土地表查询出来的BDCDYH为 DJH, 需要转换后，重新赋值BDCDYH
                    String bdcdyh = djhAndBdcdyhMap.get(bdcDyaqDO.getBdcdyh());
                    bdcDyaqDO.setBdcdyh(bdcdyh);
                }
                this.bdcEntityMapper.batchSaveSelective(bdcDyaqDOList);
            }

            // 5、生成权利人
            List<BdcQlrDO> bdcQlrDOList = this.tdMapper.queryJttdsyzQlrxx(tdsqQO);
            if(CollectionUtils.isNotEmpty(bdcQlrDOList)){
                this.bdcEntityMapper.batchSaveSelective(bdcQlrDOList);
            }

            // 6、生成证书信息
            List<BdcZsDO> bdcZsDOList = this.tdMapper.queryJttdsyzZsxx(tdsqQO);
            ...
            if(tdsqQO.getTdzt() != null && tdsqQO.getTdzt() == 5){
                ...
                // 7、生成项目证书关系
                List<BdcXmZsGxDO> bdcXmZsGxDOList = tdMapper.queryJttdsyzXmZsGxxx(tdsqQO);
    ...
    private String generateBdcdyh(String djh){
        if(StringUtils.isNotBlank(djh) && djh.indexOf(BDCDYH_JB) > -1 && djh.length() == 19 ){
