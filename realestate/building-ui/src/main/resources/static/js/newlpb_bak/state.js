...
function initState(resource){
    ...
    if(djztHtml || jyztHtml || qlztHtml){
        resource.tabDocument.find(".bdc-state-show").html(djztHtml+jyztHtml+qlztHtml);
    ...
    // refreshZoomTop(resource.tabDocument);
}
...
function getQlztState(qlzt) {
    ...
    if(qlzt){
        ...
        for(var i = 0 ;i < arr.length ;i++){
            ...
            if(arr[i] == "DJ"){
                html += "<span><i mark='"+arr[i]+"' class=\"bdc-dj\"  style='"+colorCss+"'></i>冻结</span>";
            }
            if(arr[i] == "YY"){
