...
layui.use(['table', 'laytpl', 'laydate', 'layer', 'form'], function () {
    ...
    function saveCols(data, tableid, url) {
        $.ajax({
            type: 'POST',
            url: '../rest/v1.0/gxhpz/save/table?url=' + encodeURI(url) + '&tableid=' + tableid,
    ...
    function getGxhpz(tableid) {
        var url;
        if (window.location.pathname != "../view/dtcx/commonCx.html") {
        ...
        $.ajax({
            type: 'GET',
            url: '../rest/v1.0/gxhpz/query/table?url=' + encodeURI(url) + '&tableid=' + tableid,
                ...
                for (var i = 0, len = $label.length; i < len; i++) {
                    if ($label[i].innerHTML == v.name) {
                        ...
                        if ($seniors.children().length > 0) {
                            ...
                            if ($senior && $senior.find('.layui-inline').length == 0) {
                                if ($senior) {
                                    $senior.hide();
                                }
    ...
    function formLoad() {
        if ($('.bdc-search-box').length > 0) {
            ...
            $searchBox.each(function (i) {
                $($searchBox[i]).append('<a href="javascript:;" title="自定义配置查询条件" class="bdc-set-search" style="position: absolute;top: 10px;right: 10px;">' +
                    '<img src="../lib/bdcui/images/setSearch.png" alt="" style="width: 28px;">' +
                    '</a>');
            });
        }
        var forms = $('.bdc-search-box').find('form');
        ...
        for (var i = 0; i < forms.length; i++) {
            ...
            $.ajax({
                type: 'GET',
                url: '../rest/v1.0/gxhpz/query/form?url=' + encodeURI(url) + '&formid=' + formid,
                async: false,
                success: function (data) {
                    if (data) {
        ...
        $(".bdc-set-search").on('click', function () {
            ...
            if (sameFlag == false) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '../rest/v1.0/gxhpz/save/form?url=' + encodeURI(url) + '&formid=' + formid,
