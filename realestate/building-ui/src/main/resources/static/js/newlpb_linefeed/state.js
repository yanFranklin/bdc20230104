...
function initState(resource) {
    ...
    var colorConfig = info.colorConfig;
    var state = info.htmlState;
    var ztHtml = "";
    if (state) {
        var code = $("#code").val();
        var zt = state[code];
        if (zt) {
            var stateArry = zt.split(",");
            if (stateArry) {
                if (colorConfig) {
                    for (var i in stateArry) {
                        ztHtml += getZtHtml(colorConfig, stateArry[i])
                    }
                } else {
                    ztHtml = defaultStateHtml(info, resource);
                }
            }
        } else if (colorConfig) {
            //根据颜色配置的数据展现全部
            ztHtml = getZtHtml(colorConfig, "");
        } else {
            ztHtml = defaultStateHtml(info, resource)
        }
    } else {
    ...
    if (ztHtml) {
        resource.tabDocument.find(".bdc-state-show").html(ztHtml);
...
function getQlztState(qlzt) {
    ...
    if (qlzt) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (arr[i] == "SD") {
                html += "<span><i mark='" + arr[i] + "' class=\"bdc-sd\"  style='" + colorCss + "'></i>锁定</span>";
            }
            if (arr[i] == "YY") {
...
function getZtHtml(colorConfigs, state) {
    var html = "";
    var allStateHtml = "";
    var statusList = [];
    //如果状态不为空，优先展示配置的状态，否则展示所有颜色配置页面的状态
    for (var i = 0; i < colorConfigs.length; i++) {
        //如果有状态优先判断状态
        if (state) {
            if (colorConfigs[i].title === state) {
                statusList.push(colorConfigs[i].status)
            }
        } else {
            //否则取颜色配置的所有状态
            var stateHtml = "";
            for (var k in colorConfigs[i].status) {
                var colorCss = getStateColorCss(colorConfigs[i].status[k].split("-")[1].toUpperCase());
                stateHtml += "<span><i class='" + colorConfigs[i].status[k] + "' style='" + colorCss + "'></i>" + k + "</span>";
            }
            if (stateHtml) {
                allStateHtml += "<p>" + colorConfigs[i].title + "：" + stateHtml + "</p>";
            }
        }
    }
    if (statusList.length > 0) {
        for (var j = 0; j < statusList.length; j++) {
            for (var m in statusList[j]) {
                var colorCss = getStateColorCss(statusList[j][m].split("-")[1].toUpperCase());
                html += "<span><i class='" + statusList[j][m] + "' style='" + colorCss + "'></i>" + m + "</span>";
            }
        }
        if (html) {
            if (state) {
 ...
 * @description 楼盘表页面默认展现的色块和状态
 * @date : 2021/4/28 9:53
 */
function defaultStateHtml(info, resource) {
    var ztHtml = "";
    var djzt = "";
    var qlzt = "";
    var jyzt = "";
    ...
    ztHtml = djztHtml + jyztHtml + qlztHtml + fzztHtml + baztHtml;
    return ztHtml;
}