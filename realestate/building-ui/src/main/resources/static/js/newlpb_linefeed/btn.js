    ...
    $.each(info, function () {
        if (this.type == 'BUTTON') {
            if(resource.checkHasTab(this.tabType)){
                ...
                if (this.value === "generateZsBtn" || this.value === "generateZmBtn" || this.value === "noFaZhengBtn") {
                    var paramJson = JSON.parse(param.replace(/\'/g, "\""));
                    var gzldyid = paramJson.gzldyid;
                    if (isNotBlank(btnConfig)){
                        var value = btnConfig[this.value];
                        var index = -1;
                        if (isNotBlank(value)){
                            index = value.indexOf(gzldyid);
                        }
                        if (index !== -1){
                            this.btnClass += '  bdc-hide';
                        }
                    }
                    if (paramJson) {
                        if (paramJson.zdjbdb === true) {
                            //只登记不登簿流程，隐藏证书，证明书，只登簿不发证按钮
                            this.btnClass += '  bdc-hide';
                        }
                    }
                }
                if (this.value === "djbdbBtn") {
                    var paramJson = JSON.parse(param.replace(/\'/g, "\""));
                    if (paramJson) {
                        if (paramJson.zdjbdb !== true) {
                            this.btnClass += '  bdc-hide';
...
//查看宗地量化信息
function ckZdLhxxFunction(){
    var djh = $("#djh").val();
    window.open("/building-ui/zdjsydlhxx/view?djh=" + djh);

}
//查看宗地量化信息
function ckTdlpbFunction(){
    var djh = $("#djh").val();
    //window.open("/building-ui/tdlpb/view?djh=" + djh);
    toView(getIP() + '/tdlpb/view?djh='  + djh, {tabname: "土地楼盘表"});
...
function hshbFunction() {
    layui.use(['jquery', 'laytpl', 'element', 'table'], function () {
        if (currentRes.checkboxcheckedList && currentRes.checkboxcheckedList.length > 1) {
            if (currentRes.checkboxcheckedList.length > 2) {
                ityzl_SHOW_WARN_LAYER("请选择连续的两个户室进行合并!")
                return false;
            }
            var data = [];
            //把带-的字符串转为数组
            ...
            var checkResult = hbCheck(data);
            if (checkResult.result) {
                $.hsbg.hbfx = checkResult.hbfx;
                $.hsbg._fwhsHb(data);
            } else {
                layer.alert(checkResult.msg);
            }
        } else {
        ...
        var index = layer.open({
            ...
            success: function () {
                ...
                form.on('select(hssl)', function (data) {
                    ...
                    // initBtns(currentRes);
                    getReturnData("/lpb/hssl", {hssl: data.value}, "GET", function (data) {
