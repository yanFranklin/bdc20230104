...
import cn.gtmap.realestate.etl.core.dozer.BdcDmToDsfZdComponent;
import cn.gtmap.realestate.etl.core.dozer.DsfZdToBdcDmComponent;
...
import cn.gtmap.realestate.etl.web.rest.HlwCreateRestController;
...
public class HlwYwxxServiceImpl implements HlwYwxxService {
    ...
    @Resource(name = "hlwRepository")
    private Repository hlwRepository;
    ...
    @Autowired
    HlwCreateRestController hlwCreateRestController;
    @Autowired
    ...
    public Page<Map> listHlwYwxxByPage(Pageable pageable, WwsqQO wwsqQO) {
        ...
        if (Objects.nonNull(currentUser) && CollectionUtils.isNotEmpty(currentUser.getOrgRecordList())) {
            List<String> regionCodes = currentUser.getOrgRecordList().stream()
                    .filter(organizationDto -> Objects.nonNull(organizationDto.getRegionCode()))
                    .map(OrganizationDto::getRegionCode).collect(Collectors.toList());

            //对区县代码进行转换 --> 转换失败则使用原值
            regionCodes = regionCodes.stream().map(item -> Optional
                    .ofNullable(bdcDmToDsfZdComponent.conver("DSF_ZD_QXDM", "wwsq", item))
                    .map(Object::toString).orElse(""))
                    .collect(Collectors.toList());

            if (CollectionUtils.isNotEmpty(regionCodes)) {
                wwsqQO.setQxdmList(regionCodes);
            }
...
//            hlwYwxxDataService.updateCjjl(jsonObject.getString("xmid"),jsonObject.getString("wwslbh"),jsonObject.getString("gzlslid"),jsonObject.getString("byslyy"));
        ...
        //更新申请项目
        GxWwSqxmDo gxWwSqxmDo = JSON.parseObject(JSON.toJSONString(jsonObject), GxWwSqxmDo.class);
        hlwEntityMapper.updateByPrimaryKeySelective(gxWwSqxmDo);
    }
    ...
    public void modifyHlwShzt(String processInsId) {
        ...
        if (CollectionUtils.isNotEmpty(bdcXmDOList)) {
            ...
            if (CheckWwsqOrYcslUtils.checkIsWwsq(bdcXmDO.getSply())) {
                ...
                hlwEntityMapper.updateByExampleSelectiveNotNull(gxWwSqxmDo, example);
            }
        }
    }
