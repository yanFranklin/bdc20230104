...
//涉税审批来源 1:内网一窗 5外网一窗 11一窗通办系统涉税 21：税费托管系统
var sply_ss =[1,5,11,21];
...
function addCxtj() {
    ...
    $(".cxtj-area").each(function(){
        if ($(this).attr("name")=="zl" || $(this).attr("name")=="bdcqzh"){
            $(this).append('<div class="bdc-screen-inline bdc-mhcx">' +
                '<select  name=' + $(this).attr("name") + 'mhlx'+' class="bdc-filter-select search">' +
    ...
    $(".cxtj-input").css("padding-left","70px");
}

function addbdcdyCxtj() {

    $(".cxtj-area").each(function(){
        if ($(this).attr("name")=="zl" || $(this).attr("name")=="bdcqzh"){
            $(this).append('<div class="bdc-screen-inline bdc-mhcx">' +
                '<select  name=' + $(this).attr("name") + 'mh' + ' class="bdc-filter-select search">' +
...
//根据证件号获取年龄
function getAge(zjh){
    if(isNullOrEmpty(zjh) ||zjh.length !== 15 &&zjh.length !==18){
        return null;
    }
    var age;
    ...
    var birthyear;
    if (zjh.length === 18){
        birthyear = zjh.substring(6, 10);
    }
    if(zjh.length ===15){
        birthyear = "19" + zjh.substring(6, 8);
    }
...
function getSex(zjh){
    ...
    var xbtzm;
    if (zjh.length === 18){
        xbtzm = zjh.substring(16, 17);
    }
    if(zjh.length ===15){
        xbtzm = zjh.substring(14, 15);
    }
    if (xbtzm % 2 == 1) {
        return 1;
    } else {
