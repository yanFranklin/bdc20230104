...
;layui.define(["laytpl", "laypage", "layer", "form", "util"], function (e) {
    ...
    F.prototype.config = {
        ...
        i.config.data && i.config.data.constructor === Array && delete i.config.data, i.config = t.extend({},i.config, e),
            i.config.loadTotal=c.config[ i.config.id].loadTotal,
            i.config.request.loadTotal=c.config[ i.config.id].loadTotal, i.render()
        ...
        if (i.startTime = (new Date).getTime(), a.url) {
            var r = {};
            //lst 添加属性传值
            if(l.hasOwnProperty("loadTotal")){
                a.loadTotal=l.loadTotal;
                a.loadTotalBtn=l.loadTotalBtn;
            }
            r["loadTotal"]=a.loadTotal,r[l.pageName] = e, r[l.limitName] = a.limit;
            ...
            a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), t.ajax({
                ...
                error: function (e, t) {
                    "function" == typeof a.done && a.done(t);
                    i.layMain.html('<div class="' + f + '">数据接口请求异常：' + t + "</div>"), i.renderForm(), i.setColsWidth();
                    removeModal();
                }
            })
        } else if (a.data && a.data.constructor === Array) {
            var c = {}, s1 = e * a.limit - a.limit;
            c[n.dataName] = a.data.concat().splice(s1, a.limit), c[n.countName] = a.data.length, i.renderData(c, e, a.data.length), o(), i.setColsWidth(), "function" == typeof a.done && a.done(c, e, c[n.countName]);
            removeModal();
        }
        ...
        return c.key = s.id || s.index, d.cache[c.key] = u, c.layPage[0 == o || 0 === u.length && 1 == n ? "addClass" : "removeClass"](y), r ? m() : 0 === u.length ? (c.renderForm(), c.layFixed.remove(), c.layMain.find("tbody").html(""), c.layMain.find("." + f).remove(), c.layMain.append('<div class="' + f + '">' + s.text.none + "</div>")) : (m(), c.renderTotal(u), void(s.page && (s.page = t.extend({
            ...
            groups: s.loadTotal ? 3: 1,//lst 根据loadTotal属性生成不同样式
            layout: s.loadTotal ?
                s.loadTotalBtn ? ["prev", "page", "next", "skip", "count", "limit", "loadTotalBtn"] : ["prev", "page", "next", "skip", "count", "limit"]
                : s.loadTotalBtn ? ["prev", "currentPage", "next","limit","loadTotalBtn"] :["prev", "currentPage", "next","limit"]  ,
                    ...
                    s({
                        list: function () {
                            ...
                            n.on("checkbox(LAY_TABLE_TOOL_COLS)", function (e) {
                                ...
                                if (e.elem.ownerDocument.location.pathname != "/realestate-inquiry-ui/view/dtcx/commonCx.html") {
                                    saveCols(o.cols[0], o.id, e.elem.ownerDocument.location.pathname);
                                } else {
                                    saveCols(o.cols[0], o.id, e.elem.ownerDocument.location.pathname + e.elem.ownerDocument.location.search);
                                }
                            })
                        }
                    });
        ...
        if(i.hasOwnProperty("page")){
            //lst 刷新时传值
            var page=i.page;
            if(page.hasOwnProperty("loadTotal")){
                a.loadTotal=i.page.loadTotal;
                a.request.loadTotal=i.page.loadTotal;
            }
            if(page.hasOwnProperty("layout")){
                a.page.layout=i.page.layout;
            }
        }
