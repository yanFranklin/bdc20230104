...
layui.use(['jquery', 'form', 'laytpl','element','table'], function () {
    ...
    $(function () {
        ...
        if(getNum != null && getNum.toString().length > 1){
            ...
            for(var i = 0; i < $('.accordion .bdc-outer-li').length; i++){
                ...
                if($($('.accordion .bdc-outer-li')[i]).data('num') == getNum){
                    ...
                    $('.bdc-tab-content iframe').attr('src',$('.accordion .bdc-outer-li:nth-child('+ nowI +') .link').data('src'));
                    break;
                }
            }
            // console.log(liIndex);
        ...
        $(document).on('click', '.link', function () {
            ...
            $('#childFrame').attr('src',$this.parent().data('src'));
        });
        //1.2  对于逻辑栋点击高亮
        ...
        $('.bdc-operate-show').on('click',function () {
            ...
            if(stateIndex % 2 == 0){
                ...
                $('.bdc-outer-box').animate({'padding-top':'50px'},100);

            }else {
                //展示
                $('.bdc-lc-container').animate({"height": '300px'},100);
                $('.bdc-outer-box').animate({'padding-top':'310px'},100);
            }
        ...
        $("#asideLine").mousedown(function (e) {
            is_mouse_down = true;
        ...
        $(document).bind("mouseup click",function () {
            ...
            var iEvent = ev||event;
            if (is_mouse_down){
                oBox.style.width =  iEvent.clientX + 25 + 'px';
                oLine.style.left = iEvent.clientX - 4 + 25 + 'px';
                $zoom.css('left',iEvent.clientX + 25 - 1);
                if(oBox.offsetWidth <= 280){
                    oBox.style.width = '280px';
                    oLine.style.left = oBox.offsetWidth - 4 + 'px';
                    $zoom.css('left','280px');
