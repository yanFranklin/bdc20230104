...
public class DtcxConfigServiceImpl implements DtcxConfigService {
    ...
    public void saveAllDtcxCxxx(ZrzyDtcxDTO zrzyDtcxDTO, boolean fuzhiCx) {
        // 保存查询
        DtcxDO dtcxDO = new DtcxDO();
        BeanUtils.copyProperties(zrzyDtcxDTO, dtcxDO);
        ...
        if (CollectionUtils.isNotEmpty(zrzyDtcxDTO.getCxtjDOList())) {
            ...
            entityMapper.insertBatchSelective(zrzyDtcxDTO.getCxtjDOList());
        }

        // 保存查询结果
        if (CollectionUtils.isNotEmpty(zrzyDtcxDTO.getCxjgDOList())) {
            // 先删除所有结果再重新插入
            Example example = new Example(DtcxCxjgDO.class);
            example.createCriteria().andEqualTo(DtcxConstants.CXID, cxid);
            entityMapper.deleteByExample(example);

            for (DtcxCxjgDO dtcxCxjgDO : zrzyDtcxDTO.getCxjgDOList()) {
            ...
            // 之前需要全部删除所有记录，此处再统一插入
            entityMapper.insertBatchSelective(zrzyDtcxDTO.getCxjgDOList());
        }
    }
    ...
    @Override
    public Page<ZrzyDtcxDTO> listDtcxPage(ZrzyDtcxQO qo, Pageable pageable) {
        return repo.selectPaging("listDtcxByPage", qo, pageable);
    }

    @Override
    public ZrzyDtcxDTO getConfigsByCxdh(String cxdh) {

        ZrzyDtcxDTO zrzyDtcxDTO;
        zrzyDtcxDTO = dtcxMapper.getDtcxByCxdh(cxdh);
        if (zrzyDtcxDTO != null) {
            zrzyDtcxDTO.setCxtjDOList(dtcxMapper.getCxtjConfig(cxdh));
            zrzyDtcxDTO.setCxjgDOList(dtcxMapper.getCxjgConfig(cxdh));
        }
        return zrzyDtcxDTO;
    }
}
