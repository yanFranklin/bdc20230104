...
import cn.gtmap.realestate.natural.common.BeanHelper;
...
@RestController
@Api(tags = "自定义查询")
public class ZrzyDtcxRestController extends BaseController implements ZrzyDtcxRestService {
    /**
     * 是否将中文括号转换为英文括号,默认转换
     */
    ...
    @Override
    public void saveAll(@RequestBody ZrzyDtcxDTO dtcxDO, boolean fuzhiCx) {
    ...
    @Override
    public void saveConfig(@RequestBody ZrzyDtcxDTO bdcDtcxDTO) {
        dtcxConfigService.saveAllDtcxCxxx(bdcDtcxDTO, false);
    }

    @Override
    public Page<ZrzyDtcxDTO> listDtcxPage(@RequestBody ZrzyDtcxQO dtcxQO, int page, int size, Sort sort) {
    ...
    public Page listResult(@RequestParam("dataString") String dataString, int page, int size, Sort sort) {
        Pageable pageable = new PageRequest(page, size, sort);
        ...
        if (dataMap.containsKey("executeSql")) {
            ...
            if (StringUtils.isNotBlank(paramValue)) {
                sql = sql.replace("#{executeSql}", paramValue);
            }
        }

        LOGGER.info("自定义查询语句:{}", sql);
        dataMap.put("sql", sql);
        return viewService.listResultByPage(pageable, sql, dataMap);
    }

    @Override
    public ZrzyDtcxDTO getCxxxByCxdh(@PathVariable("cxdh") String cxdh) {
    ...
    @Override
    public List<Map> listResultData(@RequestParam("dataString") String dataString) {
        // 将json 数据转换为map
        Map dataMap = new HashMap();
        if (org.apache.commons.lang3.StringUtils.isNotBlank(dataString)) {
            JSONObject jsonData = JSONObject.parseObject(dataString);
        ...
        String sql = viewService.getSqlByCxxx(dataMap);
        sql = sql.replaceAll("\n", " ").trim();
        if (StringUtils.isBlank(sql)) {
            throw new AppException("组织sql发生错误！");
        }
        dataMap.put("sql", sql);
        return viewService.listResultData(dataMap);
    }
