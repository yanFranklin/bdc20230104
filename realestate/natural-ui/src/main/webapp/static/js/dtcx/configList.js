layui.use(['layer', 'table', 'jquery', 'form'], function () {
    ...
    var form = layui.form;
    /** 配置项*/
    var configUrl = '/realestate-natural-ui/dtcx/config';
    var dataUrl = '/realestate-natural-ui/dtcx/list/page';
...
function importFile(fileObj) {
    ...
    $("#uploadFile").ajaxSubmit({
        type: 'post',
        url: '/realestate-natural-ui/dtcx/config/import/excel',
...
function seeInfo(data) {
    var url = getIP() + "/realestate-natural-ui/view/dtcx/commonCx.html?cxdh=" + data.cxdh + "&cxmc=" + encodeURI(data.cxmc);
    window.open(encodeURI(url));
}

function exportToExcel(data) {
    var url = "/realestate-natural-ui/dtcx/config/export/excel?cxdh=" + data.cxdh;
    window.open(url);
}
...
function delOne(obj) {
    layui.layer.confirm("该操作不可恢复，确认删除？", {
        ...
        btn1: function (index) {
            ...
            $.ajax({
                url: '/realestate-natural-ui/dtcx/del/' + cxid,
                type: 'delete',
                success: function (data) {
                    layer.msg("删除成功");
                    obj.del();
