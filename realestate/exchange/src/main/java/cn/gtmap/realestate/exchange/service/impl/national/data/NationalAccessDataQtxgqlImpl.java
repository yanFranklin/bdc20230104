...
public class NationalAccessDataQtxgqlImpl extends GxDataDbByPkServiceImpl implements NationalAccessDataService, QlfQlQsztService {
    ...
    public DataModel getAccessDataModel(String ywh, DataModel dataModel) {
        if (!ObjectUtils.isEmpty(dataModel) && StringUtils.isNotBlank(ywh)) {
            ...
            if (CollectionUtils.isNotEmpty(qlfQlQtxgqlList)) {
                if (CollectionUtils.size(qlfQlQtxgqlList) > 1) {
                    sendMsg(ywh);
                }
                List<QlfQlQtxgqlDO> qlfQlQtxgqlDOList = Collections.singletonList(qlfQlQtxgqlList.get(0));
                if (newDefault) {

                    for (QlfQlQtxgqlDO qlfQlQtxgqlDO : qlfQlQtxgqlDOList) {
                        if (qxdmConvert) {
                            //需要根据qxdm进行对照
                            qlfQlQtxgqlDO.setQxdm(commonService.bdcDmToDsfZdNvl("ACCESS_QXDM", "ACCESS", qlfQlQtxgqlDO.getQxdm()));
                        }
                        if (turnOnReplacezf && StringUtils.isNotBlank(qlfQlQtxgqlDO.getBdcqzh())) {
                            qlfQlQtxgqlDO.setBdcqzh(qlfQlQtxgqlDO.getBdcqzh().replaceAll(CommonConstantUtils.ZF_YW_DH, replaceZf));
                        }
                        ClassHandleUtil.setDefaultValueToNullField(qlfQlQtxgqlDO);
                    }
                }
                setData(dataModel, qlfQlQtxgqlDOList);
            }
        }
        return dataModel;
    }
    ...
    public DataModelOld getAccessDataModelOld(String ywh, DataModelOld dataModel) {
        if (!ObjectUtils.isEmpty(dataModel) && StringUtils.isNotBlank(ywh)) {
            Map<String, String> map = Maps.newHashMap();
            map.put("ywh", ywh);
            List<QlfQlQtxgqlOldDO> qlfQlQtxgqlList = qlfQlQtxgqlMapper.queryQlfQlQtxgqlListOld(map);
            if (CollectionUtils.isEmpty(qlfQlQtxgqlList)) {
                map.clear();
                map.put("yywh", ywh);
                qlfQlQtxgqlList = qlfQlQtxgqlMapper.queryQlfQlQtxgqlListOld(map);
            }
            if (CollectionUtils.isNotEmpty(qlfQlQtxgqlList)) {
                setDataOld(dataModel, qlfQlQtxgqlList);
            }
        }
        return dataModel;
    }
