...
public class NationalAccessDataYhydzbImpl extends GxDataDbByPkServiceImpl implements NationalAccessDataService {
            ...
            if (CollectionUtils.isNotEmpty(ktfZhYhydzbList)) {
                if (newDefault) {
                    ...
                    for (KtfZhYhydzbDO ktfZhYhydzbDO : ktfZhYhydzbList) {
                        if (qxdmConvert) {
                            //需要根据qxdm进行对照
                            ktfZhYhydzbDO.setQxdm(commonService.bdcDmToDsfZdNvl("ACCESS_QXDM", "ACCESS", ktfZhYhydzbDO.getQxdm()));
                        }
    ...
    public DataModelOld getAccessDataModelOld(String ywh, DataModelOld dataModel) {
        if (!ObjectUtils.isEmpty(dataModel) && StringUtils.isNotBlank(ywh)) {
            Map<String, String> map = Maps.newHashMap();
            map.put("ywh", ywh);
            List<KtfZhYhydzbOldDO> ktfZhYhydzbList = ktfZhYhYdzbMapper.queryKtfZhYhYdzbListOld(map);
            if (CollectionUtils.isNotEmpty(ktfZhYhydzbList)) {
                setDataOld(dataModel, ktfZhYhydzbList);
            }
        }
        return dataModel;
    }
