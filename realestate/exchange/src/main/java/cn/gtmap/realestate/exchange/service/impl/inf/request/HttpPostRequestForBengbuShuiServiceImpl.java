 ...
 * @description 蚌埠供水post请求，不加密,form-data格式请求
 */
@Service(value = "httpPostForBengbuShui")
public class HttpPostRequestForBengbuShuiServiceImpl extends InterfaceRequestService<HttpReqPropBO> {
    ...
    public void sendRequest(InterfaceRequestBuilder builder) {
        ...
        LOGGER.info("蚌埠供水接口请求requestBody:" + JSONObject.toJSONString(requestBody));
        ...
        if (CheckParameter.checkAnyParameter(requestBody) && StringUtils.isNotBlank(prop.getUrl())) {
            ...
            LOGGER.info("蚌埠供水paramMap：" + JSONObject.toJSONString(paramMap));
            String response = "";
            Exception logE = null;
            ...
            try {
                LOGGER.info("蚌埠供水开始执行忽略证书http请求，方法为：{}", httpPost.getMethod());
                RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(30000).setConnectTimeout(30000).build();
                httpPost.setConfig(requestConfig);
                response = httpClientService.doPost(httpPost, "UTF-8");
                LOGGER.info("蚌埠供水response：{}", response);
            } catch (Exception e) {
                logE = e;
                LOGGER.error("蚌埠供水httpPost 请求异常：url:{},reqMap:{}", prop.getUrl(), JSONObject.toJSONString(requestBody), e);
                throw new AppException("蚌埠供水httpPost 请求异常");
            } finally {
                try {
