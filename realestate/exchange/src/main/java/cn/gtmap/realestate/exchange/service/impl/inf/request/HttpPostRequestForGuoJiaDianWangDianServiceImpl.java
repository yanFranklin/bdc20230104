 ...
 * @description 针对国家电网电相关接口请求处理，只针对国家电网
 */
@Service(value = "httpPostForGuoJiaDianWangDian")
public class HttpPostRequestForGuoJiaDianWangDianServiceImpl extends InterfaceRequestService<HttpReqPropBO> {
     ...
     * 分配的appId
     */
    @Value("${guojiadianwang.dian.appId:}")
    private String appid;

    /**
     * 私钥
     */
    @Value("${guojiadianwang.dian.secret:}")
    private String secret;

    /**
     * RSA公钥
     */
    @Value("${guojiadianwang.dian.rasPublicKey:}")
    private String rasPublicKey;

    @Value("${guojiadianwang.dian.needHead: false}")
     ...
     * 不需要加密的参数字段
     */
    private static String[] UNENCRYPT_PARAMS = {"imageData"};

    @Override
    public void sendRequest(InterfaceRequestBuilder builder) {
        Object requestBody = builder.getRequestBody();
        LOGGER.info("国家电网电接口请求requestBody:" + JSONObject.toJSONString(requestBody));
        ...
        if (CheckParameter.checkAnyParameter(requestBody) && StringUtils.isNotBlank(prop.getUrl())) {
            ...
            if (CollectionUtils.isNotEmpty(requestParamMap.keySet())) {
                for (String key : requestParamMap.keySet()) {
                    if (Arrays.asList(HEADER_PARAMS).contains(key)) {
                        ...
                        try {
                            //判断是否不需要加密
                            if (Arrays.asList(UNENCRYPT_PARAMS).contains(key)) {
            ...
            LOGGER.info("国家电网paramMap：" + JSONObject.toJSONString(paramMap));
            String response = "";
            Exception logE = null;
           ...
           /* String body = JSONObject.toJSONString(paramMap);
            ...
            try {
                LOGGER.info("国家电网开始执行忽略证书http请求，方法为：{}", httpPost.getMethod());
                RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(30000).setConnectTimeout(30000).build();
                httpPost.setConfig(requestConfig);
                response = httpClientService.doPost(httpPost, "UTF-8");
                LOGGER.info("国家电网response：{}", response);
            } catch (Exception e) {
                logE = e;
                LOGGER.error("国家电网httpPost 请求异常：url:{},reqMap:{}", prop.getUrl(), JSONObject.toJSONString(requestBody), e);
                throw new AppException("国家电网httpPost 请求异常");
            } finally {
                try {
