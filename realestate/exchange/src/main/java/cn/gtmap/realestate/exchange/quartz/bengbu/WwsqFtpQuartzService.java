package cn.gtmap.realestate.exchange.quartz.bengbu;
 ...
 * @description 蚌埠外网申请FTP文件监听定时任务
 * <p>
 * 背景说明：
 * 蚌埠因为硬件网闸，互联网+创件后将信息包装成压缩包放到中间文件服务器，登记监控该目录文件，
 * 提取内容进行登记系统创件，创件完成后同样需要把返回信息包装成压缩文件放到中间文件服务器
 */
@Service
public class WwsqFtpQuartzService {
    private static final Logger LOGGER = LoggerFactory.getLogger(WwsqFtpQuartzService.class);
     ...
     */
    @Value("${bengbu.wwsqftp.enable:false}")
    private Boolean openWwsqFtpListener;

    /**
     * 是否开启
     */
    @Value("${bengbu.wwsqftp.send.msg.enable:false}")
    private Boolean openWwsqFtpSendMsg;

    /**
     * 外网申请FTP压缩文件存放服务器 IP username password port
     */
    @Value("${bengbu.wwsqftp.ip:}")
    private String ip;

    @Value("${bengbu.wwsqftp.username:}")
    private String username;

    @Value("${bengbu.wwsqftp.password:}")
    private String password;

    @Value("${bengbu.wwsqftp.port:}")
    private String port;

    /**
     * 外网申请FTP压缩文件在FTP服务器存放路径
     */
    @Value("${bengbu.wwsqftp.ftpFilePath:}")
    private String ftpFilePath;

    /**
     * 登记处理完成后FTP文件移动到已完成目录路径
     */
    @Value("${bengbu.wwsqftp.hasResolvedFtpFilePath:}")
    private String hasResolvedFtpFilePath;

    /**
     * 登记创件完成返回信息文件在FTP服务器存放路径
     */
    @Value("${bengbu.wwsqftp.responseFilePath:}")
     ...
     * 文件编码方式
     */
    @Value("${bengbu.wwsqftp.encoding:GBK}")
    ...
    @Value("${bengbu.wwsqftp.receive.msg.role:admin}")
    private String receiveMsgRole;

    @Value("${bengbu.wwsqftp.msg.content:已完成互联网+创建任务,检测出%s个问题,请查看!}")
     ...
     */
    @Scheduled(cron = "${bengbu.wwsqftp.cron:0 0 2 1 * ?}")
    ...
    public void listenWwsqFtpFiles() {
        if (!openWwsqFtpListener || StringUtils.isBlank(ftpFilePath)) {
            LOGGER.debug("{}处理结束，因为未开启此功能、或连接FTP失败、或未定义文件目录信息", FUNCTION);
            return;
        }
