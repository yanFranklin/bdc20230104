...
layui.use(['jquery', 'layer', 'element', 'form', 'formSelects', 'table', 'laytpl', 'laydate'], function () {
    ...
    $(function () {
        ...
        if (type === "national") {
            ...
            accessLogUrl = '../accessLog/getAccessLogPagesJson?wlxm=true';
        } else if (type === "city") {
        ...
        table.on('tool(dataTable)', function (obj) {
            ...
            if (data) {
                ...
                if (obj.event === "sjjy") {
                    if (data.xmid) {
                        $.ajax({
                            url: "../accessLog/checkWlxmData",
                            dataType: "json",
                            data: {
                                xmid: data.gxid + "," + data.xmid
                            },
                            success: function (data) {
                                if (data) {
                                    layer.msg("校验成功");
                ...
                if (obj.event === "sblct") {
                    //根据xmid 查看上报数据走向
                    window.open('/realestate-exchange/view/sblct.html?xmid=' + data.gxid);
                }
            } else {
        ...
        function getRedisVal() {
            ...
            $.ajax({
                url: "../rest/v1.0/access/redisval?redisKey=WLXMSJJRHJPLSB",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data) {
