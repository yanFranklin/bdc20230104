...
public class DtcxCxtjDO {
    ...
    /**
     * 设置“是”，在Excel导入查询时候，对应字段在页面输入的查询条件会和Excel里的查询条件一起查询，
     * 如果查询字段冲突，则以Excel中的查询条件值为准
     */
    @ApiModelProperty("导入查询保留条件")
    private Integer drcxbl;

    @ApiModelProperty("是否加密查询")
    private Integer encrypt;

    /**
     * 是 ：每次读卡数据会追加输入框已有数据
     * 否 ：每次读卡会清空输入框数据并设置当前读卡内容
     */
    @ApiModelProperty("读卡器数据追加")
    private Integer dkqsjzj;

    @ApiModelProperty("默认值")
    private String mrz;


    public Integer getDkqsjzj() {
        return dkqsjzj;
    }

    public void setDkqsjzj(Integer dkqsjzj) {
        this.dkqsjzj = dkqsjzj;
    }
    ...
    public Integer getDrcxbl() {
        return drcxbl;
    }

    public void setDrcxbl(Integer drcxbl) {
        this.drcxbl = drcxbl;
    }
