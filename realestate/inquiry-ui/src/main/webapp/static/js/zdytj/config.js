...
layui.use(['layer', 'table', 'jquery', 'form', 'element', 'upload'], function () {
    ...
    $(function () {
        ...
        if (!isNullOrEmpty(cxdh)) {
            ...
            dhPassCheck = true;
            $.ajax({
                url: "/realestate-inquiry-ui/zdytj/get/" + cxdh,
                type: "get",
                async: true,
...
function checkSql(sql) {
    ...
    if (sql.indexOf("update") >= 0 || sql.indexOf("delete") >= 0 || sql.indexOf("insert") >= 0 || sql.indexOf("truncate") >= 0 || sql.indexOf("drop") >= 0) {
        ...
        return false;
    }else if (sql.indexOf("sl1") == -1 || sql.indexOf("mc") == -1) {
        layui.layer.msg("请按照模板配置sql");
        return false;
    } else {
        var result = false;
        $.ajax({
            url: "/realestate-inquiry-ui/zdytj/checksql",
            type: "post",
            async: false,
            data: {
                cxsql: sql,
            ...
            success: function (data) {
                if(data == true){
                    ...
                    layui.layer.msg("sql校验失败，请检查sql");
                    return false;
                }
            },
            error: function (e) {
...
function saveAction() {
    ...
    if (checkSql(cxsql)) {
        if (dhPassCheck) {
            $.ajax({
                url: '/realestate-inquiry-ui' + "/zdytj/save/all",
                type: "post",
                async: true,
                data: {
                    cxsql: cxsql,
...
function checkCxdhIsUsed(cxdh) {
    var url = "/realestate-inquiry-ui/zdytj/check/cxdh?cxdh=" + cxdh;
    addModel();
    $.ajax({
        url: url,
        type: 'get',
...
function getUrlFromDh() {
    ...
    // if (dhPassCheck) {
    var url = "柱状图 http://{ip}:{port}/realestate-inquiry-ui/view/zdytj/echart-category.html?cxdh=" + dh  + "饼状图 http://{ip}:{port}/realestate-inquiry-ui/view/zdytj/echart-pie.html?cxdh=" + dh ;
    $("#url").val(url);
    layer.msg("获取成功");
    // } else {
    //     layer.msg("请先检查查询代号是否可用");
