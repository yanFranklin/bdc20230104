layui.use(['layer', 'table', 'jquery', 'form'], function () {
    ...
    var form = layui.form;
    /** 配置项*/
    var configUrl = '/realestate-inquiry-ui/zdytj/config';
    var dataUrl = '/realestate-inquiry-ui/zdytj/list/page';
...
function importFile(fileObj) {
    ...
    $("#uploadFile").ajaxSubmit({
        type: 'post',
        url: '/realestate-inquiry-ui/dtcx/config/import/excel',
...
function configAddOrChange(data, model) {
    ...
    if (model === "fuZhi") {
        title = "复制配置";
        content = content + "?cxdh=" + data.cxdh + "&fuzhiCx=" + true
    }
...
function seeInfo(data) {
    var url = getIP() + "/realestate-inquiry-ui/view/dtcx/commonCx.html?cxdh=" + data.cxdh + "&cxmc=" + encodeURI(data.cxmc);
    //将地址中带的参数拼接到地址上
    var params = data.url.split("&")
...
function exportToExcel(data) {
    var url = "/realestate-inquiry-ui/dtcx/config/export/excel?cxdh=" + data.cxdh;
    window.open(url);
}
...
function delOne(obj) {
    layui.layer.confirm("该操作不可恢复，确认删除？", {
        ...
        btn1: function (index) {
            ...
            $.ajax({
                url: '/realestate-inquiry-ui/zdytj/del/' + cxid,
                type: 'delete',
                success: function (data) {
                    layer.msg("删除成功");
                    obj.del();
...
function fuZhi() {
    ...
    for (var index = 0; index < globalData.length; index++) {
        if (globalData[index].LAY_CHECKED == true) {
            size += 1;
            temp = index;
        }
    }
    ...
    configAddOrChange(globalData[temp], "fuZhi")
}