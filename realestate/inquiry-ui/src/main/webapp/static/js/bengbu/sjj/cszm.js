...
layui.use(['form', 'jquery', 'layer','laydate', 'element', 'table','laytpl'], function () {
  ...
  /*  var cszmData = [];
    $(function () {
        ...
        function search() {
            var cszh = $("#cszh").val();
            var xsemqxm = $("#xsemqxm").val();
            var xsefqxm = $("#xsefqxm").val();

            if (isNullOrEmpty(cszh) || isNullOrEmpty(xsemqxm) || isNullOrEmpty(xsefqxm)) {
                warnMsg("请输入查询接口完整条件！");
                return;
            }
            addModel();
            var searchData = {"cszh": cszh, "xsemqxm": xsemqxm, "xsefqxm": xsefqxm,"beanName":"bengbu_cszm" };
            // 查询婚姻登记信息
            $.ajax({
                url: "/realestate-inquiry-ui/rest/v1.0/queryByExchange/bengbu/csxx",
                type: 'GET',
                dataType: 'json',
                contentType: "application/json;charset=UTF-8",
                // data: encodeURI(JSON.stringify(searchData)),
                data: {
                    "cszh": cszh, "xsemqxm": xsemqxm, "xsefqxm": xsefqxm,"beanName":"bengbu_cszm"
                },
                success: function (data) {
                    if (data) {
