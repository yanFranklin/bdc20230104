...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    function generateSearchCondition(searchData) {
        $('#tyshxydm').val(searchData.paramDTOS[0].zjh);
        $('#name').val(searchData.paramDTOS[0].qlrmc);

        searchData.paramDTOS.forEach(function(item, index) {
            $('#tyshxydm-select').append('<option value="' + item.zjh + '"' + ' dm=' + item.qlrmc + '>' + item.zjh + '</option>');
        });

        form.render();
    }

    form.on('select(tyshxydm-select)', function (data) {
        $(this).parents('.layui-input-inline').find("input[name='tyshxydm']").val(data.value);
