...
layui.use(['jquery', 'element', 'form', 'table', 'laydate', 'formSelects'], function () {
    ...
    $(function () {
        ...
        function search() {
            ...
            addModel();
            // 重新请求
            table.reload("bdcdjywtjTable", {
                url: getContextPath() + "/rest/v1.0/ybtj/bdcywtj",
                where: obj,
                method: 'post',
                loadTotal: true,
                even: true,
    ...
    // 监听表格操作栏按钮
    table.on('toolbar(bdcdjywtjTable)', function (obj) {
    ...
    // 监听复选框事件，缓存选中的行数据
    table.on('checkbox(bdcdjywtjTable)', function (obj) {
        // 获取选中或者取消的数据
