...
// 列表需要展示的列名称
var showElement = {
    XNBDCDY_LZ:['alias','ip','viewTypeName','lsdyh','bdcdyh','lsdyhbdclx','methodResponse','time'],
    XNBDCDY_QXLZ:['alias','ip','viewTypeName','bdcdyh','qxlzxmid','methodResponse','time'],
    FCTDPP_PPTD:['alias','ip','viewTypeName','fcxmid','tdxmid','time'],
    FCTDPP_QXPPTD:['alias','ip','viewTypeName','fcxmid','time'],
};
...
var defaultCols = [
    ...
    {field: 'lsdyh', title: '临时单元号', width: 250, hide:true},
    {field: 'bdcdyh', title: '匹配单元号', width: 250, hide:true},
    {field: 'fcxmid', title: '房产项目ID', width: 180, hide:true},
    {field: 'tdxmid', title: '土地项目ID', width: 180, hide:true},
    {field: 'methodArgs', title: '请求参数', width: 200, hide:true},
    {field: 'methodResponse', title: '请求返回值', width: 200, hide:true, templet:function(d){
            if(d.methodResponse == "true"){
                return '<span class="" style="color:limegreen;">成功</span>';
            }else if(d.methodResponse == "false"){
                return '<span class="" style="color:red;">失败</span>';
            }
            return d.methodResponse;
        }
    },
...
layui.use(['form', 'jquery', 'element', 'table', 'laydate'], function () {
    ...
    form.on('select(eventName)', function(data){
        ...
        var getTpl = pplzSearchTpl.innerHTML
        var data = {
            zdList : zdList,
...
function pageInit(){
    ...
    cols = LogListCommon.showColsByConfig(defaultCols, showElement["XNBDCDY_LZ"]);
    var getTpl = pplzSearchTpl.innerHTML
    var data = {
        zdList : zdList,
        currentTime : Format(formatDate(new Date()), "yyyy-MM-dd"),
        event: "XNBDCDY_LZ",
        cols: showElement["XNBDCDY_LZ"],
    }
    laytpl(getTpl).render(data, function (html) {
        $("#pplz-search-content").html(html);
    });
