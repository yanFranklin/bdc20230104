...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            ...
            // 判空
            if (!isNotBlank(parentCertNum) ||
                !isNotBlank(parentName) || !isNotBlank(babyCertNum)
                || !isNotBlank(babyName)
                || !isNotBlank(checktype)
            ) {
                warnMsg('请输入必要查询条件');
                return false;
            }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data && data.length > 0) {
                                ...
                                data[0].check_type = formatQueryType(param.cxywcs[0].check_type);
