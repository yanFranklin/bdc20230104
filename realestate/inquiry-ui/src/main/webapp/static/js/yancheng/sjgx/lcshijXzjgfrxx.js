...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            // 获取查询条件
            var jgmc = $(this).parents('.layui-form-item').find('#jgmc').val();
            // 判空
            if (!isNotBlank(jgmc)) {
                warnMsg('请输入必要查询条件');
                return false;
            ...
            searchData.cxywcs.push({"jgmc": jgmc});
                ...
                success: function (data) {
                    ...
                    if (data && data.length > 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            getData.cxywcs.push({"jgmc": data[i].qlrmc});
                        }
                        searchData = getData;
                    }
                    // 展示查询条件，加载表格内容
        ...
        function search() {
            ...
            if(searchData.cxywcs.length >0){
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    ...
                    // 获取查询参数
                    var param ={
                        "jgmc":searchData.cxywcs[i].jgmc
                    };
                    // 重新请求
                    $.ajax({
                        url: "/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/xzjgfrxx",
                        type: "POST",
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijxzjgfrxx","市编办—行政机关法人信息查询",key,"市级共享交换平台");
        });
    });
