...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        $('#search').on('click', function () {
            // 获取查询条件
            var dsrdm = $(this).parents('.layui-form-item').find('#dsrdm').val();
            // 判空
            if (!isNotBlank(dsrdm)) {
                warnMsg('请输入必要查询条件');
                return false;
            ...
            searchData.cxywcs.push({"dsrdm": dsrdm});
        ...
        function search() {
            ...
            if (searchData.cxywcs && searchData.cxywcs.length > 0) {
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    // 获取查询参数
                    var param = {
                        dsrdm: searchData.cxywcs[i].dsrdm,
                    };
                    // 重新请求
                    $.ajax({
                        url: "/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/mstbcxajcjxx",
                        type: "POST",
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid, "mstbcxajcjxx", "司法局-民事特别程序案件裁判信息", key, "市级共享交换平台");
        });
    });
});
