...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            ...
            var jsdw = $(this).parents('.layui-form-item').find('#jsdw').val();
            // 判空
            if (!isNotBlank(zh) || !isNotBlank(jsdw)) {
                warnMsg('请输入必要查询条件');
                return false;
            }
            // 组织查询条件
            ...
            searchData.cxywcs.push({"zh": zh, "jsdw": jsdw});
            ...
            if(searchData.cxywcs &&searchData.cxywcs.length >0){
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    ...
                    var param = {
                        ...
                        jsdw:searchData.cxywcs[i].jsdw
                    };

                    // 重新请求
                    $.ajax({
                        url:"/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/yztgcxk",
                        type: "POST",
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijyztgcxk","市自然规划局—建设工程规划许可证",key,"市级共享交换平台");
        });
    });
});
