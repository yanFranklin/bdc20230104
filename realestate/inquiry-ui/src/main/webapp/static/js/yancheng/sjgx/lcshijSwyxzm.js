...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            // 获取查询条件
            var szzjhm = $(this).parents('.layui-form-item').find('#szzjhm').val();
            // 判空
            if (!isNotBlank(szzjhm)) {
                warnMsg('请输入必要查询条件');
                return false;
            }
            // 组织查询条件
            ...
            searchData.cxywcs.push({"szzjhm": szzjhm});
        ...
        function search() {
            ...
            if(searchData.cxywcs &&searchData.cxywcs.length >0) {
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    // 获取查询参数
                    var param = {
                        szzjhm : searchData.cxywcs[i].szzjhm
                    };
                    // 重新请求
                    $.ajax({
                        url:"/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/zhrkkswyxzm",
                        type: "POST",
                            ...
                            if (data.code == 200) {
                                ...
                                if(data.data  && data.data.length>0) {
                                    for(var i=0;i<data.data.length;i++){
                                        data.data[i].szzjhm = param.szzjhm;
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijzhrkkswyxzm","死亡医学证明查询(综合法人库)",key,"市级共享交换平台");
        });
    });
});
