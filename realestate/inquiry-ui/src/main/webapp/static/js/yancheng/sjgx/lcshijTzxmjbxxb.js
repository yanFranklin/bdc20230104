...
layui.use(['jquery', 'element', 'form', 'table', 'laydate'], function () {
    ...
    $(function () {
        ...
        $('#search').on('click', function () {
            // 获取查询条件
            var xmdm = $(this).parents('.layui-form-item').find('#xmdm').val();
            // 判空
            if (!isNotBlank(xmdm)) {
                warnMsg('请输入必要查询条件');
                return false;
            ...
            searchData.cxywcs.push({"xmdm": xmdm});
        ...
        function search() {
            ...
            if(searchData.cxywcs &&searchData.cxywcs.length >0) {
                for (var i = 0; i < searchData.cxywcs.length; i++) {
                    // 获取查询参数
                    var param = {
                        xmdm:searchData.cxywcs[i].xmdm
                    };
                    // 重新请求
                    $.ajax({
                        url:"/realestate-inquiry-ui/rest/v1.0/gx/shijgxCx/spjtzxmjbxx",
                        type: "POST",
        ...
        $(".upload-pdf").click(function () {
            uploadPdf(gzlslid,"shijspjtzxmjbxx","盐城市审批局-投资项目基本信息查询",key,"市级共享交换平台");
        });
    });
});
