...
layui.use(['jquery','element','formSelects','form','response'],function () {
    ...
    $.ajax({
        ...
        type: "POST",
        data:'qllx,bdclx,sf,dyhqllx,djxl,ygdjzl',
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data){
                    ...
                    formSelects.value('selectYgdjzl',[data.ygdjzl]);
                    formSelects.value('selectYqllx',[data.yqllx]);
                    if(data.pzid!=null){
                        $('#pzid').val(data.pzid);
                        djxlpzid = data.pzid;
                        enableNextBtn();
                    }
                    formSelects.value('selectLcscSfxx', [String(data.lcscsfxx)]);
    ...
    $('.nextstep').on('click',function () {
        ...
        window.location.href = 'step2.html?gzldyid=' + $('#gzldyid').val() + '&djxl=' + formSelects.value('selectDjxl')[0].DM + '&qllx=' + formSelects.value('selectQllx')[0].DM + "&djxlpzid=" + djxlpzid + "&bdclx=" + formSelects.value('selectBdclx')[0].DM;
    })
