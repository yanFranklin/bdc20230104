...
layui.use(['table', 'laytpl', 'laydate', 'layer', 'form'], function () {
    ...
    table.render({
                    ...
                    if (zdData && zdData.zssyqk) {
                        for (var index in zdData.zssyqk) {
                            if (zdData.zssyqk[index].DM == d.syqk) {
                                if(2 == d.syqk){ // 作废
                                    return '<span class="yzh-zf">' + zdData.zssyqk[index].MC + '</span>';
                                }
                                if(6 == d.syqk){
                                    return '<span class="yzh-yly">' + zdData.zssyqk[index].MC + '</span>';
                                }
        ...
        parseData: function (res) {
            ...
            $(".yzh-zf").each(function(index, ele){
                $(ele).parents("tr").children('td:not(:last-child)').css('background-color', 'red');
            });

            $(".yzh-yly").each(function(index, ele){
                $(ele).parents("tr").children('td:not(:last-child)').css('background-color', 'green');
            });
        }
    });
    ...
    function delZsyzh(data) {
        ...
        if (data.syqk == 3 || data.zsid ) {
            // 常州允许印制号使用后作废，需要同步清除证书记录中的印制号数据
        }
