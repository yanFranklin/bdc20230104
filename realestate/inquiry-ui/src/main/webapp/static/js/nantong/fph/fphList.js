...
var djZdList = getMulZdList("fwyt");
...
// 获取银行列表
    ...
    $('#search').on('click', function () {
        ...
        var departmentArr = checkedOrgcode;//ztree传来的部门编号
    ...
    function exportExcel() {
        ...
        obj["kssj"] = $("#kssj").val();
        obj["jzsj"] = $("#jzsj").val();
        var departmentArr = checkedOrgcode;
        if (departmentArr.length > 0) {
        ...
        var hj = 0;
        $.each(checkedData, function (key, value) {
            hj += parseFloat(value.hj);
        });
        hj = calculateFloat(hj);
    ...
    function exportYhExcel() {
        ...
        // 标题
        var showColsTitle = ["", "发票号", "受理编号", "登记原因", "用途", "缴费日期", "金额", "付款人", "坐落", "义务人", "收费日期", "不动产证明号"];
        // 列内容
        var showColsField = ["xh", "fph", "slbh", "djyy", "yt", "sfztczsj", "hj", "jfrxm", "zl", "ywr", 'sfztczsj', 'bdcqzh'];
        // 列宽
        var showColsWidth = ["10", "20", "20", "40", "20", "20", "20", "40", "40", "20", "20", "40"];
        ...
        for (var i = 1; i < data.length; i++) {
            data[i].xh = i;
            data[i].sfzt = formatSfzt(data[i].sfzt);
            data[i].yt = convertZdDmToMc("fwyt", data[i].yt, 'djZdList');
            data[i].sfztczsj = Format(formatDate(data[i].sfztczsj), "yyyy-M-d");
        }
        // 设置Excel基本信息
        $("#fileName").val('银行收费信息');
    ...
    function pldy() {
        ...
        $.each(data, function (index, item) {
            $.ajax({
                url: "/realestate-accept-ui/sf/nantong/sfmode/name",
                type: "GET",
                data: {gzlslid: item.gzlslid, configName: "fs.config.dzfp.beanName"},
                async: false,
            ...
            if (sftsfsxt) {
                if(sftsfsxt.indexOf("ykq")> -1){
                    ykqDownloadDzfp(item.sfxxid);
                }else {
                    downloadDzfp(item.sfxxid);
                }
            } else {
...
function downloadDzfp(param) {
    $.ajax({
        url: "/realestate-accept-ui/sf/printDzfpxx",
        type: "POST",
        contentType: "application/json;charset=UTF-8",
        data: param,
        async: false,
...
// 一卡清下载电子票据
function ykqDownloadDzfp(param){
    $.ajax({
        url:  "/realestate-accept-ui/sf/printDzfpxx",
        type: "POST",
        contentType: "application/json;charset=UTF-8",
        ...
        success: function (data) {
            removeModal();
            if(isNotBlank(data) && isNotBlank(data.fjxx)){
                $.each(data.fjxx, function(index, fjxx){
                    showFile(fjxx.fjnr, "");
                });
            }else{
                ityzl_SHOW_WARN_LAYER("未获取到电子票据信息。");
