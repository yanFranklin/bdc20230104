...
layui.use(['form', 'jquery', 'laydate', 'element', 'table','formSelects'], function () {
    ...
    $(function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: 'yxtcqzh', width: 250, title: 'TT系统证号', sort: true},
                ...
                {
                    field: 'ghyt', title: '规划用途', minWidth: 150,sort: true,
                    templet: function (d) {
                ...
                {field: 'zdzhmj', width: 100, title: '宗地面积', sort: true},
                {   field: 'zdzhyt', width: 150, title: '宗地用途', sort: true,
                    templet: function (d) {
                        return zdDmToMc('tdyt', d.zdzhyt, tdytList);
                    }
                },
                {
        ...
        function exportExcel(d, cols) {
            ...
            for (var index in cols) {
                if (cols[index].hide == false && cols[index].type != 'checkbox' && !cols[index].toolbar) {
                    ...
                    if ('zl' == cols[index].field) {
                        ...
                        showColsWidth.push(parseInt(width));
                    }
                }
            }
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                data[i].dh 	    = isNullOrEmpty(getFjxxVal(xmid, "dh")) ? "" : toEncryptValue(getFjxxVal(xmid, "dh"));
                data[i].djsj 	= formatDate(data[i].djsj);
                data[i].ghyt 	= zdGhytToMc('fwyt', data[i].ghyt, zdList);
                data[i].zdzhyt 	= zdDmToMc('tdyt', data[i].zdzhyt, tdytList);
        ...
        function printFczm(qlrxxList) {
            ...
            if(isNullOrEmpty(qjgldm)){
                failMsg("未获取到当前用户可查询的行政区划,请检查权限配置");
                return false;
            }
            $.ajax({
        ...
        formSelects.value('selectQjgldm', [defaultQjgldm]);
    ...
    window.setSearchData = function (data, flag) {
        ...
        if (searchData) {
            ...
            formSelects.value('selectQjgldm',  searchData['qjgldm'].split(","));
        }
    }
