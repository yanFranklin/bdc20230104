...
// (房屋)注销登记簿证明打印模板
var fwZxdjbDymb = getIP() + "/realestate-inquiry-ui/static/printModel/zxdjb.fr3";
// 证明验证对应的组合规则标识
// 有房无房证明
...
layui.use(['form', 'jquery', 'element', 'table'], function () {
    ...
    $(function () {
        ...
        var dylxArr = ["yfwfzm", "fwqszm", "tdqszm", "lqqszm","zxdjb"];
        ...
        table.on('toolbar(pageTable)', function (obj) {
            ...
            switch (obj.event) {
                ...
                case 'clearDataArr':
                    ...
                    break;
                case 'zxdjb':
                    zxdjb(checkStatus.data, obj.config.cols[0]);
                    break;
            }
        });
        ...
        function printFczm(qlrmc, qlrzjh,data) {
            ...
            $.ajax({
                ...
                type: "POST",
                data: JSON.stringify({"qlrmc": qlrmc, "zjh": qlrzjh, "jbr": useralias,"zfxxDTOList":data,moduleCode:moduleCode}),
                contentType: 'application/json',
                dataType: "text",
                success: function (key) {
                    ...
                    if (key) {
                        ...
                        printChoice("yfwfzm", "realestate-inquiry-ui", dataUrl, fczmDymb, false, sessionKey);
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data && data.redisKey) {
                            ...
                            savePrintInfo(mburl, dataUrl, {'zmbh': data.cxh, "printType": "权属证明", "zl":zl, "qlr": qlrmc}, xml);
                            // 记录打印权属证明数据库记录
                            saveZhcxLogToDataBase(getSearchData(), xml, 1, "qszm");
                        } else {
        ...
        // 注销登记簿
        function zxdjb(d,cols){
            var checkedData = layui.sessionData('checkedData');
            if ($.isEmptyObject(checkedData)) {
                warnMsg(" 请选择需要打印的记录！");
                return;
            }
            var qszmArr = getQsztCheckedVal();
            var flag = false;
            $.each(qszmArr, function (index, item) {
                if(item==='2'){
                    flag = true;
                }
            })
            if(!flag){
                warnMsg("请勾选历史状态，再点击注销登记簿！");
                return;
            }
            ...
            userInfo.orgRecordList.forEach(function (value) {
                bmmcList = value.name+",";
            })
            ...
            for (var i = 0; i <d.length; i++) {
                if (!isNullOrEmpty(d[i]) && d[i].qszt == 2) {
                    var qlrxx = new Array();
                    qlrxx.push(bmmcList);
                    qlrxx.push(d[i].xmid);
                    qlrxx.push(userInfo.alias);
                    qlrxx.push(userInfo.username);
            ...
            $.ajax({
                ...
                success: function (dbkey) {
                    if (!isNullOrEmpty(dbkey)) {
                        ...
                        var mburl = fwZxdjbDymb;
                        var appName = "realestate-inquiry-ui";

                        printChoice(dylx, appName, dataUrl, mburl, false, sessionKey);
                        // 保存日志
                        var xml = getXmlStr(dataUrl);
                        savePrintInfo(mburl, dataUrl, {'zmbh': dbkey, "printType": "注销登记簿"}, xml);
                        // 记录打印权属证明数据库记录
                        saveZhcxLogToDataBase(getSearchData(), xml, 1, "zsdjb");
                    } else {
                        failMsg("打印出错，请重试！");
                    }
                },
            ...
            layer.open({
                ...
                success: function (layero, index) {
                    ...
                    if (needsearch && searchData) {
                        ...
                        searchData.sfcxtszj = true;
                        searchData.zjhpx = true;
