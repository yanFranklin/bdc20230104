...
layui.use(['form', 'jquery', 'element', 'formSelects', 'layer', 'laytpl'], function () {
    ...
    $(function () {
        ...
        //身份证读卡器设置
        window.onReadIdCard = function (qlrlb, elem) {
            try {
               ...
               if (cardInfo.ReadCard()) {
                    ...
                    if (1 == qlrlb) {
                        if($(elem).parents(".bdc-add-qlr-box").length > 0){
                            $(elem).parents(".bdc-add-qlr-box").find('input[name="qlrmc"]').val(name);
                            $(elem).parents(".bdc-add-qlr-box").find('input[name="qlrzjh"]').val(number);
                        }else{
